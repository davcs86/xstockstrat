// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        (unknown)
// source: analysis/v1/analysis.proto

package analysisv1

import (
	v1 "github.com/xstockstrat/contracts/gen/go/common/v1"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	fieldmaskpb "google.golang.org/protobuf/types/known/fieldmaskpb"
	structpb "google.golang.org/protobuf/types/known/structpb"
	timestamppb "google.golang.org/protobuf/types/known/timestamppb"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type BacktestStatus int32

const (
	BacktestStatus_BACKTEST_STATUS_UNSPECIFIED       BacktestStatus = 0
	BacktestStatus_BACKTEST_STATUS_OK                BacktestStatus = 1
	BacktestStatus_BACKTEST_STATUS_INSUFFICIENT_DATA BacktestStatus = 2
)

// Enum value maps for BacktestStatus.
var (
	BacktestStatus_name = map[int32]string{
		0: "BACKTEST_STATUS_UNSPECIFIED",
		1: "BACKTEST_STATUS_OK",
		2: "BACKTEST_STATUS_INSUFFICIENT_DATA",
	}
	BacktestStatus_value = map[string]int32{
		"BACKTEST_STATUS_UNSPECIFIED":       0,
		"BACKTEST_STATUS_OK":                1,
		"BACKTEST_STATUS_INSUFFICIENT_DATA": 2,
	}
)

func (x BacktestStatus) Enum() *BacktestStatus {
	p := new(BacktestStatus)
	*p = x
	return p
}

func (x BacktestStatus) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (BacktestStatus) Descriptor() protoreflect.EnumDescriptor {
	return file_analysis_v1_analysis_proto_enumTypes[0].Descriptor()
}

func (BacktestStatus) Type() protoreflect.EnumType {
	return &file_analysis_v1_analysis_proto_enumTypes[0]
}

func (x BacktestStatus) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use BacktestStatus.Descriptor instead.
func (BacktestStatus) EnumDescriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{0}
}

// Backtest capital-allocation model (feature 150). Closed set → enum (C-04).
// A completed run records SIZING_MODE_LEGACY or SIZING_MODE_PORTFOLIO (never UNSPECIFIED);
// UNSPECIFIED is a request-side "unset → legacy" default only.
type SizingMode int32

const (
	SizingMode_SIZING_MODE_UNSPECIFIED SizingMode = 0 // request default → the legacy serial per-symbol path
	SizingMode_SIZING_MODE_LEGACY      SizingMode = 1 // serial per-symbol compounding (the aggregate is Π(1+rᵢ)−1)
	SizingMode_SIZING_MODE_PORTFOLIO   SizingMode = 2 // one shared cash pool, concurrent positions, one equity curve
)

// Enum value maps for SizingMode.
var (
	SizingMode_name = map[int32]string{
		0: "SIZING_MODE_UNSPECIFIED",
		1: "SIZING_MODE_LEGACY",
		2: "SIZING_MODE_PORTFOLIO",
	}
	SizingMode_value = map[string]int32{
		"SIZING_MODE_UNSPECIFIED": 0,
		"SIZING_MODE_LEGACY":      1,
		"SIZING_MODE_PORTFOLIO":   2,
	}
)

func (x SizingMode) Enum() *SizingMode {
	p := new(SizingMode)
	*p = x
	return p
}

func (x SizingMode) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (SizingMode) Descriptor() protoreflect.EnumDescriptor {
	return file_analysis_v1_analysis_proto_enumTypes[1].Descriptor()
}

func (SizingMode) Type() protoreflect.EnumType {
	return &file_analysis_v1_analysis_proto_enumTypes[1]
}

func (x SizingMode) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use SizingMode.Descriptor instead.
func (SizingMode) EnumDescriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{1}
}

// Which bar/price a backtest fills a signal at (feature 151). Closed set → enum (C-04).
// A completed run records SAME_BAR_CLOSE or NEXT_BAR_OPEN (never UNSPECIFIED); UNSPECIFIED is a
// request/config "not chosen" sentinel the servicer normalizes to SAME_BAR_CLOSE (legacy).
type FillModel int32

const (
	FillModel_FILL_MODEL_UNSPECIFIED    FillModel = 0 // caller/config did not choose → resolves to SAME_BAR_CLOSE (legacy)
	FillModel_FILL_MODEL_SAME_BAR_CLOSE FillModel = 1 // legacy: fill at bar i's close ± slippage (optimistically biased)
	FillModel_FILL_MODEL_NEXT_BAR_OPEN  FillModel = 2 // bias-free: fill a bar-i signal at bar (i+1)'s open ± slippage
)

// Enum value maps for FillModel.
var (
	FillModel_name = map[int32]string{
		0: "FILL_MODEL_UNSPECIFIED",
		1: "FILL_MODEL_SAME_BAR_CLOSE",
		2: "FILL_MODEL_NEXT_BAR_OPEN",
	}
	FillModel_value = map[string]int32{
		"FILL_MODEL_UNSPECIFIED":    0,
		"FILL_MODEL_SAME_BAR_CLOSE": 1,
		"FILL_MODEL_NEXT_BAR_OPEN":  2,
	}
)

func (x FillModel) Enum() *FillModel {
	p := new(FillModel)
	*p = x
	return p
}

func (x FillModel) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (FillModel) Descriptor() protoreflect.EnumDescriptor {
	return file_analysis_v1_analysis_proto_enumTypes[2].Descriptor()
}

func (FillModel) Type() protoreflect.EnumType {
	return &file_analysis_v1_analysis_proto_enumTypes[2]
}

func (x FillModel) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use FillModel.Descriptor instead.
func (FillModel) EnumDescriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{2}
}

// The engine's decision for a single bar. Closed set → enum (C-04).
type BarAction int32

const (
	BarAction_BAR_ACTION_UNSPECIFIED BarAction = 0
	BarAction_BAR_ACTION_WARMUP      BarAction = 1 // bar within the strategy's warm-up window
	BarAction_BAR_ACTION_HOLD_FLAT   BarAction = 2 // flat, no entry this bar
	BarAction_BAR_ACTION_ENTER_LONG  BarAction = 3 // opened a long position this bar
	BarAction_BAR_ACTION_EXIT_LONG   BarAction = 4 // closed a long position this bar
	BarAction_BAR_ACTION_HOLD_LONG   BarAction = 5 // holding an existing long, no exit this bar
)

// Enum value maps for BarAction.
var (
	BarAction_name = map[int32]string{
		0: "BAR_ACTION_UNSPECIFIED",
		1: "BAR_ACTION_WARMUP",
		2: "BAR_ACTION_HOLD_FLAT",
		3: "BAR_ACTION_ENTER_LONG",
		4: "BAR_ACTION_EXIT_LONG",
		5: "BAR_ACTION_HOLD_LONG",
	}
	BarAction_value = map[string]int32{
		"BAR_ACTION_UNSPECIFIED": 0,
		"BAR_ACTION_WARMUP":      1,
		"BAR_ACTION_HOLD_FLAT":   2,
		"BAR_ACTION_ENTER_LONG":  3,
		"BAR_ACTION_EXIT_LONG":   4,
		"BAR_ACTION_HOLD_LONG":   5,
	}
)

func (x BarAction) Enum() *BarAction {
	p := new(BarAction)
	*p = x
	return p
}

func (x BarAction) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (BarAction) Descriptor() protoreflect.EnumDescriptor {
	return file_analysis_v1_analysis_proto_enumTypes[3].Descriptor()
}

func (BarAction) Type() protoreflect.EnumType {
	return &file_analysis_v1_analysis_proto_enumTypes[3]
}

func (x BarAction) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use BarAction.Descriptor instead.
func (BarAction) EnumDescriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{3}
}

// Why a symbol produced zero trades. Closed set → enum (C-04).
type NoTradeReason int32

const (
	NoTradeReason_NO_TRADE_REASON_UNSPECIFIED          NoTradeReason = 0 // symbol traded, or not classified
	NoTradeReason_NO_TRADE_REASON_ENTIRE_RANGE_WARMUP  NoTradeReason = 1 // the whole range was warm-up
	NoTradeReason_NO_TRADE_REASON_ENTRY_NEVER_TRUE     NoTradeReason = 2 // entry condition never satisfied
	NoTradeReason_NO_TRADE_REASON_INSUFFICIENT_CAPITAL NoTradeReason = 3 // reserved; not emitted this version
	NoTradeReason_NO_TRADE_REASON_FORMULA_ERROR        NoTradeReason = 4 // a custom-formula component failed to execute / returned an out-of-contract series
)

// Enum value maps for NoTradeReason.
var (
	NoTradeReason_name = map[int32]string{
		0: "NO_TRADE_REASON_UNSPECIFIED",
		1: "NO_TRADE_REASON_ENTIRE_RANGE_WARMUP",
		2: "NO_TRADE_REASON_ENTRY_NEVER_TRUE",
		3: "NO_TRADE_REASON_INSUFFICIENT_CAPITAL",
		4: "NO_TRADE_REASON_FORMULA_ERROR",
	}
	NoTradeReason_value = map[string]int32{
		"NO_TRADE_REASON_UNSPECIFIED":          0,
		"NO_TRADE_REASON_ENTIRE_RANGE_WARMUP":  1,
		"NO_TRADE_REASON_ENTRY_NEVER_TRUE":     2,
		"NO_TRADE_REASON_INSUFFICIENT_CAPITAL": 3,
		"NO_TRADE_REASON_FORMULA_ERROR":        4,
	}
)

func (x NoTradeReason) Enum() *NoTradeReason {
	p := new(NoTradeReason)
	*p = x
	return p
}

func (x NoTradeReason) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (NoTradeReason) Descriptor() protoreflect.EnumDescriptor {
	return file_analysis_v1_analysis_proto_enumTypes[4].Descriptor()
}

func (NoTradeReason) Type() protoreflect.EnumType {
	return &file_analysis_v1_analysis_proto_enumTypes[4]
}

func (x NoTradeReason) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use NoTradeReason.Descriptor instead.
func (NoTradeReason) EnumDescriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{4}
}

type ComponentKind int32

const (
	ComponentKind_COMPONENT_KIND_UNSPECIFIED       ComponentKind = 0
	ComponentKind_COMPONENT_KIND_BUILTIN_INDICATOR ComponentKind = 1
	ComponentKind_COMPONENT_KIND_CUSTOM_FORMULA    ComponentKind = 2
)

// Enum value maps for ComponentKind.
var (
	ComponentKind_name = map[int32]string{
		0: "COMPONENT_KIND_UNSPECIFIED",
		1: "COMPONENT_KIND_BUILTIN_INDICATOR",
		2: "COMPONENT_KIND_CUSTOM_FORMULA",
	}
	ComponentKind_value = map[string]int32{
		"COMPONENT_KIND_UNSPECIFIED":       0,
		"COMPONENT_KIND_BUILTIN_INDICATOR": 1,
		"COMPONENT_KIND_CUSTOM_FORMULA":    2,
	}
)

func (x ComponentKind) Enum() *ComponentKind {
	p := new(ComponentKind)
	*p = x
	return p
}

func (x ComponentKind) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ComponentKind) Descriptor() protoreflect.EnumDescriptor {
	return file_analysis_v1_analysis_proto_enumTypes[5].Descriptor()
}

func (ComponentKind) Type() protoreflect.EnumType {
	return &file_analysis_v1_analysis_proto_enumTypes[5]
}

func (x ComponentKind) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ComponentKind.Descriptor instead.
func (ComponentKind) EnumDescriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{5}
}

type StrategyOperation int32

const (
	StrategyOperation_STRATEGY_OPERATION_UNSPECIFIED StrategyOperation = 0
	StrategyOperation_STRATEGY_OPERATION_REGISTER    StrategyOperation = 1
	StrategyOperation_STRATEGY_OPERATION_UPDATE      StrategyOperation = 2
	StrategyOperation_STRATEGY_OPERATION_DEACTIVATE  StrategyOperation = 3
	StrategyOperation_STRATEGY_OPERATION_REACTIVATE  StrategyOperation = 4 // set active=TRUE; re-validates the stored definition (feature 089)
)

// Enum value maps for StrategyOperation.
var (
	StrategyOperation_name = map[int32]string{
		0: "STRATEGY_OPERATION_UNSPECIFIED",
		1: "STRATEGY_OPERATION_REGISTER",
		2: "STRATEGY_OPERATION_UPDATE",
		3: "STRATEGY_OPERATION_DEACTIVATE",
		4: "STRATEGY_OPERATION_REACTIVATE",
	}
	StrategyOperation_value = map[string]int32{
		"STRATEGY_OPERATION_UNSPECIFIED": 0,
		"STRATEGY_OPERATION_REGISTER":    1,
		"STRATEGY_OPERATION_UPDATE":      2,
		"STRATEGY_OPERATION_DEACTIVATE":  3,
		"STRATEGY_OPERATION_REACTIVATE":  4,
	}
)

func (x StrategyOperation) Enum() *StrategyOperation {
	p := new(StrategyOperation)
	*p = x
	return p
}

func (x StrategyOperation) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (StrategyOperation) Descriptor() protoreflect.EnumDescriptor {
	return file_analysis_v1_analysis_proto_enumTypes[6].Descriptor()
}

func (StrategyOperation) Type() protoreflect.EnumType {
	return &file_analysis_v1_analysis_proto_enumTypes[6]
}

func (x StrategyOperation) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use StrategyOperation.Descriptor instead.
func (StrategyOperation) EnumDescriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{6}
}

// Comparator for a screen criterion's threshold test (closed set → enum).
type Comparator int32

const (
	Comparator_COMPARATOR_UNSPECIFIED Comparator = 0
	Comparator_COMPARATOR_LT          Comparator = 1
	Comparator_COMPARATOR_LTE         Comparator = 2
	Comparator_COMPARATOR_GT          Comparator = 3
	Comparator_COMPARATOR_GTE         Comparator = 4
	Comparator_COMPARATOR_BETWEEN     Comparator = 5 // threshold <= x <= threshold_high
)

// Enum value maps for Comparator.
var (
	Comparator_name = map[int32]string{
		0: "COMPARATOR_UNSPECIFIED",
		1: "COMPARATOR_LT",
		2: "COMPARATOR_LTE",
		3: "COMPARATOR_GT",
		4: "COMPARATOR_GTE",
		5: "COMPARATOR_BETWEEN",
	}
	Comparator_value = map[string]int32{
		"COMPARATOR_UNSPECIFIED": 0,
		"COMPARATOR_LT":          1,
		"COMPARATOR_LTE":         2,
		"COMPARATOR_GT":          3,
		"COMPARATOR_GTE":         4,
		"COMPARATOR_BETWEEN":     5,
	}
)

func (x Comparator) Enum() *Comparator {
	p := new(Comparator)
	*p = x
	return p
}

func (x Comparator) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (Comparator) Descriptor() protoreflect.EnumDescriptor {
	return file_analysis_v1_analysis_proto_enumTypes[7].Descriptor()
}

func (Comparator) Type() protoreflect.EnumType {
	return &file_analysis_v1_analysis_proto_enumTypes[7]
}

func (x Comparator) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use Comparator.Descriptor instead.
func (Comparator) EnumDescriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{7}
}

// What a screen criterion evaluates.
type ScreenKind int32

const (
	ScreenKind_SCREEN_KIND_UNSPECIFIED         ScreenKind = 0
	ScreenKind_SCREEN_KIND_FUNDAMENTAL         ScreenKind = 1 // a fundamental metric (metric_name)
	ScreenKind_SCREEN_KIND_TECHNICAL_FORMULA   ScreenKind = 2 // a custom formula (component)
	ScreenKind_SCREEN_KIND_TECHNICAL_INDICATOR ScreenKind = 3 // a built-in indicator (component)
	ScreenKind_SCREEN_KIND_SIGNAL              ScreenKind = 4 // source-weighted signal blend
)

// Enum value maps for ScreenKind.
var (
	ScreenKind_name = map[int32]string{
		0: "SCREEN_KIND_UNSPECIFIED",
		1: "SCREEN_KIND_FUNDAMENTAL",
		2: "SCREEN_KIND_TECHNICAL_FORMULA",
		3: "SCREEN_KIND_TECHNICAL_INDICATOR",
		4: "SCREEN_KIND_SIGNAL",
	}
	ScreenKind_value = map[string]int32{
		"SCREEN_KIND_UNSPECIFIED":         0,
		"SCREEN_KIND_FUNDAMENTAL":         1,
		"SCREEN_KIND_TECHNICAL_FORMULA":   2,
		"SCREEN_KIND_TECHNICAL_INDICATOR": 3,
		"SCREEN_KIND_SIGNAL":              4,
	}
)

func (x ScreenKind) Enum() *ScreenKind {
	p := new(ScreenKind)
	*p = x
	return p
}

func (x ScreenKind) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ScreenKind) Descriptor() protoreflect.EnumDescriptor {
	return file_analysis_v1_analysis_proto_enumTypes[8].Descriptor()
}

func (ScreenKind) Type() protoreflect.EnumType {
	return &file_analysis_v1_analysis_proto_enumTypes[8]
}

func (x ScreenKind) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ScreenKind.Descriptor instead.
func (ScreenKind) EnumDescriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{8}
}

type ScreenResultStatus int32

const (
	ScreenResultStatus_SCREEN_RESULT_STATUS_UNSPECIFIED       ScreenResultStatus = 0
	ScreenResultStatus_SCREEN_RESULT_STATUS_OK                ScreenResultStatus = 1
	ScreenResultStatus_SCREEN_RESULT_STATUS_INSUFFICIENT_DATA ScreenResultStatus = 2
)

// Enum value maps for ScreenResultStatus.
var (
	ScreenResultStatus_name = map[int32]string{
		0: "SCREEN_RESULT_STATUS_UNSPECIFIED",
		1: "SCREEN_RESULT_STATUS_OK",
		2: "SCREEN_RESULT_STATUS_INSUFFICIENT_DATA",
	}
	ScreenResultStatus_value = map[string]int32{
		"SCREEN_RESULT_STATUS_UNSPECIFIED":       0,
		"SCREEN_RESULT_STATUS_OK":                1,
		"SCREEN_RESULT_STATUS_INSUFFICIENT_DATA": 2,
	}
)

func (x ScreenResultStatus) Enum() *ScreenResultStatus {
	p := new(ScreenResultStatus)
	*p = x
	return p
}

func (x ScreenResultStatus) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ScreenResultStatus) Descriptor() protoreflect.EnumDescriptor {
	return file_analysis_v1_analysis_proto_enumTypes[9].Descriptor()
}

func (ScreenResultStatus) Type() protoreflect.EnumType {
	return &file_analysis_v1_analysis_proto_enumTypes[9]
}

func (x ScreenResultStatus) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ScreenResultStatus.Descriptor instead.
func (ScreenResultStatus) EnumDescriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{9}
}

// The action a ranked opportunity suggests. Closed set → enum (C-04).
// TRIM/EXIT are deliberately collapsed into REDUCE — the platform must not
// synthesize a "fully exit vs trim" boundary on a row that opens a real order
// ticket; the human chooses trim vs exit at the ticket (design.md § 1).
type OpportunityActionTag int32

const (
	OpportunityActionTag_OPPORTUNITY_ACTION_TAG_UNSPECIFIED OpportunityActionTag = 0
	OpportunityActionTag_OPPORTUNITY_ACTION_TAG_ENTER       OpportunityActionTag = 1 // buy signal, not currently held
	OpportunityActionTag_OPPORTUNITY_ACTION_TAG_ADD         OpportunityActionTag = 2 // buy signal, already held
	OpportunityActionTag_OPPORTUNITY_ACTION_TAG_REDUCE      OpportunityActionTag = 3 // sell signal, held (trim or exit — human decides)
)

// Enum value maps for OpportunityActionTag.
var (
	OpportunityActionTag_name = map[int32]string{
		0: "OPPORTUNITY_ACTION_TAG_UNSPECIFIED",
		1: "OPPORTUNITY_ACTION_TAG_ENTER",
		2: "OPPORTUNITY_ACTION_TAG_ADD",
		3: "OPPORTUNITY_ACTION_TAG_REDUCE",
	}
	OpportunityActionTag_value = map[string]int32{
		"OPPORTUNITY_ACTION_TAG_UNSPECIFIED": 0,
		"OPPORTUNITY_ACTION_TAG_ENTER":       1,
		"OPPORTUNITY_ACTION_TAG_ADD":         2,
		"OPPORTUNITY_ACTION_TAG_REDUCE":      3,
	}
)

func (x OpportunityActionTag) Enum() *OpportunityActionTag {
	p := new(OpportunityActionTag)
	*p = x
	return p
}

func (x OpportunityActionTag) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (OpportunityActionTag) Descriptor() protoreflect.EnumDescriptor {
	return file_analysis_v1_analysis_proto_enumTypes[10].Descriptor()
}

func (OpportunityActionTag) Type() protoreflect.EnumType {
	return &file_analysis_v1_analysis_proto_enumTypes[10]
}

func (x OpportunityActionTag) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use OpportunityActionTag.Descriptor instead.
func (OpportunityActionTag) EnumDescriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{10}
}

// Per-condition-leaf evaluation state. Closed set → enum (C-04).
type ConditionState int32

const (
	ConditionState_CONDITION_STATE_UNSPECIFIED ConditionState = 0
	ConditionState_CONDITION_STATE_PASS        ConditionState = 1 // leaf currently satisfied
	ConditionState_CONDITION_STATE_SOFT        ConditionState = 2 // within the soft-band of the threshold but not passing
	ConditionState_CONDITION_STATE_FAIL        ConditionState = 3 // not satisfied, outside the soft-band
)

// Enum value maps for ConditionState.
var (
	ConditionState_name = map[int32]string{
		0: "CONDITION_STATE_UNSPECIFIED",
		1: "CONDITION_STATE_PASS",
		2: "CONDITION_STATE_SOFT",
		3: "CONDITION_STATE_FAIL",
	}
	ConditionState_value = map[string]int32{
		"CONDITION_STATE_UNSPECIFIED": 0,
		"CONDITION_STATE_PASS":        1,
		"CONDITION_STATE_SOFT":        2,
		"CONDITION_STATE_FAIL":        3,
	}
)

func (x ConditionState) Enum() *ConditionState {
	p := new(ConditionState)
	*p = x
	return p
}

func (x ConditionState) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ConditionState) Descriptor() protoreflect.EnumDescriptor {
	return file_analysis_v1_analysis_proto_enumTypes[11].Descriptor()
}

func (ConditionState) Type() protoreflect.EnumType {
	return &file_analysis_v1_analysis_proto_enumTypes[11]
}

func (x ConditionState) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ConditionState.Descriptor instead.
func (ConditionState) EnumDescriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{11}
}

// Which rule tree EvaluateReadiness traces (feature 138). Closed set → enum (C-04).
type ReadinessRule int32

const (
	ReadinessRule_READINESS_RULE_UNSPECIFIED ReadinessRule = 0 // server treats as ENTRY (back-compat default)
	ReadinessRule_READINESS_RULE_ENTRY       ReadinessRule = 1 // trace the entry_rule (ENTER candidates, watchlist readiness)
	ReadinessRule_READINESS_RULE_EXIT        ReadinessRule = 2 // trace the exit_rule (held REDUCE/ADD opportunities)
)

// Enum value maps for ReadinessRule.
var (
	ReadinessRule_name = map[int32]string{
		0: "READINESS_RULE_UNSPECIFIED",
		1: "READINESS_RULE_ENTRY",
		2: "READINESS_RULE_EXIT",
	}
	ReadinessRule_value = map[string]int32{
		"READINESS_RULE_UNSPECIFIED": 0,
		"READINESS_RULE_ENTRY":       1,
		"READINESS_RULE_EXIT":        2,
	}
)

func (x ReadinessRule) Enum() *ReadinessRule {
	p := new(ReadinessRule)
	*p = x
	return p
}

func (x ReadinessRule) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ReadinessRule) Descriptor() protoreflect.EnumDescriptor {
	return file_analysis_v1_analysis_proto_enumTypes[12].Descriptor()
}

func (ReadinessRule) Type() protoreflect.EnumType {
	return &file_analysis_v1_analysis_proto_enumTypes[12]
}

func (x ReadinessRule) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ReadinessRule.Descriptor instead.
func (ReadinessRule) EnumDescriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{12}
}

// The persisted per-user disposition of a queued opportunity (feature 097). Closed set → enum (C-04).
type OpportunityAction int32

const (
	OpportunityAction_OPPORTUNITY_ACTION_UNSPECIFIED OpportunityAction = 0
	OpportunityAction_OPPORTUNITY_ACTION_SNOOZE      OpportunityAction = 1 // hide until snooze_until (bounded)
	OpportunityAction_OPPORTUNITY_ACTION_DISMISS     OpportunityAction = 2 // hide indefinitely
	OpportunityAction_OPPORTUNITY_ACTION_TAKE        OpportunityAction = 3 // user acted on it (feeds queue_share/taken reconciliation)
)

// Enum value maps for OpportunityAction.
var (
	OpportunityAction_name = map[int32]string{
		0: "OPPORTUNITY_ACTION_UNSPECIFIED",
		1: "OPPORTUNITY_ACTION_SNOOZE",
		2: "OPPORTUNITY_ACTION_DISMISS",
		3: "OPPORTUNITY_ACTION_TAKE",
	}
	OpportunityAction_value = map[string]int32{
		"OPPORTUNITY_ACTION_UNSPECIFIED": 0,
		"OPPORTUNITY_ACTION_SNOOZE":      1,
		"OPPORTUNITY_ACTION_DISMISS":     2,
		"OPPORTUNITY_ACTION_TAKE":        3,
	}
)

func (x OpportunityAction) Enum() *OpportunityAction {
	p := new(OpportunityAction)
	*p = x
	return p
}

func (x OpportunityAction) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (OpportunityAction) Descriptor() protoreflect.EnumDescriptor {
	return file_analysis_v1_analysis_proto_enumTypes[13].Descriptor()
}

func (OpportunityAction) Type() protoreflect.EnumType {
	return &file_analysis_v1_analysis_proto_enumTypes[13]
}

func (x OpportunityAction) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use OpportunityAction.Descriptor instead.
func (OpportunityAction) EnumDescriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{13}
}

// The order-lifecycle event a snapshot was captured at.
type SnapshotEventType int32

const (
	SnapshotEventType_SNAPSHOT_EVENT_TYPE_UNSPECIFIED            SnapshotEventType = 0
	SnapshotEventType_SNAPSHOT_EVENT_TYPE_ORDER_CREATED          SnapshotEventType = 1
	SnapshotEventType_SNAPSHOT_EVENT_TYPE_ORDER_FILLED           SnapshotEventType = 2
	SnapshotEventType_SNAPSHOT_EVENT_TYPE_ORDER_PARTIALLY_FILLED SnapshotEventType = 3
	SnapshotEventType_SNAPSHOT_EVENT_TYPE_ORDER_CANCELLED        SnapshotEventType = 4
)

// Enum value maps for SnapshotEventType.
var (
	SnapshotEventType_name = map[int32]string{
		0: "SNAPSHOT_EVENT_TYPE_UNSPECIFIED",
		1: "SNAPSHOT_EVENT_TYPE_ORDER_CREATED",
		2: "SNAPSHOT_EVENT_TYPE_ORDER_FILLED",
		3: "SNAPSHOT_EVENT_TYPE_ORDER_PARTIALLY_FILLED",
		4: "SNAPSHOT_EVENT_TYPE_ORDER_CANCELLED",
	}
	SnapshotEventType_value = map[string]int32{
		"SNAPSHOT_EVENT_TYPE_UNSPECIFIED":            0,
		"SNAPSHOT_EVENT_TYPE_ORDER_CREATED":          1,
		"SNAPSHOT_EVENT_TYPE_ORDER_FILLED":           2,
		"SNAPSHOT_EVENT_TYPE_ORDER_PARTIALLY_FILLED": 3,
		"SNAPSHOT_EVENT_TYPE_ORDER_CANCELLED":        4,
	}
)

func (x SnapshotEventType) Enum() *SnapshotEventType {
	p := new(SnapshotEventType)
	*p = x
	return p
}

func (x SnapshotEventType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (SnapshotEventType) Descriptor() protoreflect.EnumDescriptor {
	return file_analysis_v1_analysis_proto_enumTypes[14].Descriptor()
}

func (SnapshotEventType) Type() protoreflect.EnumType {
	return &file_analysis_v1_analysis_proto_enumTypes[14]
}

func (x SnapshotEventType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use SnapshotEventType.Descriptor instead.
func (SnapshotEventType) EnumDescriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{14}
}

// Whether an attribution factor is an indicator value-range or a signal presence.
type FactorType int32

const (
	FactorType_FACTOR_TYPE_UNSPECIFIED FactorType = 0
	FactorType_FACTOR_TYPE_INDICATOR   FactorType = 1
	FactorType_FACTOR_TYPE_SIGNAL      FactorType = 2
)

// Enum value maps for FactorType.
var (
	FactorType_name = map[int32]string{
		0: "FACTOR_TYPE_UNSPECIFIED",
		1: "FACTOR_TYPE_INDICATOR",
		2: "FACTOR_TYPE_SIGNAL",
	}
	FactorType_value = map[string]int32{
		"FACTOR_TYPE_UNSPECIFIED": 0,
		"FACTOR_TYPE_INDICATOR":   1,
		"FACTOR_TYPE_SIGNAL":      2,
	}
)

func (x FactorType) Enum() *FactorType {
	p := new(FactorType)
	*p = x
	return p
}

func (x FactorType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (FactorType) Descriptor() protoreflect.EnumDescriptor {
	return file_analysis_v1_analysis_proto_enumTypes[15].Descriptor()
}

func (FactorType) Type() protoreflect.EnumType {
	return &file_analysis_v1_analysis_proto_enumTypes[15]
}

func (x FactorType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use FactorType.Descriptor instead.
func (FactorType) EnumDescriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{15}
}

type RunBacktestRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	StrategyId     string                 `protobuf:"bytes,1,opt,name=strategy_id,json=strategyId,proto3" json:"strategy_id,omitempty"`
	Range          *v1.TimeRange          `protobuf:"bytes,2,opt,name=range,proto3" json:"range,omitempty"`
	Symbols        []string               `protobuf:"bytes,3,rep,name=symbols,proto3" json:"symbols,omitempty"`
	InitialCapital float64                `protobuf:"fixed64,4,opt,name=initial_capital,json=initialCapital,proto3" json:"initial_capital,omitempty"`
	StrategyParams *structpb.Struct       `protobuf:"bytes,5,opt,name=strategy_params,json=strategyParams,proto3" json:"strategy_params,omitempty"`
	// field 6 — resolve definition from DB; legacy strategy_params (field 5) remains supported
	StrategyIdRef string `protobuf:"bytes,6,opt,name=strategy_id_ref,json=strategyIdRef,proto3" json:"strategy_id_ref,omitempty"`
	// field 7 — inline definition; takes precedence over strategy_id_ref if both supplied
	InlineDefinition *StrategyDefinition `protobuf:"bytes,7,opt,name=inline_definition,json=inlineDefinition,proto3" json:"inline_definition,omitempty"`
	// field 8 — capital-allocation model (feature 150); unset/UNSPECIFIED → legacy serial per-symbol
	// path (no behavior change for existing callers).
	SizingMode SizingMode `protobuf:"varint,8,opt,name=sizing_mode,json=sizingMode,proto3,enum=xstockstrat.analysis.v1.SizingMode" json:"sizing_mode,omitempty"`
	// field 9 — fill model (feature 151); unset/UNSPECIFIED → server default (config
	// analysis.backtest.default_fill_model, else legacy same-bar-close). No behavior change for
	// existing callers.
	FillModel     FillModel `protobuf:"varint,9,opt,name=fill_model,json=fillModel,proto3,enum=xstockstrat.analysis.v1.FillModel" json:"fill_model,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RunBacktestRequest) Reset() {
	*x = RunBacktestRequest{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RunBacktestRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RunBacktestRequest) ProtoMessage() {}

func (x *RunBacktestRequest) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RunBacktestRequest.ProtoReflect.Descriptor instead.
func (*RunBacktestRequest) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{0}
}

func (x *RunBacktestRequest) GetStrategyId() string {
	if x != nil {
		return x.StrategyId
	}
	return ""
}

func (x *RunBacktestRequest) GetRange() *v1.TimeRange {
	if x != nil {
		return x.Range
	}
	return nil
}

func (x *RunBacktestRequest) GetSymbols() []string {
	if x != nil {
		return x.Symbols
	}
	return nil
}

func (x *RunBacktestRequest) GetInitialCapital() float64 {
	if x != nil {
		return x.InitialCapital
	}
	return 0
}

func (x *RunBacktestRequest) GetStrategyParams() *structpb.Struct {
	if x != nil {
		return x.StrategyParams
	}
	return nil
}

func (x *RunBacktestRequest) GetStrategyIdRef() string {
	if x != nil {
		return x.StrategyIdRef
	}
	return ""
}

func (x *RunBacktestRequest) GetInlineDefinition() *StrategyDefinition {
	if x != nil {
		return x.InlineDefinition
	}
	return nil
}

func (x *RunBacktestRequest) GetSizingMode() SizingMode {
	if x != nil {
		return x.SizingMode
	}
	return SizingMode_SIZING_MODE_UNSPECIFIED
}

func (x *RunBacktestRequest) GetFillModel() FillModel {
	if x != nil {
		return x.FillModel
	}
	return FillModel_FILL_MODEL_UNSPECIFIED
}

type CoverageGap struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	Symbol         string                 `protobuf:"bytes,1,opt,name=symbol,proto3" json:"symbol,omitempty"`
	Timeframe      v1.Timeframe           `protobuf:"varint,2,opt,name=timeframe,proto3,enum=xstockstrat.common.v1.Timeframe" json:"timeframe,omitempty"`
	RequestedRange *v1.TimeRange          `protobuf:"bytes,3,opt,name=requested_range,json=requestedRange,proto3" json:"requested_range,omitempty"`
	BarsHave       int64                  `protobuf:"varint,4,opt,name=bars_have,json=barsHave,proto3" json:"bars_have,omitempty"`
	BarsNeed       int64                  `protobuf:"varint,5,opt,name=bars_need,json=barsNeed,proto3" json:"bars_need,omitempty"`
	// The range a caller should backfill to satisfy this backtest.
	Gap           *v1.TimeRange `protobuf:"bytes,6,opt,name=gap,proto3" json:"gap,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CoverageGap) Reset() {
	*x = CoverageGap{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CoverageGap) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CoverageGap) ProtoMessage() {}

func (x *CoverageGap) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CoverageGap.ProtoReflect.Descriptor instead.
func (*CoverageGap) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{1}
}

func (x *CoverageGap) GetSymbol() string {
	if x != nil {
		return x.Symbol
	}
	return ""
}

func (x *CoverageGap) GetTimeframe() v1.Timeframe {
	if x != nil {
		return x.Timeframe
	}
	return v1.Timeframe(0)
}

func (x *CoverageGap) GetRequestedRange() *v1.TimeRange {
	if x != nil {
		return x.RequestedRange
	}
	return nil
}

func (x *CoverageGap) GetBarsHave() int64 {
	if x != nil {
		return x.BarsHave
	}
	return 0
}

func (x *CoverageGap) GetBarsNeed() int64 {
	if x != nil {
		return x.BarsNeed
	}
	return 0
}

func (x *CoverageGap) GetGap() *v1.TimeRange {
	if x != nil {
		return x.Gap
	}
	return nil
}

// NOTE: this message's wire bytes are persisted verbatim in analysis.backtest_details
// (feature 068, "store what you serve") — renumbering or retyping any field here or in
// BarDiagnostic silently corrupts previously persisted runs. Additive changes only;
// buf breaking guards this on every PR.
type BacktestResult struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	BacktestId       string                 `protobuf:"bytes,1,opt,name=backtest_id,json=backtestId,proto3" json:"backtest_id,omitempty"`
	StrategyId       string                 `protobuf:"bytes,2,opt,name=strategy_id,json=strategyId,proto3" json:"strategy_id,omitempty"`
	TotalReturn      float64                `protobuf:"fixed64,3,opt,name=total_return,json=totalReturn,proto3" json:"total_return,omitempty"`
	AnnualizedReturn float64                `protobuf:"fixed64,4,opt,name=annualized_return,json=annualizedReturn,proto3" json:"annualized_return,omitempty"`
	SharpeRatio      float64                `protobuf:"fixed64,5,opt,name=sharpe_ratio,json=sharpeRatio,proto3" json:"sharpe_ratio,omitempty"`
	MaxDrawdown      float64                `protobuf:"fixed64,6,opt,name=max_drawdown,json=maxDrawdown,proto3" json:"max_drawdown,omitempty"`
	WinRate          float64                `protobuf:"fixed64,7,opt,name=win_rate,json=winRate,proto3" json:"win_rate,omitempty"`
	TotalTrades      int32                  `protobuf:"varint,8,opt,name=total_trades,json=totalTrades,proto3" json:"total_trades,omitempty"`
	ProfitFactor     float64                `protobuf:"fixed64,9,opt,name=profit_factor,json=profitFactor,proto3" json:"profit_factor,omitempty"`
	CompletedAt      *timestamppb.Timestamp `protobuf:"bytes,10,opt,name=completed_at,json=completedAt,proto3" json:"completed_at,omitempty"`
	Trades           []*TradeRecord         `protobuf:"bytes,11,rep,name=trades,proto3" json:"trades,omitempty"`
	Status           BacktestStatus         `protobuf:"varint,12,opt,name=status,proto3,enum=xstockstrat.analysis.v1.BacktestStatus" json:"status,omitempty"`
	CoverageGaps     []*CoverageGap         `protobuf:"bytes,13,rep,name=coverage_gaps,json=coverageGaps,proto3" json:"coverage_gaps,omitempty"` // populated per-symbol when status == INSUFFICIENT_DATA
	Diagnostics      []*SymbolDiagnostics   `protobuf:"bytes,14,rep,name=diagnostics,proto3" json:"diagnostics,omitempty"`                       // per-bar debug data for every simulated symbol (feature 064)
	// Effective starting capital the engine seeded the simulation with (the 100k default
	// when the request omitted it) — required to rebuild the equity curve for a
	// historical run (feature 068).
	InitialCapital float64 `protobuf:"fixed64,15,opt,name=initial_capital,json=initialCapital,proto3" json:"initial_capital,omitempty"`
	// Human-readable run warnings surfaced to the user (feature 086), e.g. a strategy referenced a
	// formula that has since been soft-deleted — the run still completed using its last-saved
	// definition. Empty on a clean run.
	Warnings []string `protobuf:"bytes,16,rep,name=warnings,proto3" json:"warnings,omitempty"`
	// ── Portfolio sizing (feature 150) ── additive; empty/legacy for pre-150 and legacy-mode runs.
	SizingMode   SizingMode              `protobuf:"varint,17,opt,name=sizing_mode,json=sizingMode,proto3,enum=xstockstrat.analysis.v1.SizingMode" json:"sizing_mode,omitempty"` // the mode actually used (never UNSPECIFIED on a completed run)
	CapitalSkips []*PortfolioCapitalSkip `protobuf:"bytes,18,rep,name=capital_skips,json=capitalSkips,proto3" json:"capital_skips,omitempty"`                                    // portfolio mode only; empty in legacy
	// Portfolio-level daily equity curve (cash + Σ marked-to-market positions), portfolio mode only.
	// NOTE: per-symbol BarDiagnostic.equity (field 15 there) stays per-symbol in portfolio mode — the
	// portfolio contribution lives ONLY here; do not read per-symbol equity as portfolio contribution.
	PortfolioEquityCurve []*EquityPoint `protobuf:"bytes,19,rep,name=portfolio_equity_curve,json=portfolioEquityCurve,proto3" json:"portfolio_equity_curve,omitempty"`
	// Effective fill model the run actually used (feature 151); never UNSPECIFIED on a completed run.
	FillModel     FillModel `protobuf:"varint,20,opt,name=fill_model,json=fillModel,proto3,enum=xstockstrat.analysis.v1.FillModel" json:"fill_model,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BacktestResult) Reset() {
	*x = BacktestResult{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BacktestResult) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BacktestResult) ProtoMessage() {}

func (x *BacktestResult) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BacktestResult.ProtoReflect.Descriptor instead.
func (*BacktestResult) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{2}
}

func (x *BacktestResult) GetBacktestId() string {
	if x != nil {
		return x.BacktestId
	}
	return ""
}

func (x *BacktestResult) GetStrategyId() string {
	if x != nil {
		return x.StrategyId
	}
	return ""
}

func (x *BacktestResult) GetTotalReturn() float64 {
	if x != nil {
		return x.TotalReturn
	}
	return 0
}

func (x *BacktestResult) GetAnnualizedReturn() float64 {
	if x != nil {
		return x.AnnualizedReturn
	}
	return 0
}

func (x *BacktestResult) GetSharpeRatio() float64 {
	if x != nil {
		return x.SharpeRatio
	}
	return 0
}

func (x *BacktestResult) GetMaxDrawdown() float64 {
	if x != nil {
		return x.MaxDrawdown
	}
	return 0
}

func (x *BacktestResult) GetWinRate() float64 {
	if x != nil {
		return x.WinRate
	}
	return 0
}

func (x *BacktestResult) GetTotalTrades() int32 {
	if x != nil {
		return x.TotalTrades
	}
	return 0
}

func (x *BacktestResult) GetProfitFactor() float64 {
	if x != nil {
		return x.ProfitFactor
	}
	return 0
}

func (x *BacktestResult) GetCompletedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.CompletedAt
	}
	return nil
}

func (x *BacktestResult) GetTrades() []*TradeRecord {
	if x != nil {
		return x.Trades
	}
	return nil
}

func (x *BacktestResult) GetStatus() BacktestStatus {
	if x != nil {
		return x.Status
	}
	return BacktestStatus_BACKTEST_STATUS_UNSPECIFIED
}

func (x *BacktestResult) GetCoverageGaps() []*CoverageGap {
	if x != nil {
		return x.CoverageGaps
	}
	return nil
}

func (x *BacktestResult) GetDiagnostics() []*SymbolDiagnostics {
	if x != nil {
		return x.Diagnostics
	}
	return nil
}

func (x *BacktestResult) GetInitialCapital() float64 {
	if x != nil {
		return x.InitialCapital
	}
	return 0
}

func (x *BacktestResult) GetWarnings() []string {
	if x != nil {
		return x.Warnings
	}
	return nil
}

func (x *BacktestResult) GetSizingMode() SizingMode {
	if x != nil {
		return x.SizingMode
	}
	return SizingMode_SIZING_MODE_UNSPECIFIED
}

func (x *BacktestResult) GetCapitalSkips() []*PortfolioCapitalSkip {
	if x != nil {
		return x.CapitalSkips
	}
	return nil
}

func (x *BacktestResult) GetPortfolioEquityCurve() []*EquityPoint {
	if x != nil {
		return x.PortfolioEquityCurve
	}
	return nil
}

func (x *BacktestResult) GetFillModel() FillModel {
	if x != nil {
		return x.FillModel
	}
	return FillModel_FILL_MODEL_UNSPECIFIED
}

// One entry that portfolio mode could not open because the shared pool was fully committed
// at the policy weight (feature 150, FR-5). Emitted instead of a silent zero-sized fill.
type PortfolioCapitalSkip struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	Symbol         string                 `protobuf:"bytes,1,opt,name=symbol,proto3" json:"symbol,omitempty"`
	Timestamp      *timestamppb.Timestamp `protobuf:"bytes,2,opt,name=timestamp,proto3" json:"timestamp,omitempty"`
	IntendedWeight float64                `protobuf:"fixed64,3,opt,name=intended_weight,json=intendedWeight,proto3" json:"intended_weight,omitempty"` // position_weight × initial_capital the entry would have needed
	AvailableCash  float64                `protobuf:"fixed64,4,opt,name=available_cash,json=availableCash,proto3" json:"available_cash,omitempty"`    // cash on hand in the shared pool at that bar
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *PortfolioCapitalSkip) Reset() {
	*x = PortfolioCapitalSkip{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PortfolioCapitalSkip) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PortfolioCapitalSkip) ProtoMessage() {}

func (x *PortfolioCapitalSkip) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PortfolioCapitalSkip.ProtoReflect.Descriptor instead.
func (*PortfolioCapitalSkip) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{3}
}

func (x *PortfolioCapitalSkip) GetSymbol() string {
	if x != nil {
		return x.Symbol
	}
	return ""
}

func (x *PortfolioCapitalSkip) GetTimestamp() *timestamppb.Timestamp {
	if x != nil {
		return x.Timestamp
	}
	return nil
}

func (x *PortfolioCapitalSkip) GetIntendedWeight() float64 {
	if x != nil {
		return x.IntendedWeight
	}
	return 0
}

func (x *PortfolioCapitalSkip) GetAvailableCash() float64 {
	if x != nil {
		return x.AvailableCash
	}
	return 0
}

// One point of the portfolio-level daily equity curve (cash + Σ marked-to-market positions).
type EquityPoint struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Timestamp     *timestamppb.Timestamp `protobuf:"bytes,1,opt,name=timestamp,proto3" json:"timestamp,omitempty"`
	Equity        float64                `protobuf:"fixed64,2,opt,name=equity,proto3" json:"equity,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EquityPoint) Reset() {
	*x = EquityPoint{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EquityPoint) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EquityPoint) ProtoMessage() {}

func (x *EquityPoint) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EquityPoint.ProtoReflect.Descriptor instead.
func (*EquityPoint) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{4}
}

func (x *EquityPoint) GetTimestamp() *timestamppb.Timestamp {
	if x != nil {
		return x.Timestamp
	}
	return nil
}

func (x *EquityPoint) GetEquity() float64 {
	if x != nil {
		return x.Equity
	}
	return 0
}

type TradeRecord struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Symbol        string                 `protobuf:"bytes,1,opt,name=symbol,proto3" json:"symbol,omitempty"`
	Side          string                 `protobuf:"bytes,2,opt,name=side,proto3" json:"side,omitempty"`
	Qty           float64                `protobuf:"fixed64,3,opt,name=qty,proto3" json:"qty,omitempty"`
	EntryPrice    float64                `protobuf:"fixed64,4,opt,name=entry_price,json=entryPrice,proto3" json:"entry_price,omitempty"`
	ExitPrice     float64                `protobuf:"fixed64,5,opt,name=exit_price,json=exitPrice,proto3" json:"exit_price,omitempty"`
	Pnl           float64                `protobuf:"fixed64,6,opt,name=pnl,proto3" json:"pnl,omitempty"`
	EntryTime     *timestamppb.Timestamp `protobuf:"bytes,7,opt,name=entry_time,json=entryTime,proto3" json:"entry_time,omitempty"`
	ExitTime      *timestamppb.Timestamp `protobuf:"bytes,8,opt,name=exit_time,json=exitTime,proto3" json:"exit_time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TradeRecord) Reset() {
	*x = TradeRecord{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TradeRecord) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TradeRecord) ProtoMessage() {}

func (x *TradeRecord) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TradeRecord.ProtoReflect.Descriptor instead.
func (*TradeRecord) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{5}
}

func (x *TradeRecord) GetSymbol() string {
	if x != nil {
		return x.Symbol
	}
	return ""
}

func (x *TradeRecord) GetSide() string {
	if x != nil {
		return x.Side
	}
	return ""
}

func (x *TradeRecord) GetQty() float64 {
	if x != nil {
		return x.Qty
	}
	return 0
}

func (x *TradeRecord) GetEntryPrice() float64 {
	if x != nil {
		return x.EntryPrice
	}
	return 0
}

func (x *TradeRecord) GetExitPrice() float64 {
	if x != nil {
		return x.ExitPrice
	}
	return 0
}

func (x *TradeRecord) GetPnl() float64 {
	if x != nil {
		return x.Pnl
	}
	return 0
}

func (x *TradeRecord) GetEntryTime() *timestamppb.Timestamp {
	if x != nil {
		return x.EntryTime
	}
	return nil
}

func (x *TradeRecord) GetExitTime() *timestamppb.Timestamp {
	if x != nil {
		return x.ExitTime
	}
	return nil
}

// One row of day-by-day backtest diagnostics for a single bar.
type BarDiagnostic struct {
	state       protoimpl.MessageState `protogen:"open.v1"`
	Symbol      string                 `protobuf:"bytes,1,opt,name=symbol,proto3" json:"symbol,omitempty"`
	BarIndex    int32                  `protobuf:"varint,2,opt,name=bar_index,json=barIndex,proto3" json:"bar_index,omitempty"`
	Timestamp   *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=timestamp,proto3" json:"timestamp,omitempty"`
	Open        float64                `protobuf:"fixed64,4,opt,name=open,proto3" json:"open,omitempty"`
	High        float64                `protobuf:"fixed64,5,opt,name=high,proto3" json:"high,omitempty"`
	Low         float64                `protobuf:"fixed64,6,opt,name=low,proto3" json:"low,omitempty"`
	Close       float64                `protobuf:"fixed64,7,opt,name=close,proto3" json:"close,omitempty"`
	Volume      int64                  `protobuf:"varint,8,opt,name=volume,proto3" json:"volume,omitempty"`
	Vwap        float64                `protobuf:"fixed64,9,opt,name=vwap,proto3" json:"vwap,omitempty"`
	Indicators  map[string]float64     `protobuf:"bytes,10,rep,name=indicators,proto3" json:"indicators,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"fixed64,2,opt,name=value"` // present-only: a series is absent during its warm-up
	Warmup      bool                   `protobuf:"varint,11,opt,name=warmup,proto3" json:"warmup,omitempty"`
	SignalScore float64                `protobuf:"fixed64,12,opt,name=signal_score,json=signalScore,proto3" json:"signal_score,omitempty"`
	Conviction  float64                `protobuf:"fixed64,13,opt,name=conviction,proto3" json:"conviction,omitempty"`
	Action      BarAction              `protobuf:"varint,14,opt,name=action,proto3,enum=xstockstrat.analysis.v1.BarAction" json:"action,omitempty"`
	// Portfolio value (cash + position * close) after this bar — the per-bar equity
	// point the time-based equity curve plots (feature 068).
	Equity        float64 `protobuf:"fixed64,15,opt,name=equity,proto3" json:"equity,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BarDiagnostic) Reset() {
	*x = BarDiagnostic{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BarDiagnostic) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BarDiagnostic) ProtoMessage() {}

func (x *BarDiagnostic) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BarDiagnostic.ProtoReflect.Descriptor instead.
func (*BarDiagnostic) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{6}
}

func (x *BarDiagnostic) GetSymbol() string {
	if x != nil {
		return x.Symbol
	}
	return ""
}

func (x *BarDiagnostic) GetBarIndex() int32 {
	if x != nil {
		return x.BarIndex
	}
	return 0
}

func (x *BarDiagnostic) GetTimestamp() *timestamppb.Timestamp {
	if x != nil {
		return x.Timestamp
	}
	return nil
}

func (x *BarDiagnostic) GetOpen() float64 {
	if x != nil {
		return x.Open
	}
	return 0
}

func (x *BarDiagnostic) GetHigh() float64 {
	if x != nil {
		return x.High
	}
	return 0
}

func (x *BarDiagnostic) GetLow() float64 {
	if x != nil {
		return x.Low
	}
	return 0
}

func (x *BarDiagnostic) GetClose() float64 {
	if x != nil {
		return x.Close
	}
	return 0
}

func (x *BarDiagnostic) GetVolume() int64 {
	if x != nil {
		return x.Volume
	}
	return 0
}

func (x *BarDiagnostic) GetVwap() float64 {
	if x != nil {
		return x.Vwap
	}
	return 0
}

func (x *BarDiagnostic) GetIndicators() map[string]float64 {
	if x != nil {
		return x.Indicators
	}
	return nil
}

func (x *BarDiagnostic) GetWarmup() bool {
	if x != nil {
		return x.Warmup
	}
	return false
}

func (x *BarDiagnostic) GetSignalScore() float64 {
	if x != nil {
		return x.SignalScore
	}
	return 0
}

func (x *BarDiagnostic) GetConviction() float64 {
	if x != nil {
		return x.Conviction
	}
	return 0
}

func (x *BarDiagnostic) GetAction() BarAction {
	if x != nil {
		return x.Action
	}
	return BarAction_BAR_ACTION_UNSPECIFIED
}

func (x *BarDiagnostic) GetEquity() float64 {
	if x != nil {
		return x.Equity
	}
	return 0
}

// Per-symbol diagnostics bundle attached to a BacktestResult.
type SymbolDiagnostics struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Symbol        string                 `protobuf:"bytes,1,opt,name=symbol,proto3" json:"symbol,omitempty"`
	Bars          []*BarDiagnostic       `protobuf:"bytes,2,rep,name=bars,proto3" json:"bars,omitempty"`
	NoTradeReason NoTradeReason          `protobuf:"varint,3,opt,name=no_trade_reason,json=noTradeReason,proto3,enum=xstockstrat.analysis.v1.NoTradeReason" json:"no_trade_reason,omitempty"`
	BarsTotal     int32                  `protobuf:"varint,4,opt,name=bars_total,json=barsTotal,proto3" json:"bars_total,omitempty"`
	WarmupBars    int32                  `protobuf:"varint,5,opt,name=warmup_bars,json=warmupBars,proto3" json:"warmup_bars,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SymbolDiagnostics) Reset() {
	*x = SymbolDiagnostics{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SymbolDiagnostics) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SymbolDiagnostics) ProtoMessage() {}

func (x *SymbolDiagnostics) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SymbolDiagnostics.ProtoReflect.Descriptor instead.
func (*SymbolDiagnostics) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{7}
}

func (x *SymbolDiagnostics) GetSymbol() string {
	if x != nil {
		return x.Symbol
	}
	return ""
}

func (x *SymbolDiagnostics) GetBars() []*BarDiagnostic {
	if x != nil {
		return x.Bars
	}
	return nil
}

func (x *SymbolDiagnostics) GetNoTradeReason() NoTradeReason {
	if x != nil {
		return x.NoTradeReason
	}
	return NoTradeReason_NO_TRADE_REASON_UNSPECIFIED
}

func (x *SymbolDiagnostics) GetBarsTotal() int32 {
	if x != nil {
		return x.BarsTotal
	}
	return 0
}

func (x *SymbolDiagnostics) GetWarmupBars() int32 {
	if x != nil {
		return x.WarmupBars
	}
	return 0
}

type ScoreStrategyRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StrategyId    string                 `protobuf:"bytes,1,opt,name=strategy_id,json=strategyId,proto3" json:"strategy_id,omitempty"`
	Range         *v1.TimeRange          `protobuf:"bytes,2,opt,name=range,proto3" json:"range,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ScoreStrategyRequest) Reset() {
	*x = ScoreStrategyRequest{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ScoreStrategyRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ScoreStrategyRequest) ProtoMessage() {}

func (x *ScoreStrategyRequest) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ScoreStrategyRequest.ProtoReflect.Descriptor instead.
func (*ScoreStrategyRequest) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{8}
}

func (x *ScoreStrategyRequest) GetStrategyId() string {
	if x != nil {
		return x.StrategyId
	}
	return ""
}

func (x *ScoreStrategyRequest) GetRange() *v1.TimeRange {
	if x != nil {
		return x.Range
	}
	return nil
}

type StrategyScore struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	StrategyId      string                 `protobuf:"bytes,1,opt,name=strategy_id,json=strategyId,proto3" json:"strategy_id,omitempty"`
	OverallScore    float64                `protobuf:"fixed64,2,opt,name=overall_score,json=overallScore,proto3" json:"overall_score,omitempty"`
	ComponentScores map[string]float64     `protobuf:"bytes,3,rep,name=component_scores,json=componentScores,proto3" json:"component_scores,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"fixed64,2,opt,name=value"` // e.g. sharpe, drawdown, consistency
	Rating          string                 `protobuf:"bytes,4,opt,name=rating,proto3" json:"rating,omitempty"`                                                                                                                      // A/B/C/D/F
	// Evidence provenance for the derived cross-stock headline grade (feature 065).
	EvidenceSymbols int32 `protobuf:"varint,5,opt,name=evidence_symbols,json=evidenceSymbols,proto3" json:"evidence_symbols,omitempty"` // distinct symbols contributing eligible evidence cells
	EvidenceDays    int32 `protobuf:"varint,6,opt,name=evidence_days,json=evidenceDays,proto3" json:"evidence_days,omitempty"`          // total trading days of evidence across those symbols
	Provisional     bool  `protobuf:"varint,7,opt,name=provisional,proto3" json:"provisional,omitempty"`                                // true when evidence is below the symbol/day floor
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *StrategyScore) Reset() {
	*x = StrategyScore{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StrategyScore) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StrategyScore) ProtoMessage() {}

func (x *StrategyScore) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StrategyScore.ProtoReflect.Descriptor instead.
func (*StrategyScore) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{9}
}

func (x *StrategyScore) GetStrategyId() string {
	if x != nil {
		return x.StrategyId
	}
	return ""
}

func (x *StrategyScore) GetOverallScore() float64 {
	if x != nil {
		return x.OverallScore
	}
	return 0
}

func (x *StrategyScore) GetComponentScores() map[string]float64 {
	if x != nil {
		return x.ComponentScores
	}
	return nil
}

func (x *StrategyScore) GetRating() string {
	if x != nil {
		return x.Rating
	}
	return ""
}

func (x *StrategyScore) GetEvidenceSymbols() int32 {
	if x != nil {
		return x.EvidenceSymbols
	}
	return 0
}

func (x *StrategyScore) GetEvidenceDays() int32 {
	if x != nil {
		return x.EvidenceDays
	}
	return 0
}

func (x *StrategyScore) GetProvisional() bool {
	if x != nil {
		return x.Provisional
	}
	return false
}

type StrategyReport struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	StrategyId     string                 `protobuf:"bytes,1,opt,name=strategy_id,json=strategyId,proto3" json:"strategy_id,omitempty"`
	LatestBacktest *BacktestResult        `protobuf:"bytes,2,opt,name=latest_backtest,json=latestBacktest,proto3" json:"latest_backtest,omitempty"`
	Score          *StrategyScore         `protobuf:"bytes,3,opt,name=score,proto3" json:"score,omitempty"`
	Metadata       *structpb.Struct       `protobuf:"bytes,4,opt,name=metadata,proto3" json:"metadata,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *StrategyReport) Reset() {
	*x = StrategyReport{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StrategyReport) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StrategyReport) ProtoMessage() {}

func (x *StrategyReport) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StrategyReport.ProtoReflect.Descriptor instead.
func (*StrategyReport) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{10}
}

func (x *StrategyReport) GetStrategyId() string {
	if x != nil {
		return x.StrategyId
	}
	return ""
}

func (x *StrategyReport) GetLatestBacktest() *BacktestResult {
	if x != nil {
		return x.LatestBacktest
	}
	return nil
}

func (x *StrategyReport) GetScore() *StrategyScore {
	if x != nil {
		return x.Score
	}
	return nil
}

func (x *StrategyReport) GetMetadata() *structpb.Struct {
	if x != nil {
		return x.Metadata
	}
	return nil
}

type ListBacktestsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StrategyId    string                 `protobuf:"bytes,1,opt,name=strategy_id,json=strategyId,proto3" json:"strategy_id,omitempty"`
	Limit         int32                  `protobuf:"varint,2,opt,name=limit,proto3" json:"limit,omitempty"` // 0 → server default (most recent 20)
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListBacktestsRequest) Reset() {
	*x = ListBacktestsRequest{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListBacktestsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListBacktestsRequest) ProtoMessage() {}

func (x *ListBacktestsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListBacktestsRequest.ProtoReflect.Descriptor instead.
func (*ListBacktestsRequest) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{11}
}

func (x *ListBacktestsRequest) GetStrategyId() string {
	if x != nil {
		return x.StrategyId
	}
	return ""
}

func (x *ListBacktestsRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

type BacktestRunSummary struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	BacktestId       string                 `protobuf:"bytes,1,opt,name=backtest_id,json=backtestId,proto3" json:"backtest_id,omitempty"`
	StrategyId       string                 `protobuf:"bytes,2,opt,name=strategy_id,json=strategyId,proto3" json:"strategy_id,omitempty"`
	Status           BacktestStatus         `protobuf:"varint,3,opt,name=status,proto3,enum=xstockstrat.analysis.v1.BacktestStatus" json:"status,omitempty"`
	TotalReturn      float64                `protobuf:"fixed64,4,opt,name=total_return,json=totalReturn,proto3" json:"total_return,omitempty"`
	AnnualizedReturn float64                `protobuf:"fixed64,5,opt,name=annualized_return,json=annualizedReturn,proto3" json:"annualized_return,omitempty"`
	SharpeRatio      float64                `protobuf:"fixed64,6,opt,name=sharpe_ratio,json=sharpeRatio,proto3" json:"sharpe_ratio,omitempty"`
	MaxDrawdown      float64                `protobuf:"fixed64,7,opt,name=max_drawdown,json=maxDrawdown,proto3" json:"max_drawdown,omitempty"`
	WinRate          float64                `protobuf:"fixed64,8,opt,name=win_rate,json=winRate,proto3" json:"win_rate,omitempty"`
	TotalTrades      int32                  `protobuf:"varint,9,opt,name=total_trades,json=totalTrades,proto3" json:"total_trades,omitempty"`
	ProfitFactor     float64                `protobuf:"fixed64,10,opt,name=profit_factor,json=profitFactor,proto3" json:"profit_factor,omitempty"`
	Symbols          []string               `protobuf:"bytes,11,rep,name=symbols,proto3" json:"symbols,omitempty"`
	OverallScore     float64                `protobuf:"fixed64,12,opt,name=overall_score,json=overallScore,proto3" json:"overall_score,omitempty"` // 0 when the run earned no score (e.g. INSUFFICIENT_DATA)
	Rating           string                 `protobuf:"bytes,13,opt,name=rating,proto3" json:"rating,omitempty"`                                   // "" when the run earned no score
	CompletedAt      *timestamppb.Timestamp `protobuf:"bytes,14,opt,name=completed_at,json=completedAt,proto3" json:"completed_at,omitempty"`
	// Backtest range covered by this run (feature 065); unset on legacy rows.
	RangeStart    *timestamppb.Timestamp `protobuf:"bytes,15,opt,name=range_start,json=rangeStart,proto3" json:"range_start,omitempty"`
	RangeEnd      *timestamppb.Timestamp `protobuf:"bytes,16,opt,name=range_end,json=rangeEnd,proto3" json:"range_end,omitempty"`
	SizingMode    SizingMode             `protobuf:"varint,17,opt,name=sizing_mode,json=sizingMode,proto3,enum=xstockstrat.analysis.v1.SizingMode" json:"sizing_mode,omitempty"` // capital-allocation model the run used (feature 150); UNSPECIFIED on pre-150 rows
	FillModel     FillModel              `protobuf:"varint,18,opt,name=fill_model,json=fillModel,proto3,enum=xstockstrat.analysis.v1.FillModel" json:"fill_model,omitempty"`     // fill model the run used (feature 151); UNSPECIFIED on pre-151 rows
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BacktestRunSummary) Reset() {
	*x = BacktestRunSummary{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BacktestRunSummary) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BacktestRunSummary) ProtoMessage() {}

func (x *BacktestRunSummary) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BacktestRunSummary.ProtoReflect.Descriptor instead.
func (*BacktestRunSummary) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{12}
}

func (x *BacktestRunSummary) GetBacktestId() string {
	if x != nil {
		return x.BacktestId
	}
	return ""
}

func (x *BacktestRunSummary) GetStrategyId() string {
	if x != nil {
		return x.StrategyId
	}
	return ""
}

func (x *BacktestRunSummary) GetStatus() BacktestStatus {
	if x != nil {
		return x.Status
	}
	return BacktestStatus_BACKTEST_STATUS_UNSPECIFIED
}

func (x *BacktestRunSummary) GetTotalReturn() float64 {
	if x != nil {
		return x.TotalReturn
	}
	return 0
}

func (x *BacktestRunSummary) GetAnnualizedReturn() float64 {
	if x != nil {
		return x.AnnualizedReturn
	}
	return 0
}

func (x *BacktestRunSummary) GetSharpeRatio() float64 {
	if x != nil {
		return x.SharpeRatio
	}
	return 0
}

func (x *BacktestRunSummary) GetMaxDrawdown() float64 {
	if x != nil {
		return x.MaxDrawdown
	}
	return 0
}

func (x *BacktestRunSummary) GetWinRate() float64 {
	if x != nil {
		return x.WinRate
	}
	return 0
}

func (x *BacktestRunSummary) GetTotalTrades() int32 {
	if x != nil {
		return x.TotalTrades
	}
	return 0
}

func (x *BacktestRunSummary) GetProfitFactor() float64 {
	if x != nil {
		return x.ProfitFactor
	}
	return 0
}

func (x *BacktestRunSummary) GetSymbols() []string {
	if x != nil {
		return x.Symbols
	}
	return nil
}

func (x *BacktestRunSummary) GetOverallScore() float64 {
	if x != nil {
		return x.OverallScore
	}
	return 0
}

func (x *BacktestRunSummary) GetRating() string {
	if x != nil {
		return x.Rating
	}
	return ""
}

func (x *BacktestRunSummary) GetCompletedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.CompletedAt
	}
	return nil
}

func (x *BacktestRunSummary) GetRangeStart() *timestamppb.Timestamp {
	if x != nil {
		return x.RangeStart
	}
	return nil
}

func (x *BacktestRunSummary) GetRangeEnd() *timestamppb.Timestamp {
	if x != nil {
		return x.RangeEnd
	}
	return nil
}

func (x *BacktestRunSummary) GetSizingMode() SizingMode {
	if x != nil {
		return x.SizingMode
	}
	return SizingMode_SIZING_MODE_UNSPECIFIED
}

func (x *BacktestRunSummary) GetFillModel() FillModel {
	if x != nil {
		return x.FillModel
	}
	return FillModel_FILL_MODEL_UNSPECIFIED
}

type ListBacktestsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Runs          []*BacktestRunSummary  `protobuf:"bytes,1,rep,name=runs,proto3" json:"runs,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListBacktestsResponse) Reset() {
	*x = ListBacktestsResponse{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListBacktestsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListBacktestsResponse) ProtoMessage() {}

func (x *ListBacktestsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListBacktestsResponse.ProtoReflect.Descriptor instead.
func (*ListBacktestsResponse) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{13}
}

func (x *ListBacktestsResponse) GetRuns() []*BacktestRunSummary {
	if x != nil {
		return x.Runs
	}
	return nil
}

type GetBacktestRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	BacktestId    string                 `protobuf:"bytes,1,opt,name=backtest_id,json=backtestId,proto3" json:"backtest_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetBacktestRequest) Reset() {
	*x = GetBacktestRequest{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetBacktestRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetBacktestRequest) ProtoMessage() {}

func (x *GetBacktestRequest) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetBacktestRequest.ProtoReflect.Descriptor instead.
func (*GetBacktestRequest) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{14}
}

func (x *GetBacktestRequest) GetBacktestId() string {
	if x != nil {
		return x.BacktestId
	}
	return ""
}

type ListStrategiesRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	Page  *v1.PageRequest        `protobuf:"bytes,1,opt,name=page,proto3" json:"page,omitempty"`
	// Deprecated: Marked as deprecated in analysis/v1/analysis.proto.
	UserId        string `protobuf:"bytes,2,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"` // DEPRECATED: caller identity resolved from the x-user-id header; body value ignored (feature 133).
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListStrategiesRequest) Reset() {
	*x = ListStrategiesRequest{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListStrategiesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListStrategiesRequest) ProtoMessage() {}

func (x *ListStrategiesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListStrategiesRequest.ProtoReflect.Descriptor instead.
func (*ListStrategiesRequest) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{15}
}

func (x *ListStrategiesRequest) GetPage() *v1.PageRequest {
	if x != nil {
		return x.Page
	}
	return nil
}

// Deprecated: Marked as deprecated in analysis/v1/analysis.proto.
func (x *ListStrategiesRequest) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

type ListStrategiesResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Strategies    []*StrategyScore       `protobuf:"bytes,1,rep,name=strategies,proto3" json:"strategies,omitempty"`
	Page          *v1.PageResponse       `protobuf:"bytes,2,opt,name=page,proto3" json:"page,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListStrategiesResponse) Reset() {
	*x = ListStrategiesResponse{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListStrategiesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListStrategiesResponse) ProtoMessage() {}

func (x *ListStrategiesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListStrategiesResponse.ProtoReflect.Descriptor instead.
func (*ListStrategiesResponse) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{16}
}

func (x *ListStrategiesResponse) GetStrategies() []*StrategyScore {
	if x != nil {
		return x.Strategies
	}
	return nil
}

func (x *ListStrategiesResponse) GetPage() *v1.PageResponse {
	if x != nil {
		return x.Page
	}
	return nil
}

type GetStrategyReportRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StrategyId    string                 `protobuf:"bytes,1,opt,name=strategy_id,json=strategyId,proto3" json:"strategy_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetStrategyReportRequest) Reset() {
	*x = GetStrategyReportRequest{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetStrategyReportRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetStrategyReportRequest) ProtoMessage() {}

func (x *GetStrategyReportRequest) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetStrategyReportRequest.ProtoReflect.Descriptor instead.
func (*GetStrategyReportRequest) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{17}
}

func (x *GetStrategyReportRequest) GetStrategyId() string {
	if x != nil {
		return x.StrategyId
	}
	return ""
}

type StrategyComponent struct {
	state     protoimpl.MessageState `protogen:"open.v1"`
	RefName   string                 `protobuf:"bytes,1,opt,name=ref_name,json=refName,proto3" json:"ref_name,omitempty"`
	Kind      ComponentKind          `protobuf:"varint,2,opt,name=kind,proto3,enum=xstockstrat.analysis.v1.ComponentKind" json:"kind,omitempty"`
	Indicator string                 `protobuf:"bytes,3,opt,name=indicator,proto3" json:"indicator,omitempty"`                  // used when kind == COMPONENT_KIND_BUILTIN_INDICATOR
	FormulaId string                 `protobuf:"bytes,4,opt,name=formula_id,json=formulaId,proto3" json:"formula_id,omitempty"` // used when kind == COMPONENT_KIND_CUSTOM_FORMULA
	Params    map[string]float64     `protobuf:"bytes,5,rep,name=params,proto3" json:"params,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"fixed64,2,opt,name=value"`
	// optional benchmark/reference symbol (feature 152): when non-empty the component is
	// computed on this symbol's bars (e.g. "VOO") and its output series is aligned onto the
	// evaluated symbol's bar timeline; empty = computed on the evaluated symbol (unchanged).
	SourceSymbol  string `protobuf:"bytes,6,opt,name=source_symbol,json=sourceSymbol,proto3" json:"source_symbol,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StrategyComponent) Reset() {
	*x = StrategyComponent{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StrategyComponent) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StrategyComponent) ProtoMessage() {}

func (x *StrategyComponent) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StrategyComponent.ProtoReflect.Descriptor instead.
func (*StrategyComponent) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{18}
}

func (x *StrategyComponent) GetRefName() string {
	if x != nil {
		return x.RefName
	}
	return ""
}

func (x *StrategyComponent) GetKind() ComponentKind {
	if x != nil {
		return x.Kind
	}
	return ComponentKind_COMPONENT_KIND_UNSPECIFIED
}

func (x *StrategyComponent) GetIndicator() string {
	if x != nil {
		return x.Indicator
	}
	return ""
}

func (x *StrategyComponent) GetFormulaId() string {
	if x != nil {
		return x.FormulaId
	}
	return ""
}

func (x *StrategyComponent) GetParams() map[string]float64 {
	if x != nil {
		return x.Params
	}
	return nil
}

func (x *StrategyComponent) GetSourceSymbol() string {
	if x != nil {
		return x.SourceSymbol
	}
	return ""
}

type StrategyDefinition struct {
	state        protoimpl.MessageState `protogen:"open.v1"`
	StrategyId   string                 `protobuf:"bytes,1,opt,name=strategy_id,json=strategyId,proto3" json:"strategy_id,omitempty"`
	DisplayName  string                 `protobuf:"bytes,2,opt,name=display_name,json=displayName,proto3" json:"display_name,omitempty"`
	Components   []*StrategyComponent   `protobuf:"bytes,3,rep,name=components,proto3" json:"components,omitempty"`
	EntryRule    string                 `protobuf:"bytes,4,opt,name=entry_rule,json=entryRule,proto3" json:"entry_rule,omitempty"` // JSON-encoded condition tree
	ExitRule     string                 `protobuf:"bytes,5,opt,name=exit_rule,json=exitRule,proto3" json:"exit_rule,omitempty"`    // JSON-encoded condition tree
	SignalParams *structpb.Struct       `protobuf:"bytes,6,opt,name=signal_params,json=signalParams,proto3" json:"signal_params,omitempty"`
	Active       bool                   `protobuf:"varint,7,opt,name=active,proto3" json:"active,omitempty"`
	LiveEnabled  bool                   `protobuf:"varint,8,opt,name=live_enabled,json=liveEnabled,proto3" json:"live_enabled,omitempty"`
	// Per-symbol re-entry cooldown in calendar days (feature 069). optional = explicit presence:
	// unset → platform default (analysis.strategy.default_cooldown_days); explicit 0 → no cooldown
	// (immediate re-entry allowed); negative → rejected at write time (INVALID_ARGUMENT).
	CooldownDays *int32 `protobuf:"varint,9,opt,name=cooldown_days,json=cooldownDays,proto3,oneof" json:"cooldown_days,omitempty"`
	// Human-readable status warnings surfaced to the user on read (feature 086), e.g. a component
	// references a formula that has been soft-deleted — the strategy still evaluates (live and in
	// backtests) using the formula's last-saved definition, but the deletion is flagged. Populated
	// by GetStrategy; empty elsewhere.
	Warnings []string `protobuf:"bytes,10,rep,name=warnings,proto3" json:"warnings,omitempty"`
	// Per-strategy minimum holding period in calendar days before exit_rule may fire a sell
	// (feature 116 — exit cooldown; mirrors cooldown_days but gates the exit transition).
	// optional = explicit presence: unset → platform default
	// (analysis.strategy.default_exit_cooldown_days); explicit 0 → no minimum hold (exit
	// permitted immediately, current behavior); negative → rejected at write time
	// (INVALID_ARGUMENT).
	ExitCooldownDays *int32 `protobuf:"varint,11,opt,name=exit_cooldown_days,json=exitCooldownDays,proto3,oneof" json:"exit_cooldown_days,omitempty"`
	// Normalized-uppercase symbols this strategy must never evaluate FOR ENTRY (feature 132 —
	// entry-only deny). A held position on a denied symbol keeps exit tracing (the deny suppresses
	// only the entry edge, so an operator can always exit a position they already hold). Rides
	// definition_json (no column); maskable via ManageStrategyRequest.update_mask.
	DeniedSymbols []string `protobuf:"bytes,12,rep,name=denied_symbols,json=deniedSymbols,proto3" json:"denied_symbols,omitempty"`
	// Owning user (feature 133). Server-authoritative: populated from the propagated
	// x-user-id header on ManageStrategy REGISTER, never accepted from the request body
	// (mirrors ListOpportunitiesRequest / portfolio ownership convention).
	UserId string `protobuf:"bytes,13,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	// Gates whether the platform-wide active-signal term joins this strategy's evaluation universe
	// (feature 132). Plain bool (no optional) is intentional: absent ≡ false ≡ explicit-false resolve
	// identically. A strategy that sets BOTH a non-empty signal_params.symbols allowlist AND
	// signal_eligible=true is rejected INVALID_ARGUMENT at write time (the allowlist is already an
	// explicit universe override; signals would be redundant/contradictory). Rides definition_json;
	// maskable.
	SignalEligible bool `protobuf:"varint,14,opt,name=signal_eligible,json=signalEligible,proto3" json:"signal_eligible,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *StrategyDefinition) Reset() {
	*x = StrategyDefinition{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[19]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StrategyDefinition) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StrategyDefinition) ProtoMessage() {}

func (x *StrategyDefinition) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[19]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StrategyDefinition.ProtoReflect.Descriptor instead.
func (*StrategyDefinition) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{19}
}

func (x *StrategyDefinition) GetStrategyId() string {
	if x != nil {
		return x.StrategyId
	}
	return ""
}

func (x *StrategyDefinition) GetDisplayName() string {
	if x != nil {
		return x.DisplayName
	}
	return ""
}

func (x *StrategyDefinition) GetComponents() []*StrategyComponent {
	if x != nil {
		return x.Components
	}
	return nil
}

func (x *StrategyDefinition) GetEntryRule() string {
	if x != nil {
		return x.EntryRule
	}
	return ""
}

func (x *StrategyDefinition) GetExitRule() string {
	if x != nil {
		return x.ExitRule
	}
	return ""
}

func (x *StrategyDefinition) GetSignalParams() *structpb.Struct {
	if x != nil {
		return x.SignalParams
	}
	return nil
}

func (x *StrategyDefinition) GetActive() bool {
	if x != nil {
		return x.Active
	}
	return false
}

func (x *StrategyDefinition) GetLiveEnabled() bool {
	if x != nil {
		return x.LiveEnabled
	}
	return false
}

func (x *StrategyDefinition) GetCooldownDays() int32 {
	if x != nil && x.CooldownDays != nil {
		return *x.CooldownDays
	}
	return 0
}

func (x *StrategyDefinition) GetWarnings() []string {
	if x != nil {
		return x.Warnings
	}
	return nil
}

func (x *StrategyDefinition) GetExitCooldownDays() int32 {
	if x != nil && x.ExitCooldownDays != nil {
		return *x.ExitCooldownDays
	}
	return 0
}

func (x *StrategyDefinition) GetDeniedSymbols() []string {
	if x != nil {
		return x.DeniedSymbols
	}
	return nil
}

func (x *StrategyDefinition) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *StrategyDefinition) GetSignalEligible() bool {
	if x != nil {
		return x.SignalEligible
	}
	return false
}

type ManageStrategyRequest struct {
	state      protoimpl.MessageState `protogen:"open.v1"`
	Operation  StrategyOperation      `protobuf:"varint,1,opt,name=operation,proto3,enum=xstockstrat.analysis.v1.StrategyOperation" json:"operation,omitempty"`
	Definition *StrategyDefinition    `protobuf:"bytes,2,opt,name=definition,proto3" json:"definition,omitempty"`
	// Feature 070 — partial update. Applies to STRATEGY_OPERATION_UPDATE only; ignored for
	// REGISTER/DEACTIVATE.
	//
	//   present  → MERGE: only the listed top-level StrategyDefinition paths are taken from
	//              `definition`; every other stored field is preserved. A masked path whose value
	//              is absent from the request CLEARS that field (AIP-161 semantics) — that is the
	//              only way to express "erase this", since proto3 gives `components`/`entry_rule`/
	//              `exit_rule` no field presence.
	//   absent   → FULL REPLACE: byte-for-byte the pre-070 behavior, so existing clients (the
	//              StrategyWizard, which always sends a complete definition) are unaffected.
	//
	// Allowed paths: display_name, components, entry_rule, exit_rule, signal_params, cooldown_days,
	// exit_cooldown_days, denied_symbols, signal_eligible.
	// strategy_id/active/live_enabled are column-authoritative and rejected with INVALID_ARGUMENT.
	UpdateMask    *fieldmaskpb.FieldMask `protobuf:"bytes,3,opt,name=update_mask,json=updateMask,proto3" json:"update_mask,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ManageStrategyRequest) Reset() {
	*x = ManageStrategyRequest{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[20]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ManageStrategyRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ManageStrategyRequest) ProtoMessage() {}

func (x *ManageStrategyRequest) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[20]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ManageStrategyRequest.ProtoReflect.Descriptor instead.
func (*ManageStrategyRequest) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{20}
}

func (x *ManageStrategyRequest) GetOperation() StrategyOperation {
	if x != nil {
		return x.Operation
	}
	return StrategyOperation_STRATEGY_OPERATION_UNSPECIFIED
}

func (x *ManageStrategyRequest) GetDefinition() *StrategyDefinition {
	if x != nil {
		return x.Definition
	}
	return nil
}

func (x *ManageStrategyRequest) GetUpdateMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.UpdateMask
	}
	return nil
}

type GetStrategyRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StrategyId    string                 `protobuf:"bytes,1,opt,name=strategy_id,json=strategyId,proto3" json:"strategy_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetStrategyRequest) Reset() {
	*x = GetStrategyRequest{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[21]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetStrategyRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetStrategyRequest) ProtoMessage() {}

func (x *GetStrategyRequest) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[21]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetStrategyRequest.ProtoReflect.Descriptor instead.
func (*GetStrategyRequest) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{21}
}

func (x *GetStrategyRequest) GetStrategyId() string {
	if x != nil {
		return x.StrategyId
	}
	return ""
}

type ListStrategyDefinitionsRequest struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	IncludeInactive bool                   `protobuf:"varint,1,opt,name=include_inactive,json=includeInactive,proto3" json:"include_inactive,omitempty"`
	PageSize        int32                  `protobuf:"varint,2,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	PageOffset      int32                  `protobuf:"varint,3,opt,name=page_offset,json=pageOffset,proto3" json:"page_offset,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *ListStrategyDefinitionsRequest) Reset() {
	*x = ListStrategyDefinitionsRequest{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[22]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListStrategyDefinitionsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListStrategyDefinitionsRequest) ProtoMessage() {}

func (x *ListStrategyDefinitionsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[22]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListStrategyDefinitionsRequest.ProtoReflect.Descriptor instead.
func (*ListStrategyDefinitionsRequest) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{22}
}

func (x *ListStrategyDefinitionsRequest) GetIncludeInactive() bool {
	if x != nil {
		return x.IncludeInactive
	}
	return false
}

func (x *ListStrategyDefinitionsRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListStrategyDefinitionsRequest) GetPageOffset() int32 {
	if x != nil {
		return x.PageOffset
	}
	return 0
}

type ListStrategyDefinitionsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Definitions   []*StrategyDefinition  `protobuf:"bytes,1,rep,name=definitions,proto3" json:"definitions,omitempty"`
	TotalCount    int32                  `protobuf:"varint,2,opt,name=total_count,json=totalCount,proto3" json:"total_count,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListStrategyDefinitionsResponse) Reset() {
	*x = ListStrategyDefinitionsResponse{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[23]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListStrategyDefinitionsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListStrategyDefinitionsResponse) ProtoMessage() {}

func (x *ListStrategyDefinitionsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[23]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListStrategyDefinitionsResponse.ProtoReflect.Descriptor instead.
func (*ListStrategyDefinitionsResponse) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{23}
}

func (x *ListStrategyDefinitionsResponse) GetDefinitions() []*StrategyDefinition {
	if x != nil {
		return x.Definitions
	}
	return nil
}

func (x *ListStrategyDefinitionsResponse) GetTotalCount() int32 {
	if x != nil {
		return x.TotalCount
	}
	return 0
}

type SetStrategyLiveRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StrategyId    string                 `protobuf:"bytes,1,opt,name=strategy_id,json=strategyId,proto3" json:"strategy_id,omitempty"`
	LiveEnabled   bool                   `protobuf:"varint,2,opt,name=live_enabled,json=liveEnabled,proto3" json:"live_enabled,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SetStrategyLiveRequest) Reset() {
	*x = SetStrategyLiveRequest{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[24]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SetStrategyLiveRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SetStrategyLiveRequest) ProtoMessage() {}

func (x *SetStrategyLiveRequest) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[24]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SetStrategyLiveRequest.ProtoReflect.Descriptor instead.
func (*SetStrategyLiveRequest) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{24}
}

func (x *SetStrategyLiveRequest) GetStrategyId() string {
	if x != nil {
		return x.StrategyId
	}
	return ""
}

func (x *SetStrategyLiveRequest) GetLiveEnabled() bool {
	if x != nil {
		return x.LiveEnabled
	}
	return false
}

type SetStrategyLiveResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Definition    *StrategyDefinition    `protobuf:"bytes,1,opt,name=definition,proto3" json:"definition,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SetStrategyLiveResponse) Reset() {
	*x = SetStrategyLiveResponse{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[25]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SetStrategyLiveResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SetStrategyLiveResponse) ProtoMessage() {}

func (x *SetStrategyLiveResponse) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[25]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SetStrategyLiveResponse.ProtoReflect.Descriptor instead.
func (*SetStrategyLiveResponse) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{25}
}

func (x *SetStrategyLiveResponse) GetDefinition() *StrategyDefinition {
	if x != nil {
		return x.Definition
	}
	return nil
}

type ScreenCriterion struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RefName       string                 `protobuf:"bytes,1,opt,name=ref_name,json=refName,proto3" json:"ref_name,omitempty"`
	Kind          ScreenKind             `protobuf:"varint,2,opt,name=kind,proto3,enum=xstockstrat.analysis.v1.ScreenKind" json:"kind,omitempty"`
	MetricName    string                 `protobuf:"bytes,3,opt,name=metric_name,json=metricName,proto3" json:"metric_name,omitempty"` // FUNDAMENTAL only (e.g. "pe_ratio")
	Component     *StrategyComponent     `protobuf:"bytes,4,opt,name=component,proto3" json:"component,omitempty"`                     // reused, for TECHNICAL_* kinds
	Op            Comparator             `protobuf:"varint,5,opt,name=op,proto3,enum=xstockstrat.analysis.v1.Comparator" json:"op,omitempty"`
	Threshold     float64                `protobuf:"fixed64,6,opt,name=threshold,proto3" json:"threshold,omitempty"`
	ThresholdHigh float64                `protobuf:"fixed64,7,opt,name=threshold_high,json=thresholdHigh,proto3" json:"threshold_high,omitempty"` // for COMPARATOR_BETWEEN
	Weight        float64                `protobuf:"fixed64,8,opt,name=weight,proto3" json:"weight,omitempty"`                                    // contribution to the blended score
	HardFilter    bool                   `protobuf:"varint,9,opt,name=hard_filter,json=hardFilter,proto3" json:"hard_filter,omitempty"`           // true → failing this excludes the symbol
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ScreenCriterion) Reset() {
	*x = ScreenCriterion{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[26]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ScreenCriterion) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ScreenCriterion) ProtoMessage() {}

func (x *ScreenCriterion) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[26]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ScreenCriterion.ProtoReflect.Descriptor instead.
func (*ScreenCriterion) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{26}
}

func (x *ScreenCriterion) GetRefName() string {
	if x != nil {
		return x.RefName
	}
	return ""
}

func (x *ScreenCriterion) GetKind() ScreenKind {
	if x != nil {
		return x.Kind
	}
	return ScreenKind_SCREEN_KIND_UNSPECIFIED
}

func (x *ScreenCriterion) GetMetricName() string {
	if x != nil {
		return x.MetricName
	}
	return ""
}

func (x *ScreenCriterion) GetComponent() *StrategyComponent {
	if x != nil {
		return x.Component
	}
	return nil
}

func (x *ScreenCriterion) GetOp() Comparator {
	if x != nil {
		return x.Op
	}
	return Comparator_COMPARATOR_UNSPECIFIED
}

func (x *ScreenCriterion) GetThreshold() float64 {
	if x != nil {
		return x.Threshold
	}
	return 0
}

func (x *ScreenCriterion) GetThresholdHigh() float64 {
	if x != nil {
		return x.ThresholdHigh
	}
	return 0
}

func (x *ScreenCriterion) GetWeight() float64 {
	if x != nil {
		return x.Weight
	}
	return 0
}

func (x *ScreenCriterion) GetHardFilter() bool {
	if x != nil {
		return x.HardFilter
	}
	return false
}

type ScreenResult struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	Symbol          string                 `protobuf:"bytes,1,opt,name=symbol,proto3" json:"symbol,omitempty"`
	Score           float64                `protobuf:"fixed64,2,opt,name=score,proto3" json:"score,omitempty"`
	CriterionScores map[string]float64     `protobuf:"bytes,3,rep,name=criterion_scores,json=criterionScores,proto3" json:"criterion_scores,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"fixed64,2,opt,name=value"` // per ref_name; skipped criteria are absent
	Passed          bool                   `protobuf:"varint,4,opt,name=passed,proto3" json:"passed,omitempty"`
	Status          ScreenResultStatus     `protobuf:"varint,5,opt,name=status,proto3,enum=xstockstrat.analysis.v1.ScreenResultStatus" json:"status,omitempty"`
	Gap             *CoverageGap           `protobuf:"bytes,6,opt,name=gap,proto3" json:"gap,omitempty"` // populated when status == INSUFFICIENT_DATA
	// Raw column values surfaced on the screener results table (feature 083, FR-8).
	// rsi/atr come from the analysis→indicators edge; ATR is a close-only approximation
	// (indicators_engine.py) — surfaced as a known accuracy caveat, not exact.
	// rev_growth is best-effort from Fundamentals.extra_metrics (0 when FMP omits it).
	Pe        float64 `protobuf:"fixed64,7,opt,name=pe,proto3" json:"pe,omitempty"`
	Rsi       float64 `protobuf:"fixed64,8,opt,name=rsi,proto3" json:"rsi,omitempty"`
	Atr       float64 `protobuf:"fixed64,9,opt,name=atr,proto3" json:"atr,omitempty"`
	RevGrowth float64 `protobuf:"fixed64,10,opt,name=rev_growth,json=revGrowth,proto3" json:"rev_growth,omitempty"`
	Held      bool    `protobuf:"varint,11,opt,name=held,proto3" json:"held,omitempty"`
	// Per-criterion raw readings + pass/fail, for single-symbol screening where the universe-relative
	// `score`/`criterion_scores` collapse to a content-free 0.5 (feature 125, FR-8). Populated from the
	// same engine-internal values `criterion_scores` already draws from, exposed directly instead of
	// normalized.
	CriterionRawValues map[string]float64 `protobuf:"bytes,12,rep,name=criterion_raw_values,json=criterionRawValues,proto3" json:"criterion_raw_values,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"fixed64,2,opt,name=value"`
	CriterionPassed    map[string]bool    `protobuf:"bytes,13,rep,name=criterion_passed,json=criterionPassed,proto3" json:"criterion_passed,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"varint,2,opt,name=value"`
	// True when every criterion configured for this scan was skipped for this candidate (no
	// usable data for any of them, e.g. an ETF with no P/E ratio scanned against a `pe_ratio`
	// criterion) — `score`/`criterion_scores` still carry the same neutral-abstention values the
	// engine already used to keep the signal blend well-defined (`status` stays OK; this is the
	// soft-criterion sibling of the hard-filter null-as-zero fix, feature 144), but they are not a
	// real computed result and must not be treated as one (e.g. ranked/sorted as if genuine).
	ScoreUnavailable bool `protobuf:"varint,14,opt,name=score_unavailable,json=scoreUnavailable,proto3" json:"score_unavailable,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *ScreenResult) Reset() {
	*x = ScreenResult{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[27]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ScreenResult) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ScreenResult) ProtoMessage() {}

func (x *ScreenResult) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[27]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ScreenResult.ProtoReflect.Descriptor instead.
func (*ScreenResult) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{27}
}

func (x *ScreenResult) GetSymbol() string {
	if x != nil {
		return x.Symbol
	}
	return ""
}

func (x *ScreenResult) GetScore() float64 {
	if x != nil {
		return x.Score
	}
	return 0
}

func (x *ScreenResult) GetCriterionScores() map[string]float64 {
	if x != nil {
		return x.CriterionScores
	}
	return nil
}

func (x *ScreenResult) GetPassed() bool {
	if x != nil {
		return x.Passed
	}
	return false
}

func (x *ScreenResult) GetStatus() ScreenResultStatus {
	if x != nil {
		return x.Status
	}
	return ScreenResultStatus_SCREEN_RESULT_STATUS_UNSPECIFIED
}

func (x *ScreenResult) GetGap() *CoverageGap {
	if x != nil {
		return x.Gap
	}
	return nil
}

func (x *ScreenResult) GetPe() float64 {
	if x != nil {
		return x.Pe
	}
	return 0
}

func (x *ScreenResult) GetRsi() float64 {
	if x != nil {
		return x.Rsi
	}
	return 0
}

func (x *ScreenResult) GetAtr() float64 {
	if x != nil {
		return x.Atr
	}
	return 0
}

func (x *ScreenResult) GetRevGrowth() float64 {
	if x != nil {
		return x.RevGrowth
	}
	return 0
}

func (x *ScreenResult) GetHeld() bool {
	if x != nil {
		return x.Held
	}
	return false
}

func (x *ScreenResult) GetCriterionRawValues() map[string]float64 {
	if x != nil {
		return x.CriterionRawValues
	}
	return nil
}

func (x *ScreenResult) GetCriterionPassed() map[string]bool {
	if x != nil {
		return x.CriterionPassed
	}
	return nil
}

func (x *ScreenResult) GetScoreUnavailable() bool {
	if x != nil {
		return x.ScoreUnavailable
	}
	return false
}

type ScreenSymbolsRequest struct {
	state    protoimpl.MessageState `protogen:"open.v1"`
	Symbols  []string               `protobuf:"bytes,1,rep,name=symbols,proto3" json:"symbols,omitempty"`
	Criteria []*ScreenCriterion     `protobuf:"bytes,2,rep,name=criteria,proto3" json:"criteria,omitempty"`
	// Blend params — same names the extracted scoring module reads (kept consistent with backtest).
	SignalSources   []string `protobuf:"bytes,3,rep,name=signal_sources,json=signalSources,proto3" json:"signal_sources,omitempty"`
	SignalWeight    float64  `protobuf:"fixed64,4,opt,name=signal_weight,json=signalWeight,proto3" json:"signal_weight,omitempty"`
	TechnicalWeight float64  `protobuf:"fixed64,5,opt,name=technical_weight,json=technicalWeight,proto3" json:"technical_weight,omitempty"`
	MinConviction   float64  `protobuf:"fixed64,6,opt,name=min_conviction,json=minConviction,proto3" json:"min_conviction,omitempty"`
	RankLimit       int32    `protobuf:"varint,7,opt,name=rank_limit,json=rankLimit,proto3" json:"rank_limit,omitempty"`
	// Reserved/optional — historical as-of is deferred (OQ-060-e); latest bar is the default.
	EvaluationWindow *v1.TimeRange `protobuf:"bytes,8,opt,name=evaluation_window,json=evaluationWindow,proto3" json:"evaluation_window,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *ScreenSymbolsRequest) Reset() {
	*x = ScreenSymbolsRequest{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[28]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ScreenSymbolsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ScreenSymbolsRequest) ProtoMessage() {}

func (x *ScreenSymbolsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[28]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ScreenSymbolsRequest.ProtoReflect.Descriptor instead.
func (*ScreenSymbolsRequest) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{28}
}

func (x *ScreenSymbolsRequest) GetSymbols() []string {
	if x != nil {
		return x.Symbols
	}
	return nil
}

func (x *ScreenSymbolsRequest) GetCriteria() []*ScreenCriterion {
	if x != nil {
		return x.Criteria
	}
	return nil
}

func (x *ScreenSymbolsRequest) GetSignalSources() []string {
	if x != nil {
		return x.SignalSources
	}
	return nil
}

func (x *ScreenSymbolsRequest) GetSignalWeight() float64 {
	if x != nil {
		return x.SignalWeight
	}
	return 0
}

func (x *ScreenSymbolsRequest) GetTechnicalWeight() float64 {
	if x != nil {
		return x.TechnicalWeight
	}
	return 0
}

func (x *ScreenSymbolsRequest) GetMinConviction() float64 {
	if x != nil {
		return x.MinConviction
	}
	return 0
}

func (x *ScreenSymbolsRequest) GetRankLimit() int32 {
	if x != nil {
		return x.RankLimit
	}
	return 0
}

func (x *ScreenSymbolsRequest) GetEvaluationWindow() *v1.TimeRange {
	if x != nil {
		return x.EvaluationWindow
	}
	return nil
}

type ScreenSymbolsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Results       []*ScreenResult        `protobuf:"bytes,1,rep,name=results,proto3" json:"results,omitempty"`
	CoverageGaps  []*CoverageGap         `protobuf:"bytes,2,rep,name=coverage_gaps,json=coverageGaps,proto3" json:"coverage_gaps,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ScreenSymbolsResponse) Reset() {
	*x = ScreenSymbolsResponse{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[29]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ScreenSymbolsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ScreenSymbolsResponse) ProtoMessage() {}

func (x *ScreenSymbolsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[29]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ScreenSymbolsResponse.ProtoReflect.Descriptor instead.
func (*ScreenSymbolsResponse) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{29}
}

func (x *ScreenSymbolsResponse) GetResults() []*ScreenResult {
	if x != nil {
		return x.Results
	}
	return nil
}

func (x *ScreenSymbolsResponse) GetCoverageGaps() []*CoverageGap {
	if x != nil {
		return x.CoverageGaps
	}
	return nil
}

type RunFundamentalsScanRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Force         bool                   `protobuf:"varint,1,opt,name=force,proto3" json:"force,omitempty"`                 // ignore the day's idempotency guard / re-emit
	DryRun        bool                   `protobuf:"varint,2,opt,name=dry_run,json=dryRun,proto3" json:"dry_run,omitempty"` // score + report but do not emit or spend cache calls
	Symbols       []string               `protobuf:"bytes,3,rep,name=symbols,proto3" json:"symbols,omitempty"`              // optional explicit override of the computed universe
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RunFundamentalsScanRequest) Reset() {
	*x = RunFundamentalsScanRequest{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[30]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RunFundamentalsScanRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RunFundamentalsScanRequest) ProtoMessage() {}

func (x *RunFundamentalsScanRequest) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[30]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RunFundamentalsScanRequest.ProtoReflect.Descriptor instead.
func (*RunFundamentalsScanRequest) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{30}
}

func (x *RunFundamentalsScanRequest) GetForce() bool {
	if x != nil {
		return x.Force
	}
	return false
}

func (x *RunFundamentalsScanRequest) GetDryRun() bool {
	if x != nil {
		return x.DryRun
	}
	return false
}

func (x *RunFundamentalsScanRequest) GetSymbols() []string {
	if x != nil {
		return x.Symbols
	}
	return nil
}

type FundamentalsScanSummary struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	RunId            string                 `protobuf:"bytes,1,opt,name=run_id,json=runId,proto3" json:"run_id,omitempty"`
	SymbolsProcessed int32                  `protobuf:"varint,2,opt,name=symbols_processed,json=symbolsProcessed,proto3" json:"symbols_processed,omitempty"`
	SignalsEmitted   int32                  `protobuf:"varint,3,opt,name=signals_emitted,json=signalsEmitted,proto3" json:"signals_emitted,omitempty"`
	CallsSpent       int32                  `protobuf:"varint,4,opt,name=calls_spent,json=callsSpent,proto3" json:"calls_spent,omitempty"`
	DeferredCount    int32                  `protobuf:"varint,5,opt,name=deferred_count,json=deferredCount,proto3" json:"deferred_count,omitempty"`
	Status           string                 `protobuf:"bytes,6,opt,name=status,proto3" json:"status,omitempty"` // "completed" | "budget_deferred" | "failed"
	FinishedAt       *timestamppb.Timestamp `protobuf:"bytes,7,opt,name=finished_at,json=finishedAt,proto3" json:"finished_at,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *FundamentalsScanSummary) Reset() {
	*x = FundamentalsScanSummary{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[31]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FundamentalsScanSummary) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FundamentalsScanSummary) ProtoMessage() {}

func (x *FundamentalsScanSummary) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[31]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FundamentalsScanSummary.ProtoReflect.Descriptor instead.
func (*FundamentalsScanSummary) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{31}
}

func (x *FundamentalsScanSummary) GetRunId() string {
	if x != nil {
		return x.RunId
	}
	return ""
}

func (x *FundamentalsScanSummary) GetSymbolsProcessed() int32 {
	if x != nil {
		return x.SymbolsProcessed
	}
	return 0
}

func (x *FundamentalsScanSummary) GetSignalsEmitted() int32 {
	if x != nil {
		return x.SignalsEmitted
	}
	return 0
}

func (x *FundamentalsScanSummary) GetCallsSpent() int32 {
	if x != nil {
		return x.CallsSpent
	}
	return 0
}

func (x *FundamentalsScanSummary) GetDeferredCount() int32 {
	if x != nil {
		return x.DeferredCount
	}
	return 0
}

func (x *FundamentalsScanSummary) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

func (x *FundamentalsScanSummary) GetFinishedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.FinishedAt
	}
	return nil
}

// One ranked opportunity on the Decide queue. conviction is a deterministic
// ordinal (passing/total leaves + normalized worst-distance-to-threshold), NOT a
// probability — the UI renders "N/M conditions" + strength bars, never a fake %.
type Opportunity struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	Symbol            string                 `protobuf:"bytes,1,opt,name=symbol,proto3" json:"symbol,omitempty"`
	Action            OpportunityActionTag   `protobuf:"varint,2,opt,name=action,proto3,enum=xstockstrat.analysis.v1.OpportunityActionTag" json:"action,omitempty"`
	Conviction        float64                `protobuf:"fixed64,3,opt,name=conviction,proto3" json:"conviction,omitempty"`
	PassingConditions int32                  `protobuf:"varint,4,opt,name=passing_conditions,json=passingConditions,proto3" json:"passing_conditions,omitempty"`
	TotalConditions   int32                  `protobuf:"varint,5,opt,name=total_conditions,json=totalConditions,proto3" json:"total_conditions,omitempty"`
	Thesis            string                 `protobuf:"bytes,6,opt,name=thesis,proto3" json:"thesis,omitempty"`
	StrategyId        string                 `protobuf:"bytes,7,opt,name=strategy_id,json=strategyId,proto3" json:"strategy_id,omitempty"`
	Source            string                 `protobuf:"bytes,8,opt,name=source,proto3" json:"source,omitempty"`
	ValidUntil        *timestamppb.Timestamp `protobuf:"bytes,9,opt,name=valid_until,json=validUntil,proto3" json:"valid_until,omitempty"`
	OpportunityKey    string                 `protobuf:"bytes,10,opt,name=opportunity_key,json=opportunityKey,proto3" json:"opportunity_key,omitempty"` // server-authoritative opaque key = user|symbol_norm|strategy_id (feature 097). Client echoes it verbatim to SetOpportunityAction, never derives it.
	Provenance        []string               `protobuf:"bytes,11,rep,name=provenance,proto3" json:"provenance,omitempty"`                               // contributing origins for a de-duplicated row (signal source(s) / "position" / "watchlist")
	Muted             bool                   `protobuf:"varint,12,opt,name=muted,proto3" json:"muted,omitempty"`                                        // feature 132 — the (symbol, strategy) pair is on the strategy's deny list; surfaced as an explicit muted row (never conviction=0)
	// Live-market enrichment (feature 095). 13/14/17 are READ-TIME live-market fields (set in
	// ListOpportunities after ranking — never in the ranking hot path, FR-8/AC-14); 15/16/18 are
	// COMPUTE-TIME strategy-derived fields (persisted in the row JSONB, carried by _row_to_opportunity).
	// 15/16 stay unset until the named `strategy-target-stop-authoring` follow-up populates
	// StrategyDefinition.signal_params.{target,stop}. All explicit-presence — an unset optional models
	// "unavailable", never a fabricated 0 (P-03, AC-8/AC-11).
	LivePrice   *float64          `protobuf:"fixed64,13,opt,name=live_price,json=livePrice,proto3,oneof" json:"live_price,omitempty"`
	ChangePct   *float64          `protobuf:"fixed64,14,opt,name=change_pct,json=changePct,proto3,oneof" json:"change_pct,omitempty"`
	TargetPrice *float64          `protobuf:"fixed64,15,opt,name=target_price,json=targetPrice,proto3,oneof" json:"target_price,omitempty"`
	StopPrice   *float64          `protobuf:"fixed64,16,opt,name=stop_price,json=stopPrice,proto3,oneof" json:"stop_price,omitempty"`
	Sparkline   []*SparklinePoint `protobuf:"bytes,17,rep,name=sparkline,proto3" json:"sparkline,omitempty"`
	Conditions  []*ConditionEval  `protobuf:"bytes,18,rep,name=conditions,proto3" json:"conditions,omitempty"`
	// feature 110 — the raw max per-signal ExternalSignal.conviction (0.0–1.0) among the symbol's
	// active signals; the real probability that feeds trading PlaceOrder's confidence sizing.
	// Explicit-presence optional: UNSET means "no active signal for this symbol" (never a fabricated
	// 0.0). Deliberately NAMED signal_confidence and kept distinct from the ordinal `conviction = 3`
	// (NOT a probability) and the decayed/weighted signal_axis. Next free after 095's 13-18 block.
	SignalConfidence *float64 `protobuf:"fixed64,19,opt,name=signal_confidence,json=signalConfidence,proto3,oneof" json:"signal_confidence,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *Opportunity) Reset() {
	*x = Opportunity{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[32]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Opportunity) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Opportunity) ProtoMessage() {}

func (x *Opportunity) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[32]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Opportunity.ProtoReflect.Descriptor instead.
func (*Opportunity) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{32}
}

func (x *Opportunity) GetSymbol() string {
	if x != nil {
		return x.Symbol
	}
	return ""
}

func (x *Opportunity) GetAction() OpportunityActionTag {
	if x != nil {
		return x.Action
	}
	return OpportunityActionTag_OPPORTUNITY_ACTION_TAG_UNSPECIFIED
}

func (x *Opportunity) GetConviction() float64 {
	if x != nil {
		return x.Conviction
	}
	return 0
}

func (x *Opportunity) GetPassingConditions() int32 {
	if x != nil {
		return x.PassingConditions
	}
	return 0
}

func (x *Opportunity) GetTotalConditions() int32 {
	if x != nil {
		return x.TotalConditions
	}
	return 0
}

func (x *Opportunity) GetThesis() string {
	if x != nil {
		return x.Thesis
	}
	return ""
}

func (x *Opportunity) GetStrategyId() string {
	if x != nil {
		return x.StrategyId
	}
	return ""
}

func (x *Opportunity) GetSource() string {
	if x != nil {
		return x.Source
	}
	return ""
}

func (x *Opportunity) GetValidUntil() *timestamppb.Timestamp {
	if x != nil {
		return x.ValidUntil
	}
	return nil
}

func (x *Opportunity) GetOpportunityKey() string {
	if x != nil {
		return x.OpportunityKey
	}
	return ""
}

func (x *Opportunity) GetProvenance() []string {
	if x != nil {
		return x.Provenance
	}
	return nil
}

func (x *Opportunity) GetMuted() bool {
	if x != nil {
		return x.Muted
	}
	return false
}

func (x *Opportunity) GetLivePrice() float64 {
	if x != nil && x.LivePrice != nil {
		return *x.LivePrice
	}
	return 0
}

func (x *Opportunity) GetChangePct() float64 {
	if x != nil && x.ChangePct != nil {
		return *x.ChangePct
	}
	return 0
}

func (x *Opportunity) GetTargetPrice() float64 {
	if x != nil && x.TargetPrice != nil {
		return *x.TargetPrice
	}
	return 0
}

func (x *Opportunity) GetStopPrice() float64 {
	if x != nil && x.StopPrice != nil {
		return *x.StopPrice
	}
	return 0
}

func (x *Opportunity) GetSparkline() []*SparklinePoint {
	if x != nil {
		return x.Sparkline
	}
	return nil
}

func (x *Opportunity) GetConditions() []*ConditionEval {
	if x != nil {
		return x.Conditions
	}
	return nil
}

func (x *Opportunity) GetSignalConfidence() float64 {
	if x != nil && x.SignalConfidence != nil {
		return *x.SignalConfidence
	}
	return 0
}

// One recent daily-bar close for the Decide-surface sparkline (feature 095). Explicit presence — an
// unset `close` models a warm-up/absent bar, never NaN/0 (mirrors IndicatorValue; P-03).
type SparklinePoint struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Close         *float64               `protobuf:"fixed64,1,opt,name=close,proto3,oneof" json:"close,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SparklinePoint) Reset() {
	*x = SparklinePoint{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[33]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SparklinePoint) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SparklinePoint) ProtoMessage() {}

func (x *SparklinePoint) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[33]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SparklinePoint.ProtoReflect.Descriptor instead.
func (*SparklinePoint) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{33}
}

func (x *SparklinePoint) GetClose() float64 {
	if x != nil && x.Close != nil {
		return *x.Close
	}
	return 0
}

// One evaluated condition leaf from the traced evaluator (feature 083).
type ConditionEval struct {
	state               protoimpl.MessageState `protogen:"open.v1"`
	RefName             string                 `protobuf:"bytes,1,opt,name=ref_name,json=refName,proto3" json:"ref_name,omitempty"`
	LhsValue            float64                `protobuf:"fixed64,2,opt,name=lhs_value,json=lhsValue,proto3" json:"lhs_value,omitempty"`
	Threshold           float64                `protobuf:"fixed64,3,opt,name=threshold,proto3" json:"threshold,omitempty"`
	Fn                  string                 `protobuf:"bytes,4,opt,name=fn,proto3" json:"fn,omitempty"` // >, <, >=, <=, crosses_above, crosses_below
	State               ConditionState         `protobuf:"varint,5,opt,name=state,proto3,enum=xstockstrat.analysis.v1.ConditionState" json:"state,omitempty"`
	DistanceToThreshold float64                `protobuf:"fixed64,6,opt,name=distance_to_threshold,json=distanceToThreshold,proto3" json:"distance_to_threshold,omitempty"` // normalized
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *ConditionEval) Reset() {
	*x = ConditionEval{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[34]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ConditionEval) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ConditionEval) ProtoMessage() {}

func (x *ConditionEval) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[34]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ConditionEval.ProtoReflect.Descriptor instead.
func (*ConditionEval) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{34}
}

func (x *ConditionEval) GetRefName() string {
	if x != nil {
		return x.RefName
	}
	return ""
}

func (x *ConditionEval) GetLhsValue() float64 {
	if x != nil {
		return x.LhsValue
	}
	return 0
}

func (x *ConditionEval) GetThreshold() float64 {
	if x != nil {
		return x.Threshold
	}
	return 0
}

func (x *ConditionEval) GetFn() string {
	if x != nil {
		return x.Fn
	}
	return ""
}

func (x *ConditionEval) GetState() ConditionState {
	if x != nil {
		return x.State
	}
	return ConditionState_CONDITION_STATE_UNSPECIFIED
}

func (x *ConditionEval) GetDistanceToThreshold() float64 {
	if x != nil {
		return x.DistanceToThreshold
	}
	return 0
}

// Per-symbol readiness — the traced evaluation of a strategy's entry rule.
type SymbolReadiness struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	Symbol            string                 `protobuf:"bytes,1,opt,name=symbol,proto3" json:"symbol,omitempty"`
	Conviction        float64                `protobuf:"fixed64,2,opt,name=conviction,proto3" json:"conviction,omitempty"`
	PassingConditions int32                  `protobuf:"varint,3,opt,name=passing_conditions,json=passingConditions,proto3" json:"passing_conditions,omitempty"`
	TotalConditions   int32                  `protobuf:"varint,4,opt,name=total_conditions,json=totalConditions,proto3" json:"total_conditions,omitempty"`
	Conditions        []*ConditionEval       `protobuf:"bytes,5,rep,name=conditions,proto3" json:"conditions,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *SymbolReadiness) Reset() {
	*x = SymbolReadiness{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[35]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SymbolReadiness) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SymbolReadiness) ProtoMessage() {}

func (x *SymbolReadiness) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[35]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SymbolReadiness.ProtoReflect.Descriptor instead.
func (*SymbolReadiness) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{35}
}

func (x *SymbolReadiness) GetSymbol() string {
	if x != nil {
		return x.Symbol
	}
	return ""
}

func (x *SymbolReadiness) GetConviction() float64 {
	if x != nil {
		return x.Conviction
	}
	return 0
}

func (x *SymbolReadiness) GetPassingConditions() int32 {
	if x != nil {
		return x.PassingConditions
	}
	return 0
}

func (x *SymbolReadiness) GetTotalConditions() int32 {
	if x != nil {
		return x.TotalConditions
	}
	return 0
}

func (x *SymbolReadiness) GetConditions() []*ConditionEval {
	if x != nil {
		return x.Conditions
	}
	return nil
}

// Per-strategy analytics for the Engine → Strategies surface. expectancy and
// max_drawdown derive from persisted analysis.backtest_runs (win_rate +
// profit_factor); signals_30d from ingest QuerySignals; taken from trading
// ListOrders; queue_share from the opportunity-queue join.
type StrategyAnalytics struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	StrategyId     string                 `protobuf:"bytes,1,opt,name=strategy_id,json=strategyId,proto3" json:"strategy_id,omitempty"`
	Expectancy     float64                `protobuf:"fixed64,2,opt,name=expectancy,proto3" json:"expectancy,omitempty"`
	BlendedHitRate float64                `protobuf:"fixed64,3,opt,name=blended_hit_rate,json=blendedHitRate,proto3" json:"blended_hit_rate,omitempty"`
	MaxDrawdown    float64                `protobuf:"fixed64,4,opt,name=max_drawdown,json=maxDrawdown,proto3" json:"max_drawdown,omitempty"`
	Signals_30D    int32                  `protobuf:"varint,5,opt,name=signals_30d,json=signals30d,proto3" json:"signals_30d,omitempty"`
	Taken          int32                  `protobuf:"varint,6,opt,name=taken,proto3" json:"taken,omitempty"`
	QueueShare     float64                `protobuf:"fixed64,7,opt,name=queue_share,json=queueShare,proto3" json:"queue_share,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *StrategyAnalytics) Reset() {
	*x = StrategyAnalytics{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[36]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StrategyAnalytics) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StrategyAnalytics) ProtoMessage() {}

func (x *StrategyAnalytics) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[36]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StrategyAnalytics.ProtoReflect.Descriptor instead.
func (*StrategyAnalytics) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{36}
}

func (x *StrategyAnalytics) GetStrategyId() string {
	if x != nil {
		return x.StrategyId
	}
	return ""
}

func (x *StrategyAnalytics) GetExpectancy() float64 {
	if x != nil {
		return x.Expectancy
	}
	return 0
}

func (x *StrategyAnalytics) GetBlendedHitRate() float64 {
	if x != nil {
		return x.BlendedHitRate
	}
	return 0
}

func (x *StrategyAnalytics) GetMaxDrawdown() float64 {
	if x != nil {
		return x.MaxDrawdown
	}
	return 0
}

func (x *StrategyAnalytics) GetSignals_30D() int32 {
	if x != nil {
		return x.Signals_30D
	}
	return 0
}

func (x *StrategyAnalytics) GetTaken() int32 {
	if x != nil {
		return x.Taken
	}
	return 0
}

func (x *StrategyAnalytics) GetQueueShare() float64 {
	if x != nil {
		return x.QueueShare
	}
	return 0
}

// user_id is intentionally absent — taken from the propagated x-user-id header
// server-side (matching the portfolio watchlist convention), never from the wire.
type ListOpportunitiesRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Page          *v1.PageRequest        `protobuf:"bytes,1,opt,name=page,proto3" json:"page,omitempty"`
	MinConviction float64                `protobuf:"fixed64,2,opt,name=min_conviction,json=minConviction,proto3" json:"min_conviction,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListOpportunitiesRequest) Reset() {
	*x = ListOpportunitiesRequest{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[37]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListOpportunitiesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListOpportunitiesRequest) ProtoMessage() {}

func (x *ListOpportunitiesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[37]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListOpportunitiesRequest.ProtoReflect.Descriptor instead.
func (*ListOpportunitiesRequest) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{37}
}

func (x *ListOpportunitiesRequest) GetPage() *v1.PageRequest {
	if x != nil {
		return x.Page
	}
	return nil
}

func (x *ListOpportunitiesRequest) GetMinConviction() float64 {
	if x != nil {
		return x.MinConviction
	}
	return 0
}

type ListOpportunitiesResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Opportunities []*Opportunity         `protobuf:"bytes,1,rep,name=opportunities,proto3" json:"opportunities,omitempty"`
	Page          *v1.PageResponse       `protobuf:"bytes,2,opt,name=page,proto3" json:"page,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListOpportunitiesResponse) Reset() {
	*x = ListOpportunitiesResponse{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[38]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListOpportunitiesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListOpportunitiesResponse) ProtoMessage() {}

func (x *ListOpportunitiesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[38]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListOpportunitiesResponse.ProtoReflect.Descriptor instead.
func (*ListOpportunitiesResponse) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{38}
}

func (x *ListOpportunitiesResponse) GetOpportunities() []*Opportunity {
	if x != nil {
		return x.Opportunities
	}
	return nil
}

func (x *ListOpportunitiesResponse) GetPage() *v1.PageResponse {
	if x != nil {
		return x.Page
	}
	return nil
}

type EvaluateReadinessRequest struct {
	state      protoimpl.MessageState `protogen:"open.v1"`
	StrategyId string                 `protobuf:"bytes,1,opt,name=strategy_id,json=strategyId,proto3" json:"strategy_id,omitempty"`
	Symbols    []string               `protobuf:"bytes,2,rep,name=symbols,proto3" json:"symbols,omitempty"`
	// feature 138 — which rule tree to trace. UNSPECIFIED == ENTRY (back-compat). The Signal-detail
	// "Why this fired" panel requests EXIT for a held (REDUCE/ADD) opportunity so it explains the
	// exit rule that actually fired, reconciling with the queue's exit-derived conviction; every
	// other caller (watchlist readiness) leaves it unset and keeps entry-rule tracing.
	Rule          ReadinessRule `protobuf:"varint,3,opt,name=rule,proto3,enum=xstockstrat.analysis.v1.ReadinessRule" json:"rule,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EvaluateReadinessRequest) Reset() {
	*x = EvaluateReadinessRequest{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[39]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EvaluateReadinessRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EvaluateReadinessRequest) ProtoMessage() {}

func (x *EvaluateReadinessRequest) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[39]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EvaluateReadinessRequest.ProtoReflect.Descriptor instead.
func (*EvaluateReadinessRequest) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{39}
}

func (x *EvaluateReadinessRequest) GetStrategyId() string {
	if x != nil {
		return x.StrategyId
	}
	return ""
}

func (x *EvaluateReadinessRequest) GetSymbols() []string {
	if x != nil {
		return x.Symbols
	}
	return nil
}

func (x *EvaluateReadinessRequest) GetRule() ReadinessRule {
	if x != nil {
		return x.Rule
	}
	return ReadinessRule_READINESS_RULE_UNSPECIFIED
}

type EvaluateReadinessResponse struct {
	state     protoimpl.MessageState `protogen:"open.v1"`
	Readiness []*SymbolReadiness     `protobuf:"bytes,1,rep,name=readiness,proto3" json:"readiness,omitempty"`
	// The oldest per-symbol cache "computed at" among the served rows — the response is never
	// presented as fresher than this (feature 177, FR-5). Bounded by the readiness staleness window.
	ComputedAt    *timestamppb.Timestamp `protobuf:"bytes,2,opt,name=computed_at,json=computedAt,proto3" json:"computed_at,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EvaluateReadinessResponse) Reset() {
	*x = EvaluateReadinessResponse{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[40]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EvaluateReadinessResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EvaluateReadinessResponse) ProtoMessage() {}

func (x *EvaluateReadinessResponse) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[40]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EvaluateReadinessResponse.ProtoReflect.Descriptor instead.
func (*EvaluateReadinessResponse) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{40}
}

func (x *EvaluateReadinessResponse) GetReadiness() []*SymbolReadiness {
	if x != nil {
		return x.Readiness
	}
	return nil
}

func (x *EvaluateReadinessResponse) GetComputedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.ComputedAt
	}
	return nil
}

// user_id is intentionally absent — taken from the propagated x-user-id header server-side
// (match the ListOpportunitiesRequest convention), never from the wire.
type SetOpportunityActionRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	OpportunityKey string                 `protobuf:"bytes,1,opt,name=opportunity_key,json=opportunityKey,proto3" json:"opportunity_key,omitempty"` // the server-issued key, echoed verbatim
	Action         OpportunityAction      `protobuf:"varint,2,opt,name=action,proto3,enum=xstockstrat.analysis.v1.OpportunityAction" json:"action,omitempty"`
	SnoozeUntil    *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=snooze_until,json=snoozeUntil,proto3" json:"snooze_until,omitempty"` // set only for SNOOZE; a bounded "snooze until"
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *SetOpportunityActionRequest) Reset() {
	*x = SetOpportunityActionRequest{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[41]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SetOpportunityActionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SetOpportunityActionRequest) ProtoMessage() {}

func (x *SetOpportunityActionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[41]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SetOpportunityActionRequest.ProtoReflect.Descriptor instead.
func (*SetOpportunityActionRequest) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{41}
}

func (x *SetOpportunityActionRequest) GetOpportunityKey() string {
	if x != nil {
		return x.OpportunityKey
	}
	return ""
}

func (x *SetOpportunityActionRequest) GetAction() OpportunityAction {
	if x != nil {
		return x.Action
	}
	return OpportunityAction_OPPORTUNITY_ACTION_UNSPECIFIED
}

func (x *SetOpportunityActionRequest) GetSnoozeUntil() *timestamppb.Timestamp {
	if x != nil {
		return x.SnoozeUntil
	}
	return nil
}

type SetOpportunityActionResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SetOpportunityActionResponse) Reset() {
	*x = SetOpportunityActionResponse{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[42]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SetOpportunityActionResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SetOpportunityActionResponse) ProtoMessage() {}

func (x *SetOpportunityActionResponse) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[42]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SetOpportunityActionResponse.ProtoReflect.Descriptor instead.
func (*SetOpportunityActionResponse) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{42}
}

type GetStrategyAnalyticsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StrategyId    string                 `protobuf:"bytes,1,opt,name=strategy_id,json=strategyId,proto3" json:"strategy_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetStrategyAnalyticsRequest) Reset() {
	*x = GetStrategyAnalyticsRequest{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[43]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetStrategyAnalyticsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetStrategyAnalyticsRequest) ProtoMessage() {}

func (x *GetStrategyAnalyticsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[43]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetStrategyAnalyticsRequest.ProtoReflect.Descriptor instead.
func (*GetStrategyAnalyticsRequest) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{43}
}

func (x *GetStrategyAnalyticsRequest) GetStrategyId() string {
	if x != nil {
		return x.StrategyId
	}
	return ""
}

type GetIndicatorSeriesRequest struct {
	state      protoimpl.MessageState `protogen:"open.v1"`
	StrategyId string                 `protobuf:"bytes,1,opt,name=strategy_id,json=strategyId,proto3" json:"strategy_id,omitempty"`
	Symbol     string                 `protobuf:"bytes,2,opt,name=symbol,proto3" json:"symbol,omitempty"`
	// The caller's own already-fetched candlestick closes + their timestamps (the page passes the
	// exact bars it drew, so the x-axis is parity-aligned and no server re-fetch happens). closes
	// and times are index-aligned and equal length.
	Closes        []float64                `protobuf:"fixed64,3,rep,packed,name=closes,proto3" json:"closes,omitempty"`
	Times         []*timestamppb.Timestamp `protobuf:"bytes,4,rep,name=times,proto3" json:"times,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetIndicatorSeriesRequest) Reset() {
	*x = GetIndicatorSeriesRequest{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[44]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetIndicatorSeriesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetIndicatorSeriesRequest) ProtoMessage() {}

func (x *GetIndicatorSeriesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[44]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetIndicatorSeriesRequest.ProtoReflect.Descriptor instead.
func (*GetIndicatorSeriesRequest) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{44}
}

func (x *GetIndicatorSeriesRequest) GetStrategyId() string {
	if x != nil {
		return x.StrategyId
	}
	return ""
}

func (x *GetIndicatorSeriesRequest) GetSymbol() string {
	if x != nil {
		return x.Symbol
	}
	return ""
}

func (x *GetIndicatorSeriesRequest) GetCloses() []float64 {
	if x != nil {
		return x.Closes
	}
	return nil
}

func (x *GetIndicatorSeriesRequest) GetTimes() []*timestamppb.Timestamp {
	if x != nil {
		return x.Times
	}
	return nil
}

type GetIndicatorSeriesResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Echoes the request times, index-aligned across every series in every component.
	Times         []*timestamppb.Timestamp `protobuf:"bytes,1,rep,name=times,proto3" json:"times,omitempty"`
	Components    []*ComponentSeries       `protobuf:"bytes,2,rep,name=components,proto3" json:"components,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetIndicatorSeriesResponse) Reset() {
	*x = GetIndicatorSeriesResponse{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[45]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetIndicatorSeriesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetIndicatorSeriesResponse) ProtoMessage() {}

func (x *GetIndicatorSeriesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[45]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetIndicatorSeriesResponse.ProtoReflect.Descriptor instead.
func (*GetIndicatorSeriesResponse) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{45}
}

func (x *GetIndicatorSeriesResponse) GetTimes() []*timestamppb.Timestamp {
	if x != nil {
		return x.Times
	}
	return nil
}

func (x *GetIndicatorSeriesResponse) GetComponents() []*ComponentSeries {
	if x != nil {
		return x.Components
	}
	return nil
}

type ComponentSeries struct {
	state   protoimpl.MessageState `protogen:"open.v1"`
	RefName string                 `protobuf:"bytes,1,opt,name=ref_name,json=refName,proto3" json:"ref_name,omitempty"`
	Kind    ComponentKind          `protobuf:"varint,2,opt,name=kind,proto3,enum=xstockstrat.analysis.v1.ComponentKind" json:"kind,omitempty"`
	Series  []*NamedSeries         `protobuf:"bytes,3,rep,name=series,proto3" json:"series,omitempty"`
	// Non-empty when this component failed to compute (soft-deleted formula, sandbox timeout, NaN
	// output); series is then empty and the UI renders a per-panel error state. Per-component fault
	// isolation — one bad component never fails the whole RPC.
	Error         string `protobuf:"bytes,4,opt,name=error,proto3" json:"error,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ComponentSeries) Reset() {
	*x = ComponentSeries{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[46]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ComponentSeries) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ComponentSeries) ProtoMessage() {}

func (x *ComponentSeries) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[46]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ComponentSeries.ProtoReflect.Descriptor instead.
func (*ComponentSeries) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{46}
}

func (x *ComponentSeries) GetRefName() string {
	if x != nil {
		return x.RefName
	}
	return ""
}

func (x *ComponentSeries) GetKind() ComponentKind {
	if x != nil {
		return x.Kind
	}
	return ComponentKind_COMPONENT_KIND_UNSPECIFIED
}

func (x *ComponentSeries) GetSeries() []*NamedSeries {
	if x != nil {
		return x.Series
	}
	return nil
}

func (x *ComponentSeries) GetError() string {
	if x != nil {
		return x.Error
	}
	return ""
}

type NamedSeries struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// "value" (primary) plus each secondary the component emits (bb.upper/bb.lower,
	// macd.signal/macd.histogram, stoch.d, or custom-formula output keys).
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Index-aligned with the response times. Each point is an IndicatorValue whose `value` is UNSET
	// for a warm-up-head or mid-series None, so a gap never round-trips as a fabricated 0.0 (feature
	// 125, AC-4a/P-03). A bare `google.protobuf.DoubleValue` element cannot do this — in a repeated
	// field an empty DoubleValue is byte-identical to DoubleValue(0.0) and serializes to JSON `0`, so
	// the wrapper is a message with a proto3 `optional double` (explicit presence: HasField works and
	// JSON omits an unset value) instead.
	Values        []*IndicatorValue `protobuf:"bytes,2,rep,name=values,proto3" json:"values,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *NamedSeries) Reset() {
	*x = NamedSeries{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[47]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *NamedSeries) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*NamedSeries) ProtoMessage() {}

func (x *NamedSeries) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[47]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use NamedSeries.ProtoReflect.Descriptor instead.
func (*NamedSeries) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{47}
}

func (x *NamedSeries) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *NamedSeries) GetValues() []*IndicatorValue {
	if x != nil {
		return x.Values
	}
	return nil
}

// One point of an indicator series. `value` unset == a gap (warm-up head / NaN / None), never 0.0.
type IndicatorValue struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Value         *float64               `protobuf:"fixed64,1,opt,name=value,proto3,oneof" json:"value,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *IndicatorValue) Reset() {
	*x = IndicatorValue{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[48]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *IndicatorValue) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*IndicatorValue) ProtoMessage() {}

func (x *IndicatorValue) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[48]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use IndicatorValue.ProtoReflect.Descriptor instead.
func (*IndicatorValue) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{48}
}

func (x *IndicatorValue) GetValue() float64 {
	if x != nil && x.Value != nil {
		return *x.Value
	}
	return 0
}

// One signal active for the symbol at snapshot time; `value` is the ingest conviction (0.0–1.0).
type SignalEntry struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	Value         float64                `protobuf:"fixed64,2,opt,name=value,proto3" json:"value,omitempty"`
	Source        string                 `protobuf:"bytes,3,opt,name=source,proto3" json:"source,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SignalEntry) Reset() {
	*x = SignalEntry{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[49]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SignalEntry) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SignalEntry) ProtoMessage() {}

func (x *SignalEntry) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[49]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SignalEntry.ProtoReflect.Descriptor instead.
func (*SignalEntry) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{49}
}

func (x *SignalEntry) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *SignalEntry) GetValue() float64 {
	if x != nil {
		return x.Value
	}
	return 0
}

func (x *SignalEntry) GetSource() string {
	if x != nil {
		return x.Source
	}
	return ""
}

// A point-in-time capture of the indicator/signal/market context at an order event.
type OrderSnapshot struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	OrderId         string                 `protobuf:"bytes,1,opt,name=order_id,json=orderId,proto3" json:"order_id,omitempty"`
	PositionId      string                 `protobuf:"bytes,2,opt,name=position_id,json=positionId,proto3" json:"position_id,omitempty"`
	Symbol          string                 `protobuf:"bytes,3,opt,name=symbol,proto3" json:"symbol,omitempty"`
	EventType       SnapshotEventType      `protobuf:"varint,4,opt,name=event_type,json=eventType,proto3,enum=xstockstrat.analysis.v1.SnapshotEventType" json:"event_type,omitempty"`
	EventTs         *timestamppb.Timestamp `protobuf:"bytes,5,opt,name=event_ts,json=eventTs,proto3" json:"event_ts,omitempty"`
	Side            string                 `protobuf:"bytes,6,opt,name=side,proto3" json:"side,omitempty"`
	Quantity        float64                `protobuf:"fixed64,7,opt,name=quantity,proto3" json:"quantity,omitempty"`
	Price           float64                `protobuf:"fixed64,8,opt,name=price,proto3" json:"price,omitempty"`
	OhlcvBar        *structpb.Struct       `protobuf:"bytes,9,opt,name=ohlcv_bar,json=ohlcvBar,proto3" json:"ohlcv_bar,omitempty"`
	IndicatorValues map[string]float64     `protobuf:"bytes,10,rep,name=indicator_values,json=indicatorValues,proto3" json:"indicator_values,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"fixed64,2,opt,name=value"`
	Signals         []*SignalEntry         `protobuf:"bytes,11,rep,name=signals,proto3" json:"signals,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *OrderSnapshot) Reset() {
	*x = OrderSnapshot{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[50]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *OrderSnapshot) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OrderSnapshot) ProtoMessage() {}

func (x *OrderSnapshot) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[50]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OrderSnapshot.ProtoReflect.Descriptor instead.
func (*OrderSnapshot) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{50}
}

func (x *OrderSnapshot) GetOrderId() string {
	if x != nil {
		return x.OrderId
	}
	return ""
}

func (x *OrderSnapshot) GetPositionId() string {
	if x != nil {
		return x.PositionId
	}
	return ""
}

func (x *OrderSnapshot) GetSymbol() string {
	if x != nil {
		return x.Symbol
	}
	return ""
}

func (x *OrderSnapshot) GetEventType() SnapshotEventType {
	if x != nil {
		return x.EventType
	}
	return SnapshotEventType_SNAPSHOT_EVENT_TYPE_UNSPECIFIED
}

func (x *OrderSnapshot) GetEventTs() *timestamppb.Timestamp {
	if x != nil {
		return x.EventTs
	}
	return nil
}

func (x *OrderSnapshot) GetSide() string {
	if x != nil {
		return x.Side
	}
	return ""
}

func (x *OrderSnapshot) GetQuantity() float64 {
	if x != nil {
		return x.Quantity
	}
	return 0
}

func (x *OrderSnapshot) GetPrice() float64 {
	if x != nil {
		return x.Price
	}
	return 0
}

func (x *OrderSnapshot) GetOhlcvBar() *structpb.Struct {
	if x != nil {
		return x.OhlcvBar
	}
	return nil
}

func (x *OrderSnapshot) GetIndicatorValues() map[string]float64 {
	if x != nil {
		return x.IndicatorValues
	}
	return nil
}

func (x *OrderSnapshot) GetSignals() []*SignalEntry {
	if x != nil {
		return x.Signals
	}
	return nil
}

// A ranked attribution factor: an indicator value-range or a signal, with its avg realized-P&L impact.
type PnLPatternFactor struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	FactorName     string                 `protobuf:"bytes,1,opt,name=factor_name,json=factorName,proto3" json:"factor_name,omitempty"`
	FactorType     FactorType             `protobuf:"varint,2,opt,name=factor_type,json=factorType,proto3,enum=xstockstrat.analysis.v1.FactorType" json:"factor_type,omitempty"`
	ValueRangeLow  float64                `protobuf:"fixed64,3,opt,name=value_range_low,json=valueRangeLow,proto3" json:"value_range_low,omitempty"`
	ValueRangeHigh float64                `protobuf:"fixed64,4,opt,name=value_range_high,json=valueRangeHigh,proto3" json:"value_range_high,omitempty"`
	SampleCount    int32                  `protobuf:"varint,5,opt,name=sample_count,json=sampleCount,proto3" json:"sample_count,omitempty"`
	AvgPnlImpact   float64                `protobuf:"fixed64,6,opt,name=avg_pnl_impact,json=avgPnlImpact,proto3" json:"avg_pnl_impact,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *PnLPatternFactor) Reset() {
	*x = PnLPatternFactor{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[51]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PnLPatternFactor) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PnLPatternFactor) ProtoMessage() {}

func (x *PnLPatternFactor) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[51]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PnLPatternFactor.ProtoReflect.Descriptor instead.
func (*PnLPatternFactor) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{51}
}

func (x *PnLPatternFactor) GetFactorName() string {
	if x != nil {
		return x.FactorName
	}
	return ""
}

func (x *PnLPatternFactor) GetFactorType() FactorType {
	if x != nil {
		return x.FactorType
	}
	return FactorType_FACTOR_TYPE_UNSPECIFIED
}

func (x *PnLPatternFactor) GetValueRangeLow() float64 {
	if x != nil {
		return x.ValueRangeLow
	}
	return 0
}

func (x *PnLPatternFactor) GetValueRangeHigh() float64 {
	if x != nil {
		return x.ValueRangeHigh
	}
	return 0
}

func (x *PnLPatternFactor) GetSampleCount() int32 {
	if x != nil {
		return x.SampleCount
	}
	return 0
}

func (x *PnLPatternFactor) GetAvgPnlImpact() float64 {
	if x != nil {
		return x.AvgPnlImpact
	}
	return 0
}

type QueryPnLPatternsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Symbol        string                 `protobuf:"bytes,1,opt,name=symbol,proto3" json:"symbol,omitempty"`
	StrategyId    string                 `protobuf:"bytes,2,opt,name=strategy_id,json=strategyId,proto3" json:"strategy_id,omitempty"`
	FromTs        *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=from_ts,json=fromTs,proto3" json:"from_ts,omitempty"`
	ToTs          *timestamppb.Timestamp `protobuf:"bytes,4,opt,name=to_ts,json=toTs,proto3" json:"to_ts,omitempty"`
	Limit         int32                  `protobuf:"varint,5,opt,name=limit,proto3" json:"limit,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *QueryPnLPatternsRequest) Reset() {
	*x = QueryPnLPatternsRequest{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[52]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *QueryPnLPatternsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QueryPnLPatternsRequest) ProtoMessage() {}

func (x *QueryPnLPatternsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[52]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use QueryPnLPatternsRequest.ProtoReflect.Descriptor instead.
func (*QueryPnLPatternsRequest) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{52}
}

func (x *QueryPnLPatternsRequest) GetSymbol() string {
	if x != nil {
		return x.Symbol
	}
	return ""
}

func (x *QueryPnLPatternsRequest) GetStrategyId() string {
	if x != nil {
		return x.StrategyId
	}
	return ""
}

func (x *QueryPnLPatternsRequest) GetFromTs() *timestamppb.Timestamp {
	if x != nil {
		return x.FromTs
	}
	return nil
}

func (x *QueryPnLPatternsRequest) GetToTs() *timestamppb.Timestamp {
	if x != nil {
		return x.ToTs
	}
	return nil
}

func (x *QueryPnLPatternsRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

type QueryPnLPatternsResponse struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	PositiveFactors []*PnLPatternFactor    `protobuf:"bytes,1,rep,name=positive_factors,json=positiveFactors,proto3" json:"positive_factors,omitempty"`
	NegativeFactors []*PnLPatternFactor    `protobuf:"bytes,2,rep,name=negative_factors,json=negativeFactors,proto3" json:"negative_factors,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *QueryPnLPatternsResponse) Reset() {
	*x = QueryPnLPatternsResponse{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[53]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *QueryPnLPatternsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QueryPnLPatternsResponse) ProtoMessage() {}

func (x *QueryPnLPatternsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[53]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use QueryPnLPatternsResponse.ProtoReflect.Descriptor instead.
func (*QueryPnLPatternsResponse) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{53}
}

func (x *QueryPnLPatternsResponse) GetPositiveFactors() []*PnLPatternFactor {
	if x != nil {
		return x.PositiveFactors
	}
	return nil
}

func (x *QueryPnLPatternsResponse) GetNegativeFactors() []*PnLPatternFactor {
	if x != nil {
		return x.NegativeFactors
	}
	return nil
}

// ── Signal-performance attribution (feature 029) ───────────────────────────────
type GetAttributionRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Start         *timestamppb.Timestamp `protobuf:"bytes,1,opt,name=start,proto3" json:"start,omitempty"`
	End           *timestamppb.Timestamp `protobuf:"bytes,2,opt,name=end,proto3" json:"end,omitempty"`
	SourceId      string                 `protobuf:"bytes,3,opt,name=source_id,json=sourceId,proto3" json:"source_id,omitempty"` // optional filter — the signal_sources.slug; empty = all sources (open registry, C-04: string not enum)
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetAttributionRequest) Reset() {
	*x = GetAttributionRequest{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[54]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAttributionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAttributionRequest) ProtoMessage() {}

func (x *GetAttributionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[54]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAttributionRequest.ProtoReflect.Descriptor instead.
func (*GetAttributionRequest) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{54}
}

func (x *GetAttributionRequest) GetStart() *timestamppb.Timestamp {
	if x != nil {
		return x.Start
	}
	return nil
}

func (x *GetAttributionRequest) GetEnd() *timestamppb.Timestamp {
	if x != nil {
		return x.End
	}
	return nil
}

func (x *GetAttributionRequest) GetSourceId() string {
	if x != nil {
		return x.SourceId
	}
	return ""
}

// Per-source metrics. trade_count/win_count are DOUBLE (not int32): FR-3's exact-tie case
// contributes 0.5 to each tied source (AC-5); winner-takes-all contributes 1.0. total_pnl is
// NET of fees (realized_pnl − fees_total). avg_return is a percent over an approximate cost basis.
type SourceAttribution struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	SourceId      string                 `protobuf:"bytes,1,opt,name=source_id,json=sourceId,proto3" json:"source_id,omitempty"`       // signal_sources.slug (the snapshot's signal source)
	SourceName    string                 `protobuf:"bytes,2,opt,name=source_name,json=sourceName,proto3" json:"source_name,omitempty"` // resolved via ingest ListSignalSources; falls back to the slug
	TradeCount    float64                `protobuf:"fixed64,3,opt,name=trade_count,json=tradeCount,proto3" json:"trade_count,omitempty"`
	WinCount      float64                `protobuf:"fixed64,4,opt,name=win_count,json=winCount,proto3" json:"win_count,omitempty"`
	WinRate       float64                `protobuf:"fixed64,5,opt,name=win_rate,json=winRate,proto3" json:"win_rate,omitempty"`       // win_count / trade_count
	AvgReturn     float64                `protobuf:"fixed64,6,opt,name=avg_return,json=avgReturn,proto3" json:"avg_return,omitempty"` // mean per-trade net_pnl / cost_basis (percent, v1 approximation)
	TotalPnl      float64                `protobuf:"fixed64,7,opt,name=total_pnl,json=totalPnl,proto3" json:"total_pnl,omitempty"`    // net of fees
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SourceAttribution) Reset() {
	*x = SourceAttribution{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[55]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SourceAttribution) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SourceAttribution) ProtoMessage() {}

func (x *SourceAttribution) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[55]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SourceAttribution.ProtoReflect.Descriptor instead.
func (*SourceAttribution) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{55}
}

func (x *SourceAttribution) GetSourceId() string {
	if x != nil {
		return x.SourceId
	}
	return ""
}

func (x *SourceAttribution) GetSourceName() string {
	if x != nil {
		return x.SourceName
	}
	return ""
}

func (x *SourceAttribution) GetTradeCount() float64 {
	if x != nil {
		return x.TradeCount
	}
	return 0
}

func (x *SourceAttribution) GetWinCount() float64 {
	if x != nil {
		return x.WinCount
	}
	return 0
}

func (x *SourceAttribution) GetWinRate() float64 {
	if x != nil {
		return x.WinRate
	}
	return 0
}

func (x *SourceAttribution) GetAvgReturn() float64 {
	if x != nil {
		return x.AvgReturn
	}
	return 0
}

func (x *SourceAttribution) GetTotalPnl() float64 {
	if x != nil {
		return x.TotalPnl
	}
	return 0
}

type GetAttributionResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Attributions  []*SourceAttribution   `protobuf:"bytes,1,rep,name=attributions,proto3" json:"attributions,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetAttributionResponse) Reset() {
	*x = GetAttributionResponse{}
	mi := &file_analysis_v1_analysis_proto_msgTypes[56]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAttributionResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAttributionResponse) ProtoMessage() {}

func (x *GetAttributionResponse) ProtoReflect() protoreflect.Message {
	mi := &file_analysis_v1_analysis_proto_msgTypes[56]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAttributionResponse.ProtoReflect.Descriptor instead.
func (*GetAttributionResponse) Descriptor() ([]byte, []int) {
	return file_analysis_v1_analysis_proto_rawDescGZIP(), []int{56}
}

func (x *GetAttributionResponse) GetAttributions() []*SourceAttribution {
	if x != nil {
		return x.Attributions
	}
	return nil
}

var File_analysis_v1_analysis_proto protoreflect.FileDescriptor

const file_analysis_v1_analysis_proto_rawDesc = "" +
	"\n" +
	"\x1aanalysis/v1/analysis.proto\x12\x17xstockstrat.analysis.v1\x1a\x1fgoogle/protobuf/timestamp.proto\x1a\x1cgoogle/protobuf/struct.proto\x1a google/protobuf/field_mask.proto\x1a\x16common/v1/common.proto\"\xfd\x03\n" +
	"\x12RunBacktestRequest\x12\x1f\n" +
	"\vstrategy_id\x18\x01 \x01(\tR\n" +
	"strategyId\x126\n" +
	"\x05range\x18\x02 \x01(\v2 .xstockstrat.common.v1.TimeRangeR\x05range\x12\x18\n" +
	"\asymbols\x18\x03 \x03(\tR\asymbols\x12'\n" +
	"\x0finitial_capital\x18\x04 \x01(\x01R\x0einitialCapital\x12@\n" +
	"\x0fstrategy_params\x18\x05 \x01(\v2\x17.google.protobuf.StructR\x0estrategyParams\x12&\n" +
	"\x0fstrategy_id_ref\x18\x06 \x01(\tR\rstrategyIdRef\x12X\n" +
	"\x11inline_definition\x18\a \x01(\v2+.xstockstrat.analysis.v1.StrategyDefinitionR\x10inlineDefinition\x12D\n" +
	"\vsizing_mode\x18\b \x01(\x0e2#.xstockstrat.analysis.v1.SizingModeR\n" +
	"sizingMode\x12A\n" +
	"\n" +
	"fill_model\x18\t \x01(\x0e2\".xstockstrat.analysis.v1.FillModelR\tfillModel\"\x9e\x02\n" +
	"\vCoverageGap\x12\x16\n" +
	"\x06symbol\x18\x01 \x01(\tR\x06symbol\x12>\n" +
	"\ttimeframe\x18\x02 \x01(\x0e2 .xstockstrat.common.v1.TimeframeR\ttimeframe\x12I\n" +
	"\x0frequested_range\x18\x03 \x01(\v2 .xstockstrat.common.v1.TimeRangeR\x0erequestedRange\x12\x1b\n" +
	"\tbars_have\x18\x04 \x01(\x03R\bbarsHave\x12\x1b\n" +
	"\tbars_need\x18\x05 \x01(\x03R\bbarsNeed\x122\n" +
	"\x03gap\x18\x06 \x01(\v2 .xstockstrat.common.v1.TimeRangeR\x03gap\"\xa0\b\n" +
	"\x0eBacktestResult\x12\x1f\n" +
	"\vbacktest_id\x18\x01 \x01(\tR\n" +
	"backtestId\x12\x1f\n" +
	"\vstrategy_id\x18\x02 \x01(\tR\n" +
	"strategyId\x12!\n" +
	"\ftotal_return\x18\x03 \x01(\x01R\vtotalReturn\x12+\n" +
	"\x11annualized_return\x18\x04 \x01(\x01R\x10annualizedReturn\x12!\n" +
	"\fsharpe_ratio\x18\x05 \x01(\x01R\vsharpeRatio\x12!\n" +
	"\fmax_drawdown\x18\x06 \x01(\x01R\vmaxDrawdown\x12\x19\n" +
	"\bwin_rate\x18\a \x01(\x01R\awinRate\x12!\n" +
	"\ftotal_trades\x18\b \x01(\x05R\vtotalTrades\x12#\n" +
	"\rprofit_factor\x18\t \x01(\x01R\fprofitFactor\x12=\n" +
	"\fcompleted_at\x18\n" +
	" \x01(\v2\x1a.google.protobuf.TimestampR\vcompletedAt\x12<\n" +
	"\x06trades\x18\v \x03(\v2$.xstockstrat.analysis.v1.TradeRecordR\x06trades\x12?\n" +
	"\x06status\x18\f \x01(\x0e2'.xstockstrat.analysis.v1.BacktestStatusR\x06status\x12I\n" +
	"\rcoverage_gaps\x18\r \x03(\v2$.xstockstrat.analysis.v1.CoverageGapR\fcoverageGaps\x12L\n" +
	"\vdiagnostics\x18\x0e \x03(\v2*.xstockstrat.analysis.v1.SymbolDiagnosticsR\vdiagnostics\x12'\n" +
	"\x0finitial_capital\x18\x0f \x01(\x01R\x0einitialCapital\x12\x1a\n" +
	"\bwarnings\x18\x10 \x03(\tR\bwarnings\x12D\n" +
	"\vsizing_mode\x18\x11 \x01(\x0e2#.xstockstrat.analysis.v1.SizingModeR\n" +
	"sizingMode\x12R\n" +
	"\rcapital_skips\x18\x12 \x03(\v2-.xstockstrat.analysis.v1.PortfolioCapitalSkipR\fcapitalSkips\x12Z\n" +
	"\x16portfolio_equity_curve\x18\x13 \x03(\v2$.xstockstrat.analysis.v1.EquityPointR\x14portfolioEquityCurve\x12A\n" +
	"\n" +
	"fill_model\x18\x14 \x01(\x0e2\".xstockstrat.analysis.v1.FillModelR\tfillModel\"\xb8\x01\n" +
	"\x14PortfolioCapitalSkip\x12\x16\n" +
	"\x06symbol\x18\x01 \x01(\tR\x06symbol\x128\n" +
	"\ttimestamp\x18\x02 \x01(\v2\x1a.google.protobuf.TimestampR\ttimestamp\x12'\n" +
	"\x0fintended_weight\x18\x03 \x01(\x01R\x0eintendedWeight\x12%\n" +
	"\x0eavailable_cash\x18\x04 \x01(\x01R\ravailableCash\"_\n" +
	"\vEquityPoint\x128\n" +
	"\ttimestamp\x18\x01 \x01(\v2\x1a.google.protobuf.TimestampR\ttimestamp\x12\x16\n" +
	"\x06equity\x18\x02 \x01(\x01R\x06equity\"\x91\x02\n" +
	"\vTradeRecord\x12\x16\n" +
	"\x06symbol\x18\x01 \x01(\tR\x06symbol\x12\x12\n" +
	"\x04side\x18\x02 \x01(\tR\x04side\x12\x10\n" +
	"\x03qty\x18\x03 \x01(\x01R\x03qty\x12\x1f\n" +
	"\ventry_price\x18\x04 \x01(\x01R\n" +
	"entryPrice\x12\x1d\n" +
	"\n" +
	"exit_price\x18\x05 \x01(\x01R\texitPrice\x12\x10\n" +
	"\x03pnl\x18\x06 \x01(\x01R\x03pnl\x129\n" +
	"\n" +
	"entry_time\x18\a \x01(\v2\x1a.google.protobuf.TimestampR\tentryTime\x127\n" +
	"\texit_time\x18\b \x01(\v2\x1a.google.protobuf.TimestampR\bexitTime\"\xc0\x04\n" +
	"\rBarDiagnostic\x12\x16\n" +
	"\x06symbol\x18\x01 \x01(\tR\x06symbol\x12\x1b\n" +
	"\tbar_index\x18\x02 \x01(\x05R\bbarIndex\x128\n" +
	"\ttimestamp\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\ttimestamp\x12\x12\n" +
	"\x04open\x18\x04 \x01(\x01R\x04open\x12\x12\n" +
	"\x04high\x18\x05 \x01(\x01R\x04high\x12\x10\n" +
	"\x03low\x18\x06 \x01(\x01R\x03low\x12\x14\n" +
	"\x05close\x18\a \x01(\x01R\x05close\x12\x16\n" +
	"\x06volume\x18\b \x01(\x03R\x06volume\x12\x12\n" +
	"\x04vwap\x18\t \x01(\x01R\x04vwap\x12V\n" +
	"\n" +
	"indicators\x18\n" +
	" \x03(\v26.xstockstrat.analysis.v1.BarDiagnostic.IndicatorsEntryR\n" +
	"indicators\x12\x16\n" +
	"\x06warmup\x18\v \x01(\bR\x06warmup\x12!\n" +
	"\fsignal_score\x18\f \x01(\x01R\vsignalScore\x12\x1e\n" +
	"\n" +
	"conviction\x18\r \x01(\x01R\n" +
	"conviction\x12:\n" +
	"\x06action\x18\x0e \x01(\x0e2\".xstockstrat.analysis.v1.BarActionR\x06action\x12\x16\n" +
	"\x06equity\x18\x0f \x01(\x01R\x06equity\x1a=\n" +
	"\x0fIndicatorsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\x01R\x05value:\x028\x01\"\xf7\x01\n" +
	"\x11SymbolDiagnostics\x12\x16\n" +
	"\x06symbol\x18\x01 \x01(\tR\x06symbol\x12:\n" +
	"\x04bars\x18\x02 \x03(\v2&.xstockstrat.analysis.v1.BarDiagnosticR\x04bars\x12N\n" +
	"\x0fno_trade_reason\x18\x03 \x01(\x0e2&.xstockstrat.analysis.v1.NoTradeReasonR\rnoTradeReason\x12\x1d\n" +
	"\n" +
	"bars_total\x18\x04 \x01(\x05R\tbarsTotal\x12\x1f\n" +
	"\vwarmup_bars\x18\x05 \x01(\x05R\n" +
	"warmupBars\"o\n" +
	"\x14ScoreStrategyRequest\x12\x1f\n" +
	"\vstrategy_id\x18\x01 \x01(\tR\n" +
	"strategyId\x126\n" +
	"\x05range\x18\x02 \x01(\v2 .xstockstrat.common.v1.TimeRangeR\x05range\"\x8b\x03\n" +
	"\rStrategyScore\x12\x1f\n" +
	"\vstrategy_id\x18\x01 \x01(\tR\n" +
	"strategyId\x12#\n" +
	"\roverall_score\x18\x02 \x01(\x01R\foverallScore\x12f\n" +
	"\x10component_scores\x18\x03 \x03(\v2;.xstockstrat.analysis.v1.StrategyScore.ComponentScoresEntryR\x0fcomponentScores\x12\x16\n" +
	"\x06rating\x18\x04 \x01(\tR\x06rating\x12)\n" +
	"\x10evidence_symbols\x18\x05 \x01(\x05R\x0fevidenceSymbols\x12#\n" +
	"\revidence_days\x18\x06 \x01(\x05R\fevidenceDays\x12 \n" +
	"\vprovisional\x18\a \x01(\bR\vprovisional\x1aB\n" +
	"\x14ComponentScoresEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\x01R\x05value:\x028\x01\"\xf6\x01\n" +
	"\x0eStrategyReport\x12\x1f\n" +
	"\vstrategy_id\x18\x01 \x01(\tR\n" +
	"strategyId\x12P\n" +
	"\x0flatest_backtest\x18\x02 \x01(\v2'.xstockstrat.analysis.v1.BacktestResultR\x0elatestBacktest\x12<\n" +
	"\x05score\x18\x03 \x01(\v2&.xstockstrat.analysis.v1.StrategyScoreR\x05score\x123\n" +
	"\bmetadata\x18\x04 \x01(\v2\x17.google.protobuf.StructR\bmetadata\"M\n" +
	"\x14ListBacktestsRequest\x12\x1f\n" +
	"\vstrategy_id\x18\x01 \x01(\tR\n" +
	"strategyId\x12\x14\n" +
	"\x05limit\x18\x02 \x01(\x05R\x05limit\"\xa5\x06\n" +
	"\x12BacktestRunSummary\x12\x1f\n" +
	"\vbacktest_id\x18\x01 \x01(\tR\n" +
	"backtestId\x12\x1f\n" +
	"\vstrategy_id\x18\x02 \x01(\tR\n" +
	"strategyId\x12?\n" +
	"\x06status\x18\x03 \x01(\x0e2'.xstockstrat.analysis.v1.BacktestStatusR\x06status\x12!\n" +
	"\ftotal_return\x18\x04 \x01(\x01R\vtotalReturn\x12+\n" +
	"\x11annualized_return\x18\x05 \x01(\x01R\x10annualizedReturn\x12!\n" +
	"\fsharpe_ratio\x18\x06 \x01(\x01R\vsharpeRatio\x12!\n" +
	"\fmax_drawdown\x18\a \x01(\x01R\vmaxDrawdown\x12\x19\n" +
	"\bwin_rate\x18\b \x01(\x01R\awinRate\x12!\n" +
	"\ftotal_trades\x18\t \x01(\x05R\vtotalTrades\x12#\n" +
	"\rprofit_factor\x18\n" +
	" \x01(\x01R\fprofitFactor\x12\x18\n" +
	"\asymbols\x18\v \x03(\tR\asymbols\x12#\n" +
	"\roverall_score\x18\f \x01(\x01R\foverallScore\x12\x16\n" +
	"\x06rating\x18\r \x01(\tR\x06rating\x12=\n" +
	"\fcompleted_at\x18\x0e \x01(\v2\x1a.google.protobuf.TimestampR\vcompletedAt\x12;\n" +
	"\vrange_start\x18\x0f \x01(\v2\x1a.google.protobuf.TimestampR\n" +
	"rangeStart\x127\n" +
	"\trange_end\x18\x10 \x01(\v2\x1a.google.protobuf.TimestampR\brangeEnd\x12D\n" +
	"\vsizing_mode\x18\x11 \x01(\x0e2#.xstockstrat.analysis.v1.SizingModeR\n" +
	"sizingMode\x12A\n" +
	"\n" +
	"fill_model\x18\x12 \x01(\x0e2\".xstockstrat.analysis.v1.FillModelR\tfillModel\"X\n" +
	"\x15ListBacktestsResponse\x12?\n" +
	"\x04runs\x18\x01 \x03(\v2+.xstockstrat.analysis.v1.BacktestRunSummaryR\x04runs\"5\n" +
	"\x12GetBacktestRequest\x12\x1f\n" +
	"\vbacktest_id\x18\x01 \x01(\tR\n" +
	"backtestId\"l\n" +
	"\x15ListStrategiesRequest\x126\n" +
	"\x04page\x18\x01 \x01(\v2\".xstockstrat.common.v1.PageRequestR\x04page\x12\x1b\n" +
	"\auser_id\x18\x02 \x01(\tB\x02\x18\x01R\x06userId\"\x99\x01\n" +
	"\x16ListStrategiesResponse\x12F\n" +
	"\n" +
	"strategies\x18\x01 \x03(\v2&.xstockstrat.analysis.v1.StrategyScoreR\n" +
	"strategies\x127\n" +
	"\x04page\x18\x02 \x01(\v2#.xstockstrat.common.v1.PageResponseR\x04page\";\n" +
	"\x18GetStrategyReportRequest\x12\x1f\n" +
	"\vstrategy_id\x18\x01 \x01(\tR\n" +
	"strategyId\"\xd7\x02\n" +
	"\x11StrategyComponent\x12\x19\n" +
	"\bref_name\x18\x01 \x01(\tR\arefName\x12:\n" +
	"\x04kind\x18\x02 \x01(\x0e2&.xstockstrat.analysis.v1.ComponentKindR\x04kind\x12\x1c\n" +
	"\tindicator\x18\x03 \x01(\tR\tindicator\x12\x1d\n" +
	"\n" +
	"formula_id\x18\x04 \x01(\tR\tformulaId\x12N\n" +
	"\x06params\x18\x05 \x03(\v26.xstockstrat.analysis.v1.StrategyComponent.ParamsEntryR\x06params\x12#\n" +
	"\rsource_symbol\x18\x06 \x01(\tR\fsourceSymbol\x1a9\n" +
	"\vParamsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\x01R\x05value:\x028\x01\"\xe4\x04\n" +
	"\x12StrategyDefinition\x12\x1f\n" +
	"\vstrategy_id\x18\x01 \x01(\tR\n" +
	"strategyId\x12!\n" +
	"\fdisplay_name\x18\x02 \x01(\tR\vdisplayName\x12J\n" +
	"\n" +
	"components\x18\x03 \x03(\v2*.xstockstrat.analysis.v1.StrategyComponentR\n" +
	"components\x12\x1d\n" +
	"\n" +
	"entry_rule\x18\x04 \x01(\tR\tentryRule\x12\x1b\n" +
	"\texit_rule\x18\x05 \x01(\tR\bexitRule\x12<\n" +
	"\rsignal_params\x18\x06 \x01(\v2\x17.google.protobuf.StructR\fsignalParams\x12\x16\n" +
	"\x06active\x18\a \x01(\bR\x06active\x12!\n" +
	"\flive_enabled\x18\b \x01(\bR\vliveEnabled\x12(\n" +
	"\rcooldown_days\x18\t \x01(\x05H\x00R\fcooldownDays\x88\x01\x01\x12\x1a\n" +
	"\bwarnings\x18\n" +
	" \x03(\tR\bwarnings\x121\n" +
	"\x12exit_cooldown_days\x18\v \x01(\x05H\x01R\x10exitCooldownDays\x88\x01\x01\x12%\n" +
	"\x0edenied_symbols\x18\f \x03(\tR\rdeniedSymbols\x12\x17\n" +
	"\auser_id\x18\r \x01(\tR\x06userId\x12'\n" +
	"\x0fsignal_eligible\x18\x0e \x01(\bR\x0esignalEligibleB\x10\n" +
	"\x0e_cooldown_daysB\x15\n" +
	"\x13_exit_cooldown_days\"\xeb\x01\n" +
	"\x15ManageStrategyRequest\x12H\n" +
	"\toperation\x18\x01 \x01(\x0e2*.xstockstrat.analysis.v1.StrategyOperationR\toperation\x12K\n" +
	"\n" +
	"definition\x18\x02 \x01(\v2+.xstockstrat.analysis.v1.StrategyDefinitionR\n" +
	"definition\x12;\n" +
	"\vupdate_mask\x18\x03 \x01(\v2\x1a.google.protobuf.FieldMaskR\n" +
	"updateMask\"5\n" +
	"\x12GetStrategyRequest\x12\x1f\n" +
	"\vstrategy_id\x18\x01 \x01(\tR\n" +
	"strategyId\"\x89\x01\n" +
	"\x1eListStrategyDefinitionsRequest\x12)\n" +
	"\x10include_inactive\x18\x01 \x01(\bR\x0fincludeInactive\x12\x1b\n" +
	"\tpage_size\x18\x02 \x01(\x05R\bpageSize\x12\x1f\n" +
	"\vpage_offset\x18\x03 \x01(\x05R\n" +
	"pageOffset\"\x91\x01\n" +
	"\x1fListStrategyDefinitionsResponse\x12M\n" +
	"\vdefinitions\x18\x01 \x03(\v2+.xstockstrat.analysis.v1.StrategyDefinitionR\vdefinitions\x12\x1f\n" +
	"\vtotal_count\x18\x02 \x01(\x05R\n" +
	"totalCount\"\\\n" +
	"\x16SetStrategyLiveRequest\x12\x1f\n" +
	"\vstrategy_id\x18\x01 \x01(\tR\n" +
	"strategyId\x12!\n" +
	"\flive_enabled\x18\x02 \x01(\bR\vliveEnabled\"f\n" +
	"\x17SetStrategyLiveResponse\x12K\n" +
	"\n" +
	"definition\x18\x01 \x01(\v2+.xstockstrat.analysis.v1.StrategyDefinitionR\n" +
	"definition\"\x83\x03\n" +
	"\x0fScreenCriterion\x12\x19\n" +
	"\bref_name\x18\x01 \x01(\tR\arefName\x127\n" +
	"\x04kind\x18\x02 \x01(\x0e2#.xstockstrat.analysis.v1.ScreenKindR\x04kind\x12\x1f\n" +
	"\vmetric_name\x18\x03 \x01(\tR\n" +
	"metricName\x12H\n" +
	"\tcomponent\x18\x04 \x01(\v2*.xstockstrat.analysis.v1.StrategyComponentR\tcomponent\x123\n" +
	"\x02op\x18\x05 \x01(\x0e2#.xstockstrat.analysis.v1.ComparatorR\x02op\x12\x1c\n" +
	"\tthreshold\x18\x06 \x01(\x01R\tthreshold\x12%\n" +
	"\x0ethreshold_high\x18\a \x01(\x01R\rthresholdHigh\x12\x16\n" +
	"\x06weight\x18\b \x01(\x01R\x06weight\x12\x1f\n" +
	"\vhard_filter\x18\t \x01(\bR\n" +
	"hardFilter\"\xf3\x06\n" +
	"\fScreenResult\x12\x16\n" +
	"\x06symbol\x18\x01 \x01(\tR\x06symbol\x12\x14\n" +
	"\x05score\x18\x02 \x01(\x01R\x05score\x12e\n" +
	"\x10criterion_scores\x18\x03 \x03(\v2:.xstockstrat.analysis.v1.ScreenResult.CriterionScoresEntryR\x0fcriterionScores\x12\x16\n" +
	"\x06passed\x18\x04 \x01(\bR\x06passed\x12C\n" +
	"\x06status\x18\x05 \x01(\x0e2+.xstockstrat.analysis.v1.ScreenResultStatusR\x06status\x126\n" +
	"\x03gap\x18\x06 \x01(\v2$.xstockstrat.analysis.v1.CoverageGapR\x03gap\x12\x0e\n" +
	"\x02pe\x18\a \x01(\x01R\x02pe\x12\x10\n" +
	"\x03rsi\x18\b \x01(\x01R\x03rsi\x12\x10\n" +
	"\x03atr\x18\t \x01(\x01R\x03atr\x12\x1d\n" +
	"\n" +
	"rev_growth\x18\n" +
	" \x01(\x01R\trevGrowth\x12\x12\n" +
	"\x04held\x18\v \x01(\bR\x04held\x12o\n" +
	"\x14criterion_raw_values\x18\f \x03(\v2=.xstockstrat.analysis.v1.ScreenResult.CriterionRawValuesEntryR\x12criterionRawValues\x12e\n" +
	"\x10criterion_passed\x18\r \x03(\v2:.xstockstrat.analysis.v1.ScreenResult.CriterionPassedEntryR\x0fcriterionPassed\x12+\n" +
	"\x11score_unavailable\x18\x0e \x01(\bR\x10scoreUnavailable\x1aB\n" +
	"\x14CriterionScoresEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\x01R\x05value:\x028\x01\x1aE\n" +
	"\x17CriterionRawValuesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\x01R\x05value:\x028\x01\x1aB\n" +
	"\x14CriterionPassedEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\bR\x05value:\x028\x01\"\x82\x03\n" +
	"\x14ScreenSymbolsRequest\x12\x18\n" +
	"\asymbols\x18\x01 \x03(\tR\asymbols\x12D\n" +
	"\bcriteria\x18\x02 \x03(\v2(.xstockstrat.analysis.v1.ScreenCriterionR\bcriteria\x12%\n" +
	"\x0esignal_sources\x18\x03 \x03(\tR\rsignalSources\x12#\n" +
	"\rsignal_weight\x18\x04 \x01(\x01R\fsignalWeight\x12)\n" +
	"\x10technical_weight\x18\x05 \x01(\x01R\x0ftechnicalWeight\x12%\n" +
	"\x0emin_conviction\x18\x06 \x01(\x01R\rminConviction\x12\x1d\n" +
	"\n" +
	"rank_limit\x18\a \x01(\x05R\trankLimit\x12M\n" +
	"\x11evaluation_window\x18\b \x01(\v2 .xstockstrat.common.v1.TimeRangeR\x10evaluationWindow\"\xa3\x01\n" +
	"\x15ScreenSymbolsResponse\x12?\n" +
	"\aresults\x18\x01 \x03(\v2%.xstockstrat.analysis.v1.ScreenResultR\aresults\x12I\n" +
	"\rcoverage_gaps\x18\x02 \x03(\v2$.xstockstrat.analysis.v1.CoverageGapR\fcoverageGaps\"e\n" +
	"\x1aRunFundamentalsScanRequest\x12\x14\n" +
	"\x05force\x18\x01 \x01(\bR\x05force\x12\x17\n" +
	"\adry_run\x18\x02 \x01(\bR\x06dryRun\x12\x18\n" +
	"\asymbols\x18\x03 \x03(\tR\asymbols\"\xa3\x02\n" +
	"\x17FundamentalsScanSummary\x12\x15\n" +
	"\x06run_id\x18\x01 \x01(\tR\x05runId\x12+\n" +
	"\x11symbols_processed\x18\x02 \x01(\x05R\x10symbolsProcessed\x12'\n" +
	"\x0fsignals_emitted\x18\x03 \x01(\x05R\x0esignalsEmitted\x12\x1f\n" +
	"\vcalls_spent\x18\x04 \x01(\x05R\n" +
	"callsSpent\x12%\n" +
	"\x0edeferred_count\x18\x05 \x01(\x05R\rdeferredCount\x12\x16\n" +
	"\x06status\x18\x06 \x01(\tR\x06status\x12;\n" +
	"\vfinished_at\x18\a \x01(\v2\x1a.google.protobuf.TimestampR\n" +
	"finishedAt\"\xfc\x06\n" +
	"\vOpportunity\x12\x16\n" +
	"\x06symbol\x18\x01 \x01(\tR\x06symbol\x12E\n" +
	"\x06action\x18\x02 \x01(\x0e2-.xstockstrat.analysis.v1.OpportunityActionTagR\x06action\x12\x1e\n" +
	"\n" +
	"conviction\x18\x03 \x01(\x01R\n" +
	"conviction\x12-\n" +
	"\x12passing_conditions\x18\x04 \x01(\x05R\x11passingConditions\x12)\n" +
	"\x10total_conditions\x18\x05 \x01(\x05R\x0ftotalConditions\x12\x16\n" +
	"\x06thesis\x18\x06 \x01(\tR\x06thesis\x12\x1f\n" +
	"\vstrategy_id\x18\a \x01(\tR\n" +
	"strategyId\x12\x16\n" +
	"\x06source\x18\b \x01(\tR\x06source\x12;\n" +
	"\vvalid_until\x18\t \x01(\v2\x1a.google.protobuf.TimestampR\n" +
	"validUntil\x12'\n" +
	"\x0fopportunity_key\x18\n" +
	" \x01(\tR\x0eopportunityKey\x12\x1e\n" +
	"\n" +
	"provenance\x18\v \x03(\tR\n" +
	"provenance\x12\x14\n" +
	"\x05muted\x18\f \x01(\bR\x05muted\x12\"\n" +
	"\n" +
	"live_price\x18\r \x01(\x01H\x00R\tlivePrice\x88\x01\x01\x12\"\n" +
	"\n" +
	"change_pct\x18\x0e \x01(\x01H\x01R\tchangePct\x88\x01\x01\x12&\n" +
	"\ftarget_price\x18\x0f \x01(\x01H\x02R\vtargetPrice\x88\x01\x01\x12\"\n" +
	"\n" +
	"stop_price\x18\x10 \x01(\x01H\x03R\tstopPrice\x88\x01\x01\x12E\n" +
	"\tsparkline\x18\x11 \x03(\v2'.xstockstrat.analysis.v1.SparklinePointR\tsparkline\x12F\n" +
	"\n" +
	"conditions\x18\x12 \x03(\v2&.xstockstrat.analysis.v1.ConditionEvalR\n" +
	"conditions\x120\n" +
	"\x11signal_confidence\x18\x13 \x01(\x01H\x04R\x10signalConfidence\x88\x01\x01B\r\n" +
	"\v_live_priceB\r\n" +
	"\v_change_pctB\x0f\n" +
	"\r_target_priceB\r\n" +
	"\v_stop_priceB\x14\n" +
	"\x12_signal_confidence\"5\n" +
	"\x0eSparklinePoint\x12\x19\n" +
	"\x05close\x18\x01 \x01(\x01H\x00R\x05close\x88\x01\x01B\b\n" +
	"\x06_close\"\xe8\x01\n" +
	"\rConditionEval\x12\x19\n" +
	"\bref_name\x18\x01 \x01(\tR\arefName\x12\x1b\n" +
	"\tlhs_value\x18\x02 \x01(\x01R\blhsValue\x12\x1c\n" +
	"\tthreshold\x18\x03 \x01(\x01R\tthreshold\x12\x0e\n" +
	"\x02fn\x18\x04 \x01(\tR\x02fn\x12=\n" +
	"\x05state\x18\x05 \x01(\x0e2'.xstockstrat.analysis.v1.ConditionStateR\x05state\x122\n" +
	"\x15distance_to_threshold\x18\x06 \x01(\x01R\x13distanceToThreshold\"\xeb\x01\n" +
	"\x0fSymbolReadiness\x12\x16\n" +
	"\x06symbol\x18\x01 \x01(\tR\x06symbol\x12\x1e\n" +
	"\n" +
	"conviction\x18\x02 \x01(\x01R\n" +
	"conviction\x12-\n" +
	"\x12passing_conditions\x18\x03 \x01(\x05R\x11passingConditions\x12)\n" +
	"\x10total_conditions\x18\x04 \x01(\x05R\x0ftotalConditions\x12F\n" +
	"\n" +
	"conditions\x18\x05 \x03(\v2&.xstockstrat.analysis.v1.ConditionEvalR\n" +
	"conditions\"\xf9\x01\n" +
	"\x11StrategyAnalytics\x12\x1f\n" +
	"\vstrategy_id\x18\x01 \x01(\tR\n" +
	"strategyId\x12\x1e\n" +
	"\n" +
	"expectancy\x18\x02 \x01(\x01R\n" +
	"expectancy\x12(\n" +
	"\x10blended_hit_rate\x18\x03 \x01(\x01R\x0eblendedHitRate\x12!\n" +
	"\fmax_drawdown\x18\x04 \x01(\x01R\vmaxDrawdown\x12\x1f\n" +
	"\vsignals_30d\x18\x05 \x01(\x05R\n" +
	"signals30d\x12\x14\n" +
	"\x05taken\x18\x06 \x01(\x05R\x05taken\x12\x1f\n" +
	"\vqueue_share\x18\a \x01(\x01R\n" +
	"queueShare\"y\n" +
	"\x18ListOpportunitiesRequest\x126\n" +
	"\x04page\x18\x01 \x01(\v2\".xstockstrat.common.v1.PageRequestR\x04page\x12%\n" +
	"\x0emin_conviction\x18\x02 \x01(\x01R\rminConviction\"\xa0\x01\n" +
	"\x19ListOpportunitiesResponse\x12J\n" +
	"\ropportunities\x18\x01 \x03(\v2$.xstockstrat.analysis.v1.OpportunityR\ropportunities\x127\n" +
	"\x04page\x18\x02 \x01(\v2#.xstockstrat.common.v1.PageResponseR\x04page\"\x91\x01\n" +
	"\x18EvaluateReadinessRequest\x12\x1f\n" +
	"\vstrategy_id\x18\x01 \x01(\tR\n" +
	"strategyId\x12\x18\n" +
	"\asymbols\x18\x02 \x03(\tR\asymbols\x12:\n" +
	"\x04rule\x18\x03 \x01(\x0e2&.xstockstrat.analysis.v1.ReadinessRuleR\x04rule\"\xa0\x01\n" +
	"\x19EvaluateReadinessResponse\x12F\n" +
	"\treadiness\x18\x01 \x03(\v2(.xstockstrat.analysis.v1.SymbolReadinessR\treadiness\x12;\n" +
	"\vcomputed_at\x18\x02 \x01(\v2\x1a.google.protobuf.TimestampR\n" +
	"computedAt\"\xc9\x01\n" +
	"\x1bSetOpportunityActionRequest\x12'\n" +
	"\x0fopportunity_key\x18\x01 \x01(\tR\x0eopportunityKey\x12B\n" +
	"\x06action\x18\x02 \x01(\x0e2*.xstockstrat.analysis.v1.OpportunityActionR\x06action\x12=\n" +
	"\fsnooze_until\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\vsnoozeUntil\"\x1e\n" +
	"\x1cSetOpportunityActionResponse\">\n" +
	"\x1bGetStrategyAnalyticsRequest\x12\x1f\n" +
	"\vstrategy_id\x18\x01 \x01(\tR\n" +
	"strategyId\"\x9e\x01\n" +
	"\x19GetIndicatorSeriesRequest\x12\x1f\n" +
	"\vstrategy_id\x18\x01 \x01(\tR\n" +
	"strategyId\x12\x16\n" +
	"\x06symbol\x18\x02 \x01(\tR\x06symbol\x12\x16\n" +
	"\x06closes\x18\x03 \x03(\x01R\x06closes\x120\n" +
	"\x05times\x18\x04 \x03(\v2\x1a.google.protobuf.TimestampR\x05times\"\x98\x01\n" +
	"\x1aGetIndicatorSeriesResponse\x120\n" +
	"\x05times\x18\x01 \x03(\v2\x1a.google.protobuf.TimestampR\x05times\x12H\n" +
	"\n" +
	"components\x18\x02 \x03(\v2(.xstockstrat.analysis.v1.ComponentSeriesR\n" +
	"components\"\xbc\x01\n" +
	"\x0fComponentSeries\x12\x19\n" +
	"\bref_name\x18\x01 \x01(\tR\arefName\x12:\n" +
	"\x04kind\x18\x02 \x01(\x0e2&.xstockstrat.analysis.v1.ComponentKindR\x04kind\x12<\n" +
	"\x06series\x18\x03 \x03(\v2$.xstockstrat.analysis.v1.NamedSeriesR\x06series\x12\x14\n" +
	"\x05error\x18\x04 \x01(\tR\x05error\"b\n" +
	"\vNamedSeries\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12?\n" +
	"\x06values\x18\x02 \x03(\v2'.xstockstrat.analysis.v1.IndicatorValueR\x06values\"5\n" +
	"\x0eIndicatorValue\x12\x19\n" +
	"\x05value\x18\x01 \x01(\x01H\x00R\x05value\x88\x01\x01B\b\n" +
	"\x06_value\"O\n" +
	"\vSignalEntry\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12\x14\n" +
	"\x05value\x18\x02 \x01(\x01R\x05value\x12\x16\n" +
	"\x06source\x18\x03 \x01(\tR\x06source\"\xcd\x04\n" +
	"\rOrderSnapshot\x12\x19\n" +
	"\border_id\x18\x01 \x01(\tR\aorderId\x12\x1f\n" +
	"\vposition_id\x18\x02 \x01(\tR\n" +
	"positionId\x12\x16\n" +
	"\x06symbol\x18\x03 \x01(\tR\x06symbol\x12I\n" +
	"\n" +
	"event_type\x18\x04 \x01(\x0e2*.xstockstrat.analysis.v1.SnapshotEventTypeR\teventType\x125\n" +
	"\bevent_ts\x18\x05 \x01(\v2\x1a.google.protobuf.TimestampR\aeventTs\x12\x12\n" +
	"\x04side\x18\x06 \x01(\tR\x04side\x12\x1a\n" +
	"\bquantity\x18\a \x01(\x01R\bquantity\x12\x14\n" +
	"\x05price\x18\b \x01(\x01R\x05price\x124\n" +
	"\tohlcv_bar\x18\t \x01(\v2\x17.google.protobuf.StructR\bohlcvBar\x12f\n" +
	"\x10indicator_values\x18\n" +
	" \x03(\v2;.xstockstrat.analysis.v1.OrderSnapshot.IndicatorValuesEntryR\x0findicatorValues\x12>\n" +
	"\asignals\x18\v \x03(\v2$.xstockstrat.analysis.v1.SignalEntryR\asignals\x1aB\n" +
	"\x14IndicatorValuesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\x01R\x05value:\x028\x01\"\x94\x02\n" +
	"\x10PnLPatternFactor\x12\x1f\n" +
	"\vfactor_name\x18\x01 \x01(\tR\n" +
	"factorName\x12D\n" +
	"\vfactor_type\x18\x02 \x01(\x0e2#.xstockstrat.analysis.v1.FactorTypeR\n" +
	"factorType\x12&\n" +
	"\x0fvalue_range_low\x18\x03 \x01(\x01R\rvalueRangeLow\x12(\n" +
	"\x10value_range_high\x18\x04 \x01(\x01R\x0evalueRangeHigh\x12!\n" +
	"\fsample_count\x18\x05 \x01(\x05R\vsampleCount\x12$\n" +
	"\x0eavg_pnl_impact\x18\x06 \x01(\x01R\favgPnlImpact\"\xce\x01\n" +
	"\x17QueryPnLPatternsRequest\x12\x16\n" +
	"\x06symbol\x18\x01 \x01(\tR\x06symbol\x12\x1f\n" +
	"\vstrategy_id\x18\x02 \x01(\tR\n" +
	"strategyId\x123\n" +
	"\afrom_ts\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\x06fromTs\x12/\n" +
	"\x05to_ts\x18\x04 \x01(\v2\x1a.google.protobuf.TimestampR\x04toTs\x12\x14\n" +
	"\x05limit\x18\x05 \x01(\x05R\x05limit\"\xc6\x01\n" +
	"\x18QueryPnLPatternsResponse\x12T\n" +
	"\x10positive_factors\x18\x01 \x03(\v2).xstockstrat.analysis.v1.PnLPatternFactorR\x0fpositiveFactors\x12T\n" +
	"\x10negative_factors\x18\x02 \x03(\v2).xstockstrat.analysis.v1.PnLPatternFactorR\x0fnegativeFactors\"\x94\x01\n" +
	"\x15GetAttributionRequest\x120\n" +
	"\x05start\x18\x01 \x01(\v2\x1a.google.protobuf.TimestampR\x05start\x12,\n" +
	"\x03end\x18\x02 \x01(\v2\x1a.google.protobuf.TimestampR\x03end\x12\x1b\n" +
	"\tsource_id\x18\x03 \x01(\tR\bsourceId\"\xe6\x01\n" +
	"\x11SourceAttribution\x12\x1b\n" +
	"\tsource_id\x18\x01 \x01(\tR\bsourceId\x12\x1f\n" +
	"\vsource_name\x18\x02 \x01(\tR\n" +
	"sourceName\x12\x1f\n" +
	"\vtrade_count\x18\x03 \x01(\x01R\n" +
	"tradeCount\x12\x1b\n" +
	"\twin_count\x18\x04 \x01(\x01R\bwinCount\x12\x19\n" +
	"\bwin_rate\x18\x05 \x01(\x01R\awinRate\x12\x1d\n" +
	"\n" +
	"avg_return\x18\x06 \x01(\x01R\tavgReturn\x12\x1b\n" +
	"\ttotal_pnl\x18\a \x01(\x01R\btotalPnl\"h\n" +
	"\x16GetAttributionResponse\x12N\n" +
	"\fattributions\x18\x01 \x03(\v2*.xstockstrat.analysis.v1.SourceAttributionR\fattributions*p\n" +
	"\x0eBacktestStatus\x12\x1f\n" +
	"\x1bBACKTEST_STATUS_UNSPECIFIED\x10\x00\x12\x16\n" +
	"\x12BACKTEST_STATUS_OK\x10\x01\x12%\n" +
	"!BACKTEST_STATUS_INSUFFICIENT_DATA\x10\x02*\\\n" +
	"\n" +
	"SizingMode\x12\x1b\n" +
	"\x17SIZING_MODE_UNSPECIFIED\x10\x00\x12\x16\n" +
	"\x12SIZING_MODE_LEGACY\x10\x01\x12\x19\n" +
	"\x15SIZING_MODE_PORTFOLIO\x10\x02*d\n" +
	"\tFillModel\x12\x1a\n" +
	"\x16FILL_MODEL_UNSPECIFIED\x10\x00\x12\x1d\n" +
	"\x19FILL_MODEL_SAME_BAR_CLOSE\x10\x01\x12\x1c\n" +
	"\x18FILL_MODEL_NEXT_BAR_OPEN\x10\x02*\xa7\x01\n" +
	"\tBarAction\x12\x1a\n" +
	"\x16BAR_ACTION_UNSPECIFIED\x10\x00\x12\x15\n" +
	"\x11BAR_ACTION_WARMUP\x10\x01\x12\x18\n" +
	"\x14BAR_ACTION_HOLD_FLAT\x10\x02\x12\x19\n" +
	"\x15BAR_ACTION_ENTER_LONG\x10\x03\x12\x18\n" +
	"\x14BAR_ACTION_EXIT_LONG\x10\x04\x12\x18\n" +
	"\x14BAR_ACTION_HOLD_LONG\x10\x05*\xcc\x01\n" +
	"\rNoTradeReason\x12\x1f\n" +
	"\x1bNO_TRADE_REASON_UNSPECIFIED\x10\x00\x12'\n" +
	"#NO_TRADE_REASON_ENTIRE_RANGE_WARMUP\x10\x01\x12$\n" +
	" NO_TRADE_REASON_ENTRY_NEVER_TRUE\x10\x02\x12(\n" +
	"$NO_TRADE_REASON_INSUFFICIENT_CAPITAL\x10\x03\x12!\n" +
	"\x1dNO_TRADE_REASON_FORMULA_ERROR\x10\x04*x\n" +
	"\rComponentKind\x12\x1e\n" +
	"\x1aCOMPONENT_KIND_UNSPECIFIED\x10\x00\x12$\n" +
	" COMPONENT_KIND_BUILTIN_INDICATOR\x10\x01\x12!\n" +
	"\x1dCOMPONENT_KIND_CUSTOM_FORMULA\x10\x02*\xbd\x01\n" +
	"\x11StrategyOperation\x12\"\n" +
	"\x1eSTRATEGY_OPERATION_UNSPECIFIED\x10\x00\x12\x1f\n" +
	"\x1bSTRATEGY_OPERATION_REGISTER\x10\x01\x12\x1d\n" +
	"\x19STRATEGY_OPERATION_UPDATE\x10\x02\x12!\n" +
	"\x1dSTRATEGY_OPERATION_DEACTIVATE\x10\x03\x12!\n" +
	"\x1dSTRATEGY_OPERATION_REACTIVATE\x10\x04*\x8e\x01\n" +
	"\n" +
	"Comparator\x12\x1a\n" +
	"\x16COMPARATOR_UNSPECIFIED\x10\x00\x12\x11\n" +
	"\rCOMPARATOR_LT\x10\x01\x12\x12\n" +
	"\x0eCOMPARATOR_LTE\x10\x02\x12\x11\n" +
	"\rCOMPARATOR_GT\x10\x03\x12\x12\n" +
	"\x0eCOMPARATOR_GTE\x10\x04\x12\x16\n" +
	"\x12COMPARATOR_BETWEEN\x10\x05*\xa6\x01\n" +
	"\n" +
	"ScreenKind\x12\x1b\n" +
	"\x17SCREEN_KIND_UNSPECIFIED\x10\x00\x12\x1b\n" +
	"\x17SCREEN_KIND_FUNDAMENTAL\x10\x01\x12!\n" +
	"\x1dSCREEN_KIND_TECHNICAL_FORMULA\x10\x02\x12#\n" +
	"\x1fSCREEN_KIND_TECHNICAL_INDICATOR\x10\x03\x12\x16\n" +
	"\x12SCREEN_KIND_SIGNAL\x10\x04*\x83\x01\n" +
	"\x12ScreenResultStatus\x12$\n" +
	" SCREEN_RESULT_STATUS_UNSPECIFIED\x10\x00\x12\x1b\n" +
	"\x17SCREEN_RESULT_STATUS_OK\x10\x01\x12*\n" +
	"&SCREEN_RESULT_STATUS_INSUFFICIENT_DATA\x10\x02*\xa3\x01\n" +
	"\x14OpportunityActionTag\x12&\n" +
	"\"OPPORTUNITY_ACTION_TAG_UNSPECIFIED\x10\x00\x12 \n" +
	"\x1cOPPORTUNITY_ACTION_TAG_ENTER\x10\x01\x12\x1e\n" +
	"\x1aOPPORTUNITY_ACTION_TAG_ADD\x10\x02\x12!\n" +
	"\x1dOPPORTUNITY_ACTION_TAG_REDUCE\x10\x03*\x7f\n" +
	"\x0eConditionState\x12\x1f\n" +
	"\x1bCONDITION_STATE_UNSPECIFIED\x10\x00\x12\x18\n" +
	"\x14CONDITION_STATE_PASS\x10\x01\x12\x18\n" +
	"\x14CONDITION_STATE_SOFT\x10\x02\x12\x18\n" +
	"\x14CONDITION_STATE_FAIL\x10\x03*b\n" +
	"\rReadinessRule\x12\x1e\n" +
	"\x1aREADINESS_RULE_UNSPECIFIED\x10\x00\x12\x18\n" +
	"\x14READINESS_RULE_ENTRY\x10\x01\x12\x17\n" +
	"\x13READINESS_RULE_EXIT\x10\x02*\x93\x01\n" +
	"\x11OpportunityAction\x12\"\n" +
	"\x1eOPPORTUNITY_ACTION_UNSPECIFIED\x10\x00\x12\x1d\n" +
	"\x19OPPORTUNITY_ACTION_SNOOZE\x10\x01\x12\x1e\n" +
	"\x1aOPPORTUNITY_ACTION_DISMISS\x10\x02\x12\x1b\n" +
	"\x17OPPORTUNITY_ACTION_TAKE\x10\x03*\xde\x01\n" +
	"\x11SnapshotEventType\x12#\n" +
	"\x1fSNAPSHOT_EVENT_TYPE_UNSPECIFIED\x10\x00\x12%\n" +
	"!SNAPSHOT_EVENT_TYPE_ORDER_CREATED\x10\x01\x12$\n" +
	" SNAPSHOT_EVENT_TYPE_ORDER_FILLED\x10\x02\x12.\n" +
	"*SNAPSHOT_EVENT_TYPE_ORDER_PARTIALLY_FILLED\x10\x03\x12'\n" +
	"#SNAPSHOT_EVENT_TYPE_ORDER_CANCELLED\x10\x04*\\\n" +
	"\n" +
	"FactorType\x12\x1b\n" +
	"\x17FACTOR_TYPE_UNSPECIFIED\x10\x00\x12\x19\n" +
	"\x15FACTOR_TYPE_INDICATOR\x10\x01\x12\x16\n" +
	"\x12FACTOR_TYPE_SIGNAL\x10\x022\xc5\x11\n" +
	"\x0fAnalysisService\x12c\n" +
	"\vRunBacktest\x12+.xstockstrat.analysis.v1.RunBacktestRequest\x1a'.xstockstrat.analysis.v1.BacktestResult\x12f\n" +
	"\rScoreStrategy\x12-.xstockstrat.analysis.v1.ScoreStrategyRequest\x1a&.xstockstrat.analysis.v1.StrategyScore\x12q\n" +
	"\x0eListStrategies\x12..xstockstrat.analysis.v1.ListStrategiesRequest\x1a/.xstockstrat.analysis.v1.ListStrategiesResponse\x12o\n" +
	"\x11GetStrategyReport\x121.xstockstrat.analysis.v1.GetStrategyReportRequest\x1a'.xstockstrat.analysis.v1.StrategyReport\x12n\n" +
	"\rListBacktests\x12-.xstockstrat.analysis.v1.ListBacktestsRequest\x1a..xstockstrat.analysis.v1.ListBacktestsResponse\x12c\n" +
	"\vGetBacktest\x12+.xstockstrat.analysis.v1.GetBacktestRequest\x1a'.xstockstrat.analysis.v1.BacktestResult\x12m\n" +
	"\x0eManageStrategy\x12..xstockstrat.analysis.v1.ManageStrategyRequest\x1a+.xstockstrat.analysis.v1.StrategyDefinition\x12g\n" +
	"\vGetStrategy\x12+.xstockstrat.analysis.v1.GetStrategyRequest\x1a+.xstockstrat.analysis.v1.StrategyDefinition\x12\x8c\x01\n" +
	"\x17ListStrategyDefinitions\x127.xstockstrat.analysis.v1.ListStrategyDefinitionsRequest\x1a8.xstockstrat.analysis.v1.ListStrategyDefinitionsResponse\x12t\n" +
	"\x0fSetStrategyLive\x12/.xstockstrat.analysis.v1.SetStrategyLiveRequest\x1a0.xstockstrat.analysis.v1.SetStrategyLiveResponse\x12n\n" +
	"\rScreenSymbols\x12-.xstockstrat.analysis.v1.ScreenSymbolsRequest\x1a..xstockstrat.analysis.v1.ScreenSymbolsResponse\x12|\n" +
	"\x13RunFundamentalsScan\x123.xstockstrat.analysis.v1.RunFundamentalsScanRequest\x1a0.xstockstrat.analysis.v1.FundamentalsScanSummary\x12z\n" +
	"\x11ListOpportunities\x121.xstockstrat.analysis.v1.ListOpportunitiesRequest\x1a2.xstockstrat.analysis.v1.ListOpportunitiesResponse\x12z\n" +
	"\x11EvaluateReadiness\x121.xstockstrat.analysis.v1.EvaluateReadinessRequest\x1a2.xstockstrat.analysis.v1.EvaluateReadinessResponse\x12\x83\x01\n" +
	"\x14SetOpportunityAction\x124.xstockstrat.analysis.v1.SetOpportunityActionRequest\x1a5.xstockstrat.analysis.v1.SetOpportunityActionResponse\x12x\n" +
	"\x14GetStrategyAnalytics\x124.xstockstrat.analysis.v1.GetStrategyAnalyticsRequest\x1a*.xstockstrat.analysis.v1.StrategyAnalytics\x12}\n" +
	"\x12GetIndicatorSeries\x122.xstockstrat.analysis.v1.GetIndicatorSeriesRequest\x1a3.xstockstrat.analysis.v1.GetIndicatorSeriesResponse\x12w\n" +
	"\x10QueryPnLPatterns\x120.xstockstrat.analysis.v1.QueryPnLPatternsRequest\x1a1.xstockstrat.analysis.v1.QueryPnLPatternsResponse\x12q\n" +
	"\x0eGetAttribution\x12..xstockstrat.analysis.v1.GetAttributionRequest\x1a/.xstockstrat.analysis.v1.GetAttributionResponseB@Z>github.com/xstockstrat/contracts/gen/go/analysis/v1;analysisv1b\x06proto3"

var (
	file_analysis_v1_analysis_proto_rawDescOnce sync.Once
	file_analysis_v1_analysis_proto_rawDescData []byte
)

func file_analysis_v1_analysis_proto_rawDescGZIP() []byte {
	file_analysis_v1_analysis_proto_rawDescOnce.Do(func() {
		file_analysis_v1_analysis_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_analysis_v1_analysis_proto_rawDesc), len(file_analysis_v1_analysis_proto_rawDesc)))
	})
	return file_analysis_v1_analysis_proto_rawDescData
}

var file_analysis_v1_analysis_proto_enumTypes = make([]protoimpl.EnumInfo, 16)
var file_analysis_v1_analysis_proto_msgTypes = make([]protoimpl.MessageInfo, 64)
var file_analysis_v1_analysis_proto_goTypes = []any{
	(BacktestStatus)(0),                     // 0: xstockstrat.analysis.v1.BacktestStatus
	(SizingMode)(0),                         // 1: xstockstrat.analysis.v1.SizingMode
	(FillModel)(0),                          // 2: xstockstrat.analysis.v1.FillModel
	(BarAction)(0),                          // 3: xstockstrat.analysis.v1.BarAction
	(NoTradeReason)(0),                      // 4: xstockstrat.analysis.v1.NoTradeReason
	(ComponentKind)(0),                      // 5: xstockstrat.analysis.v1.ComponentKind
	(StrategyOperation)(0),                  // 6: xstockstrat.analysis.v1.StrategyOperation
	(Comparator)(0),                         // 7: xstockstrat.analysis.v1.Comparator
	(ScreenKind)(0),                         // 8: xstockstrat.analysis.v1.ScreenKind
	(ScreenResultStatus)(0),                 // 9: xstockstrat.analysis.v1.ScreenResultStatus
	(OpportunityActionTag)(0),               // 10: xstockstrat.analysis.v1.OpportunityActionTag
	(ConditionState)(0),                     // 11: xstockstrat.analysis.v1.ConditionState
	(ReadinessRule)(0),                      // 12: xstockstrat.analysis.v1.ReadinessRule
	(OpportunityAction)(0),                  // 13: xstockstrat.analysis.v1.OpportunityAction
	(SnapshotEventType)(0),                  // 14: xstockstrat.analysis.v1.SnapshotEventType
	(FactorType)(0),                         // 15: xstockstrat.analysis.v1.FactorType
	(*RunBacktestRequest)(nil),              // 16: xstockstrat.analysis.v1.RunBacktestRequest
	(*CoverageGap)(nil),                     // 17: xstockstrat.analysis.v1.CoverageGap
	(*BacktestResult)(nil),                  // 18: xstockstrat.analysis.v1.BacktestResult
	(*PortfolioCapitalSkip)(nil),            // 19: xstockstrat.analysis.v1.PortfolioCapitalSkip
	(*EquityPoint)(nil),                     // 20: xstockstrat.analysis.v1.EquityPoint
	(*TradeRecord)(nil),                     // 21: xstockstrat.analysis.v1.TradeRecord
	(*BarDiagnostic)(nil),                   // 22: xstockstrat.analysis.v1.BarDiagnostic
	(*SymbolDiagnostics)(nil),               // 23: xstockstrat.analysis.v1.SymbolDiagnostics
	(*ScoreStrategyRequest)(nil),            // 24: xstockstrat.analysis.v1.ScoreStrategyRequest
	(*StrategyScore)(nil),                   // 25: xstockstrat.analysis.v1.StrategyScore
	(*StrategyReport)(nil),                  // 26: xstockstrat.analysis.v1.StrategyReport
	(*ListBacktestsRequest)(nil),            // 27: xstockstrat.analysis.v1.ListBacktestsRequest
	(*BacktestRunSummary)(nil),              // 28: xstockstrat.analysis.v1.BacktestRunSummary
	(*ListBacktestsResponse)(nil),           // 29: xstockstrat.analysis.v1.ListBacktestsResponse
	(*GetBacktestRequest)(nil),              // 30: xstockstrat.analysis.v1.GetBacktestRequest
	(*ListStrategiesRequest)(nil),           // 31: xstockstrat.analysis.v1.ListStrategiesRequest
	(*ListStrategiesResponse)(nil),          // 32: xstockstrat.analysis.v1.ListStrategiesResponse
	(*GetStrategyReportRequest)(nil),        // 33: xstockstrat.analysis.v1.GetStrategyReportRequest
	(*StrategyComponent)(nil),               // 34: xstockstrat.analysis.v1.StrategyComponent
	(*StrategyDefinition)(nil),              // 35: xstockstrat.analysis.v1.StrategyDefinition
	(*ManageStrategyRequest)(nil),           // 36: xstockstrat.analysis.v1.ManageStrategyRequest
	(*GetStrategyRequest)(nil),              // 37: xstockstrat.analysis.v1.GetStrategyRequest
	(*ListStrategyDefinitionsRequest)(nil),  // 38: xstockstrat.analysis.v1.ListStrategyDefinitionsRequest
	(*ListStrategyDefinitionsResponse)(nil), // 39: xstockstrat.analysis.v1.ListStrategyDefinitionsResponse
	(*SetStrategyLiveRequest)(nil),          // 40: xstockstrat.analysis.v1.SetStrategyLiveRequest
	(*SetStrategyLiveResponse)(nil),         // 41: xstockstrat.analysis.v1.SetStrategyLiveResponse
	(*ScreenCriterion)(nil),                 // 42: xstockstrat.analysis.v1.ScreenCriterion
	(*ScreenResult)(nil),                    // 43: xstockstrat.analysis.v1.ScreenResult
	(*ScreenSymbolsRequest)(nil),            // 44: xstockstrat.analysis.v1.ScreenSymbolsRequest
	(*ScreenSymbolsResponse)(nil),           // 45: xstockstrat.analysis.v1.ScreenSymbolsResponse
	(*RunFundamentalsScanRequest)(nil),      // 46: xstockstrat.analysis.v1.RunFundamentalsScanRequest
	(*FundamentalsScanSummary)(nil),         // 47: xstockstrat.analysis.v1.FundamentalsScanSummary
	(*Opportunity)(nil),                     // 48: xstockstrat.analysis.v1.Opportunity
	(*SparklinePoint)(nil),                  // 49: xstockstrat.analysis.v1.SparklinePoint
	(*ConditionEval)(nil),                   // 50: xstockstrat.analysis.v1.ConditionEval
	(*SymbolReadiness)(nil),                 // 51: xstockstrat.analysis.v1.SymbolReadiness
	(*StrategyAnalytics)(nil),               // 52: xstockstrat.analysis.v1.StrategyAnalytics
	(*ListOpportunitiesRequest)(nil),        // 53: xstockstrat.analysis.v1.ListOpportunitiesRequest
	(*ListOpportunitiesResponse)(nil),       // 54: xstockstrat.analysis.v1.ListOpportunitiesResponse
	(*EvaluateReadinessRequest)(nil),        // 55: xstockstrat.analysis.v1.EvaluateReadinessRequest
	(*EvaluateReadinessResponse)(nil),       // 56: xstockstrat.analysis.v1.EvaluateReadinessResponse
	(*SetOpportunityActionRequest)(nil),     // 57: xstockstrat.analysis.v1.SetOpportunityActionRequest
	(*SetOpportunityActionResponse)(nil),    // 58: xstockstrat.analysis.v1.SetOpportunityActionResponse
	(*GetStrategyAnalyticsRequest)(nil),     // 59: xstockstrat.analysis.v1.GetStrategyAnalyticsRequest
	(*GetIndicatorSeriesRequest)(nil),       // 60: xstockstrat.analysis.v1.GetIndicatorSeriesRequest
	(*GetIndicatorSeriesResponse)(nil),      // 61: xstockstrat.analysis.v1.GetIndicatorSeriesResponse
	(*ComponentSeries)(nil),                 // 62: xstockstrat.analysis.v1.ComponentSeries
	(*NamedSeries)(nil),                     // 63: xstockstrat.analysis.v1.NamedSeries
	(*IndicatorValue)(nil),                  // 64: xstockstrat.analysis.v1.IndicatorValue
	(*SignalEntry)(nil),                     // 65: xstockstrat.analysis.v1.SignalEntry
	(*OrderSnapshot)(nil),                   // 66: xstockstrat.analysis.v1.OrderSnapshot
	(*PnLPatternFactor)(nil),                // 67: xstockstrat.analysis.v1.PnLPatternFactor
	(*QueryPnLPatternsRequest)(nil),         // 68: xstockstrat.analysis.v1.QueryPnLPatternsRequest
	(*QueryPnLPatternsResponse)(nil),        // 69: xstockstrat.analysis.v1.QueryPnLPatternsResponse
	(*GetAttributionRequest)(nil),           // 70: xstockstrat.analysis.v1.GetAttributionRequest
	(*SourceAttribution)(nil),               // 71: xstockstrat.analysis.v1.SourceAttribution
	(*GetAttributionResponse)(nil),          // 72: xstockstrat.analysis.v1.GetAttributionResponse
	nil,                                     // 73: xstockstrat.analysis.v1.BarDiagnostic.IndicatorsEntry
	nil,                                     // 74: xstockstrat.analysis.v1.StrategyScore.ComponentScoresEntry
	nil,                                     // 75: xstockstrat.analysis.v1.StrategyComponent.ParamsEntry
	nil,                                     // 76: xstockstrat.analysis.v1.ScreenResult.CriterionScoresEntry
	nil,                                     // 77: xstockstrat.analysis.v1.ScreenResult.CriterionRawValuesEntry
	nil,                                     // 78: xstockstrat.analysis.v1.ScreenResult.CriterionPassedEntry
	nil,                                     // 79: xstockstrat.analysis.v1.OrderSnapshot.IndicatorValuesEntry
	(*v1.TimeRange)(nil),                    // 80: xstockstrat.common.v1.TimeRange
	(*structpb.Struct)(nil),                 // 81: google.protobuf.Struct
	(v1.Timeframe)(0),                       // 82: xstockstrat.common.v1.Timeframe
	(*timestamppb.Timestamp)(nil),           // 83: google.protobuf.Timestamp
	(*v1.PageRequest)(nil),                  // 84: xstockstrat.common.v1.PageRequest
	(*v1.PageResponse)(nil),                 // 85: xstockstrat.common.v1.PageResponse
	(*fieldmaskpb.FieldMask)(nil),           // 86: google.protobuf.FieldMask
}
var file_analysis_v1_analysis_proto_depIdxs = []int32{
	80,  // 0: xstockstrat.analysis.v1.RunBacktestRequest.range:type_name -> xstockstrat.common.v1.TimeRange
	81,  // 1: xstockstrat.analysis.v1.RunBacktestRequest.strategy_params:type_name -> google.protobuf.Struct
	35,  // 2: xstockstrat.analysis.v1.RunBacktestRequest.inline_definition:type_name -> xstockstrat.analysis.v1.StrategyDefinition
	1,   // 3: xstockstrat.analysis.v1.RunBacktestRequest.sizing_mode:type_name -> xstockstrat.analysis.v1.SizingMode
	2,   // 4: xstockstrat.analysis.v1.RunBacktestRequest.fill_model:type_name -> xstockstrat.analysis.v1.FillModel
	82,  // 5: xstockstrat.analysis.v1.CoverageGap.timeframe:type_name -> xstockstrat.common.v1.Timeframe
	80,  // 6: xstockstrat.analysis.v1.CoverageGap.requested_range:type_name -> xstockstrat.common.v1.TimeRange
	80,  // 7: xstockstrat.analysis.v1.CoverageGap.gap:type_name -> xstockstrat.common.v1.TimeRange
	83,  // 8: xstockstrat.analysis.v1.BacktestResult.completed_at:type_name -> google.protobuf.Timestamp
	21,  // 9: xstockstrat.analysis.v1.BacktestResult.trades:type_name -> xstockstrat.analysis.v1.TradeRecord
	0,   // 10: xstockstrat.analysis.v1.BacktestResult.status:type_name -> xstockstrat.analysis.v1.BacktestStatus
	17,  // 11: xstockstrat.analysis.v1.BacktestResult.coverage_gaps:type_name -> xstockstrat.analysis.v1.CoverageGap
	23,  // 12: xstockstrat.analysis.v1.BacktestResult.diagnostics:type_name -> xstockstrat.analysis.v1.SymbolDiagnostics
	1,   // 13: xstockstrat.analysis.v1.BacktestResult.sizing_mode:type_name -> xstockstrat.analysis.v1.SizingMode
	19,  // 14: xstockstrat.analysis.v1.BacktestResult.capital_skips:type_name -> xstockstrat.analysis.v1.PortfolioCapitalSkip
	20,  // 15: xstockstrat.analysis.v1.BacktestResult.portfolio_equity_curve:type_name -> xstockstrat.analysis.v1.EquityPoint
	2,   // 16: xstockstrat.analysis.v1.BacktestResult.fill_model:type_name -> xstockstrat.analysis.v1.FillModel
	83,  // 17: xstockstrat.analysis.v1.PortfolioCapitalSkip.timestamp:type_name -> google.protobuf.Timestamp
	83,  // 18: xstockstrat.analysis.v1.EquityPoint.timestamp:type_name -> google.protobuf.Timestamp
	83,  // 19: xstockstrat.analysis.v1.TradeRecord.entry_time:type_name -> google.protobuf.Timestamp
	83,  // 20: xstockstrat.analysis.v1.TradeRecord.exit_time:type_name -> google.protobuf.Timestamp
	83,  // 21: xstockstrat.analysis.v1.BarDiagnostic.timestamp:type_name -> google.protobuf.Timestamp
	73,  // 22: xstockstrat.analysis.v1.BarDiagnostic.indicators:type_name -> xstockstrat.analysis.v1.BarDiagnostic.IndicatorsEntry
	3,   // 23: xstockstrat.analysis.v1.BarDiagnostic.action:type_name -> xstockstrat.analysis.v1.BarAction
	22,  // 24: xstockstrat.analysis.v1.SymbolDiagnostics.bars:type_name -> xstockstrat.analysis.v1.BarDiagnostic
	4,   // 25: xstockstrat.analysis.v1.SymbolDiagnostics.no_trade_reason:type_name -> xstockstrat.analysis.v1.NoTradeReason
	80,  // 26: xstockstrat.analysis.v1.ScoreStrategyRequest.range:type_name -> xstockstrat.common.v1.TimeRange
	74,  // 27: xstockstrat.analysis.v1.StrategyScore.component_scores:type_name -> xstockstrat.analysis.v1.StrategyScore.ComponentScoresEntry
	18,  // 28: xstockstrat.analysis.v1.StrategyReport.latest_backtest:type_name -> xstockstrat.analysis.v1.BacktestResult
	25,  // 29: xstockstrat.analysis.v1.StrategyReport.score:type_name -> xstockstrat.analysis.v1.StrategyScore
	81,  // 30: xstockstrat.analysis.v1.StrategyReport.metadata:type_name -> google.protobuf.Struct
	0,   // 31: xstockstrat.analysis.v1.BacktestRunSummary.status:type_name -> xstockstrat.analysis.v1.BacktestStatus
	83,  // 32: xstockstrat.analysis.v1.BacktestRunSummary.completed_at:type_name -> google.protobuf.Timestamp
	83,  // 33: xstockstrat.analysis.v1.BacktestRunSummary.range_start:type_name -> google.protobuf.Timestamp
	83,  // 34: xstockstrat.analysis.v1.BacktestRunSummary.range_end:type_name -> google.protobuf.Timestamp
	1,   // 35: xstockstrat.analysis.v1.BacktestRunSummary.sizing_mode:type_name -> xstockstrat.analysis.v1.SizingMode
	2,   // 36: xstockstrat.analysis.v1.BacktestRunSummary.fill_model:type_name -> xstockstrat.analysis.v1.FillModel
	28,  // 37: xstockstrat.analysis.v1.ListBacktestsResponse.runs:type_name -> xstockstrat.analysis.v1.BacktestRunSummary
	84,  // 38: xstockstrat.analysis.v1.ListStrategiesRequest.page:type_name -> xstockstrat.common.v1.PageRequest
	25,  // 39: xstockstrat.analysis.v1.ListStrategiesResponse.strategies:type_name -> xstockstrat.analysis.v1.StrategyScore
	85,  // 40: xstockstrat.analysis.v1.ListStrategiesResponse.page:type_name -> xstockstrat.common.v1.PageResponse
	5,   // 41: xstockstrat.analysis.v1.StrategyComponent.kind:type_name -> xstockstrat.analysis.v1.ComponentKind
	75,  // 42: xstockstrat.analysis.v1.StrategyComponent.params:type_name -> xstockstrat.analysis.v1.StrategyComponent.ParamsEntry
	34,  // 43: xstockstrat.analysis.v1.StrategyDefinition.components:type_name -> xstockstrat.analysis.v1.StrategyComponent
	81,  // 44: xstockstrat.analysis.v1.StrategyDefinition.signal_params:type_name -> google.protobuf.Struct
	6,   // 45: xstockstrat.analysis.v1.ManageStrategyRequest.operation:type_name -> xstockstrat.analysis.v1.StrategyOperation
	35,  // 46: xstockstrat.analysis.v1.ManageStrategyRequest.definition:type_name -> xstockstrat.analysis.v1.StrategyDefinition
	86,  // 47: xstockstrat.analysis.v1.ManageStrategyRequest.update_mask:type_name -> google.protobuf.FieldMask
	35,  // 48: xstockstrat.analysis.v1.ListStrategyDefinitionsResponse.definitions:type_name -> xstockstrat.analysis.v1.StrategyDefinition
	35,  // 49: xstockstrat.analysis.v1.SetStrategyLiveResponse.definition:type_name -> xstockstrat.analysis.v1.StrategyDefinition
	8,   // 50: xstockstrat.analysis.v1.ScreenCriterion.kind:type_name -> xstockstrat.analysis.v1.ScreenKind
	34,  // 51: xstockstrat.analysis.v1.ScreenCriterion.component:type_name -> xstockstrat.analysis.v1.StrategyComponent
	7,   // 52: xstockstrat.analysis.v1.ScreenCriterion.op:type_name -> xstockstrat.analysis.v1.Comparator
	76,  // 53: xstockstrat.analysis.v1.ScreenResult.criterion_scores:type_name -> xstockstrat.analysis.v1.ScreenResult.CriterionScoresEntry
	9,   // 54: xstockstrat.analysis.v1.ScreenResult.status:type_name -> xstockstrat.analysis.v1.ScreenResultStatus
	17,  // 55: xstockstrat.analysis.v1.ScreenResult.gap:type_name -> xstockstrat.analysis.v1.CoverageGap
	77,  // 56: xstockstrat.analysis.v1.ScreenResult.criterion_raw_values:type_name -> xstockstrat.analysis.v1.ScreenResult.CriterionRawValuesEntry
	78,  // 57: xstockstrat.analysis.v1.ScreenResult.criterion_passed:type_name -> xstockstrat.analysis.v1.ScreenResult.CriterionPassedEntry
	42,  // 58: xstockstrat.analysis.v1.ScreenSymbolsRequest.criteria:type_name -> xstockstrat.analysis.v1.ScreenCriterion
	80,  // 59: xstockstrat.analysis.v1.ScreenSymbolsRequest.evaluation_window:type_name -> xstockstrat.common.v1.TimeRange
	43,  // 60: xstockstrat.analysis.v1.ScreenSymbolsResponse.results:type_name -> xstockstrat.analysis.v1.ScreenResult
	17,  // 61: xstockstrat.analysis.v1.ScreenSymbolsResponse.coverage_gaps:type_name -> xstockstrat.analysis.v1.CoverageGap
	83,  // 62: xstockstrat.analysis.v1.FundamentalsScanSummary.finished_at:type_name -> google.protobuf.Timestamp
	10,  // 63: xstockstrat.analysis.v1.Opportunity.action:type_name -> xstockstrat.analysis.v1.OpportunityActionTag
	83,  // 64: xstockstrat.analysis.v1.Opportunity.valid_until:type_name -> google.protobuf.Timestamp
	49,  // 65: xstockstrat.analysis.v1.Opportunity.sparkline:type_name -> xstockstrat.analysis.v1.SparklinePoint
	50,  // 66: xstockstrat.analysis.v1.Opportunity.conditions:type_name -> xstockstrat.analysis.v1.ConditionEval
	11,  // 67: xstockstrat.analysis.v1.ConditionEval.state:type_name -> xstockstrat.analysis.v1.ConditionState
	50,  // 68: xstockstrat.analysis.v1.SymbolReadiness.conditions:type_name -> xstockstrat.analysis.v1.ConditionEval
	84,  // 69: xstockstrat.analysis.v1.ListOpportunitiesRequest.page:type_name -> xstockstrat.common.v1.PageRequest
	48,  // 70: xstockstrat.analysis.v1.ListOpportunitiesResponse.opportunities:type_name -> xstockstrat.analysis.v1.Opportunity
	85,  // 71: xstockstrat.analysis.v1.ListOpportunitiesResponse.page:type_name -> xstockstrat.common.v1.PageResponse
	12,  // 72: xstockstrat.analysis.v1.EvaluateReadinessRequest.rule:type_name -> xstockstrat.analysis.v1.ReadinessRule
	51,  // 73: xstockstrat.analysis.v1.EvaluateReadinessResponse.readiness:type_name -> xstockstrat.analysis.v1.SymbolReadiness
	83,  // 74: xstockstrat.analysis.v1.EvaluateReadinessResponse.computed_at:type_name -> google.protobuf.Timestamp
	13,  // 75: xstockstrat.analysis.v1.SetOpportunityActionRequest.action:type_name -> xstockstrat.analysis.v1.OpportunityAction
	83,  // 76: xstockstrat.analysis.v1.SetOpportunityActionRequest.snooze_until:type_name -> google.protobuf.Timestamp
	83,  // 77: xstockstrat.analysis.v1.GetIndicatorSeriesRequest.times:type_name -> google.protobuf.Timestamp
	83,  // 78: xstockstrat.analysis.v1.GetIndicatorSeriesResponse.times:type_name -> google.protobuf.Timestamp
	62,  // 79: xstockstrat.analysis.v1.GetIndicatorSeriesResponse.components:type_name -> xstockstrat.analysis.v1.ComponentSeries
	5,   // 80: xstockstrat.analysis.v1.ComponentSeries.kind:type_name -> xstockstrat.analysis.v1.ComponentKind
	63,  // 81: xstockstrat.analysis.v1.ComponentSeries.series:type_name -> xstockstrat.analysis.v1.NamedSeries
	64,  // 82: xstockstrat.analysis.v1.NamedSeries.values:type_name -> xstockstrat.analysis.v1.IndicatorValue
	14,  // 83: xstockstrat.analysis.v1.OrderSnapshot.event_type:type_name -> xstockstrat.analysis.v1.SnapshotEventType
	83,  // 84: xstockstrat.analysis.v1.OrderSnapshot.event_ts:type_name -> google.protobuf.Timestamp
	81,  // 85: xstockstrat.analysis.v1.OrderSnapshot.ohlcv_bar:type_name -> google.protobuf.Struct
	79,  // 86: xstockstrat.analysis.v1.OrderSnapshot.indicator_values:type_name -> xstockstrat.analysis.v1.OrderSnapshot.IndicatorValuesEntry
	65,  // 87: xstockstrat.analysis.v1.OrderSnapshot.signals:type_name -> xstockstrat.analysis.v1.SignalEntry
	15,  // 88: xstockstrat.analysis.v1.PnLPatternFactor.factor_type:type_name -> xstockstrat.analysis.v1.FactorType
	83,  // 89: xstockstrat.analysis.v1.QueryPnLPatternsRequest.from_ts:type_name -> google.protobuf.Timestamp
	83,  // 90: xstockstrat.analysis.v1.QueryPnLPatternsRequest.to_ts:type_name -> google.protobuf.Timestamp
	67,  // 91: xstockstrat.analysis.v1.QueryPnLPatternsResponse.positive_factors:type_name -> xstockstrat.analysis.v1.PnLPatternFactor
	67,  // 92: xstockstrat.analysis.v1.QueryPnLPatternsResponse.negative_factors:type_name -> xstockstrat.analysis.v1.PnLPatternFactor
	83,  // 93: xstockstrat.analysis.v1.GetAttributionRequest.start:type_name -> google.protobuf.Timestamp
	83,  // 94: xstockstrat.analysis.v1.GetAttributionRequest.end:type_name -> google.protobuf.Timestamp
	71,  // 95: xstockstrat.analysis.v1.GetAttributionResponse.attributions:type_name -> xstockstrat.analysis.v1.SourceAttribution
	16,  // 96: xstockstrat.analysis.v1.AnalysisService.RunBacktest:input_type -> xstockstrat.analysis.v1.RunBacktestRequest
	24,  // 97: xstockstrat.analysis.v1.AnalysisService.ScoreStrategy:input_type -> xstockstrat.analysis.v1.ScoreStrategyRequest
	31,  // 98: xstockstrat.analysis.v1.AnalysisService.ListStrategies:input_type -> xstockstrat.analysis.v1.ListStrategiesRequest
	33,  // 99: xstockstrat.analysis.v1.AnalysisService.GetStrategyReport:input_type -> xstockstrat.analysis.v1.GetStrategyReportRequest
	27,  // 100: xstockstrat.analysis.v1.AnalysisService.ListBacktests:input_type -> xstockstrat.analysis.v1.ListBacktestsRequest
	30,  // 101: xstockstrat.analysis.v1.AnalysisService.GetBacktest:input_type -> xstockstrat.analysis.v1.GetBacktestRequest
	36,  // 102: xstockstrat.analysis.v1.AnalysisService.ManageStrategy:input_type -> xstockstrat.analysis.v1.ManageStrategyRequest
	37,  // 103: xstockstrat.analysis.v1.AnalysisService.GetStrategy:input_type -> xstockstrat.analysis.v1.GetStrategyRequest
	38,  // 104: xstockstrat.analysis.v1.AnalysisService.ListStrategyDefinitions:input_type -> xstockstrat.analysis.v1.ListStrategyDefinitionsRequest
	40,  // 105: xstockstrat.analysis.v1.AnalysisService.SetStrategyLive:input_type -> xstockstrat.analysis.v1.SetStrategyLiveRequest
	44,  // 106: xstockstrat.analysis.v1.AnalysisService.ScreenSymbols:input_type -> xstockstrat.analysis.v1.ScreenSymbolsRequest
	46,  // 107: xstockstrat.analysis.v1.AnalysisService.RunFundamentalsScan:input_type -> xstockstrat.analysis.v1.RunFundamentalsScanRequest
	53,  // 108: xstockstrat.analysis.v1.AnalysisService.ListOpportunities:input_type -> xstockstrat.analysis.v1.ListOpportunitiesRequest
	55,  // 109: xstockstrat.analysis.v1.AnalysisService.EvaluateReadiness:input_type -> xstockstrat.analysis.v1.EvaluateReadinessRequest
	57,  // 110: xstockstrat.analysis.v1.AnalysisService.SetOpportunityAction:input_type -> xstockstrat.analysis.v1.SetOpportunityActionRequest
	59,  // 111: xstockstrat.analysis.v1.AnalysisService.GetStrategyAnalytics:input_type -> xstockstrat.analysis.v1.GetStrategyAnalyticsRequest
	60,  // 112: xstockstrat.analysis.v1.AnalysisService.GetIndicatorSeries:input_type -> xstockstrat.analysis.v1.GetIndicatorSeriesRequest
	68,  // 113: xstockstrat.analysis.v1.AnalysisService.QueryPnLPatterns:input_type -> xstockstrat.analysis.v1.QueryPnLPatternsRequest
	70,  // 114: xstockstrat.analysis.v1.AnalysisService.GetAttribution:input_type -> xstockstrat.analysis.v1.GetAttributionRequest
	18,  // 115: xstockstrat.analysis.v1.AnalysisService.RunBacktest:output_type -> xstockstrat.analysis.v1.BacktestResult
	25,  // 116: xstockstrat.analysis.v1.AnalysisService.ScoreStrategy:output_type -> xstockstrat.analysis.v1.StrategyScore
	32,  // 117: xstockstrat.analysis.v1.AnalysisService.ListStrategies:output_type -> xstockstrat.analysis.v1.ListStrategiesResponse
	26,  // 118: xstockstrat.analysis.v1.AnalysisService.GetStrategyReport:output_type -> xstockstrat.analysis.v1.StrategyReport
	29,  // 119: xstockstrat.analysis.v1.AnalysisService.ListBacktests:output_type -> xstockstrat.analysis.v1.ListBacktestsResponse
	18,  // 120: xstockstrat.analysis.v1.AnalysisService.GetBacktest:output_type -> xstockstrat.analysis.v1.BacktestResult
	35,  // 121: xstockstrat.analysis.v1.AnalysisService.ManageStrategy:output_type -> xstockstrat.analysis.v1.StrategyDefinition
	35,  // 122: xstockstrat.analysis.v1.AnalysisService.GetStrategy:output_type -> xstockstrat.analysis.v1.StrategyDefinition
	39,  // 123: xstockstrat.analysis.v1.AnalysisService.ListStrategyDefinitions:output_type -> xstockstrat.analysis.v1.ListStrategyDefinitionsResponse
	41,  // 124: xstockstrat.analysis.v1.AnalysisService.SetStrategyLive:output_type -> xstockstrat.analysis.v1.SetStrategyLiveResponse
	45,  // 125: xstockstrat.analysis.v1.AnalysisService.ScreenSymbols:output_type -> xstockstrat.analysis.v1.ScreenSymbolsResponse
	47,  // 126: xstockstrat.analysis.v1.AnalysisService.RunFundamentalsScan:output_type -> xstockstrat.analysis.v1.FundamentalsScanSummary
	54,  // 127: xstockstrat.analysis.v1.AnalysisService.ListOpportunities:output_type -> xstockstrat.analysis.v1.ListOpportunitiesResponse
	56,  // 128: xstockstrat.analysis.v1.AnalysisService.EvaluateReadiness:output_type -> xstockstrat.analysis.v1.EvaluateReadinessResponse
	58,  // 129: xstockstrat.analysis.v1.AnalysisService.SetOpportunityAction:output_type -> xstockstrat.analysis.v1.SetOpportunityActionResponse
	52,  // 130: xstockstrat.analysis.v1.AnalysisService.GetStrategyAnalytics:output_type -> xstockstrat.analysis.v1.StrategyAnalytics
	61,  // 131: xstockstrat.analysis.v1.AnalysisService.GetIndicatorSeries:output_type -> xstockstrat.analysis.v1.GetIndicatorSeriesResponse
	69,  // 132: xstockstrat.analysis.v1.AnalysisService.QueryPnLPatterns:output_type -> xstockstrat.analysis.v1.QueryPnLPatternsResponse
	72,  // 133: xstockstrat.analysis.v1.AnalysisService.GetAttribution:output_type -> xstockstrat.analysis.v1.GetAttributionResponse
	115, // [115:134] is the sub-list for method output_type
	96,  // [96:115] is the sub-list for method input_type
	96,  // [96:96] is the sub-list for extension type_name
	96,  // [96:96] is the sub-list for extension extendee
	0,   // [0:96] is the sub-list for field type_name
}

func init() { file_analysis_v1_analysis_proto_init() }
func file_analysis_v1_analysis_proto_init() {
	if File_analysis_v1_analysis_proto != nil {
		return
	}
	file_analysis_v1_analysis_proto_msgTypes[19].OneofWrappers = []any{}
	file_analysis_v1_analysis_proto_msgTypes[32].OneofWrappers = []any{}
	file_analysis_v1_analysis_proto_msgTypes[33].OneofWrappers = []any{}
	file_analysis_v1_analysis_proto_msgTypes[48].OneofWrappers = []any{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_analysis_v1_analysis_proto_rawDesc), len(file_analysis_v1_analysis_proto_rawDesc)),
			NumEnums:      16,
			NumMessages:   64,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_analysis_v1_analysis_proto_goTypes,
		DependencyIndexes: file_analysis_v1_analysis_proto_depIdxs,
		EnumInfos:         file_analysis_v1_analysis_proto_enumTypes,
		MessageInfos:      file_analysis_v1_analysis_proto_msgTypes,
	}.Build()
	File_analysis_v1_analysis_proto = out.File
	file_analysis_v1_analysis_proto_goTypes = nil
	file_analysis_v1_analysis_proto_depIdxs = nil
}
