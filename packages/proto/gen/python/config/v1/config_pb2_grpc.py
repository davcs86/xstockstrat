# Generated by the gRPC Python protocol compiler plugin. DO NOT EDIT!
"""Client and server classes corresponding to protobuf-defined services."""
import grpc
import warnings

from config.v1 import config_pb2 as config_dot_v1_dot_config__pb2

GRPC_GENERATED_VERSION = '1.80.0'
GRPC_VERSION = grpc.__version__
_version_not_supported = False

try:
    from grpc._utilities import first_version_is_lower
    _version_not_supported = first_version_is_lower(GRPC_VERSION, GRPC_GENERATED_VERSION)
except ImportError:
    _version_not_supported = True

if _version_not_supported:
    raise RuntimeError(
        f'The grpc package installed is at version {GRPC_VERSION},'
        + ' but the generated code in config/v1/config_pb2_grpc.py depends on'
        + f' grpcio>={GRPC_GENERATED_VERSION}.'
        + f' Please upgrade your grpc module to grpcio>={GRPC_GENERATED_VERSION}'
        + f' or downgrade your generated code using grpcio-tools<={GRPC_VERSION}.'
    )


class ConfigServiceStub(object):
    """ConfigService — live configuration via server-streaming WatchConfig.
    All services call WatchConfig at startup and stream config updates.
    Config values are scoped by environment (dev/production) and trading_mode (paper/live/all).
    """

    def __init__(self, channel):
        """Constructor.

        Args:
            channel: A grpc.Channel.
        """
        self.WatchConfig = channel.unary_stream(
                '/xstockstrat.config.v1.ConfigService/WatchConfig',
                request_serializer=config_dot_v1_dot_config__pb2.WatchConfigRequest.SerializeToString,
                response_deserializer=config_dot_v1_dot_config__pb2.ConfigSnapshot.FromString,
                _registered_method=True)
        self.GetConfig = channel.unary_unary(
                '/xstockstrat.config.v1.ConfigService/GetConfig',
                request_serializer=config_dot_v1_dot_config__pb2.GetConfigRequest.SerializeToString,
                response_deserializer=config_dot_v1_dot_config__pb2.ConfigSnapshot.FromString,
                _registered_method=True)
        self.SetConfig = channel.unary_unary(
                '/xstockstrat.config.v1.ConfigService/SetConfig',
                request_serializer=config_dot_v1_dot_config__pb2.SetConfigRequest.SerializeToString,
                response_deserializer=config_dot_v1_dot_config__pb2.SetConfigResponse.FromString,
                _registered_method=True)
        self.ListKeys = channel.unary_unary(
                '/xstockstrat.config.v1.ConfigService/ListKeys',
                request_serializer=config_dot_v1_dot_config__pb2.ListKeysRequest.SerializeToString,
                response_deserializer=config_dot_v1_dot_config__pb2.ListKeysResponse.FromString,
                _registered_method=True)


class ConfigServiceServicer(object):
    """ConfigService — live configuration via server-streaming WatchConfig.
    All services call WatchConfig at startup and stream config updates.
    Config values are scoped by environment (dev/production) and trading_mode (paper/live/all).
    """

    def WatchConfig(self, request, context):
        """Subscribe to config updates for a given namespace/service.
        Server streams updates as config changes; initial snapshot is the first message.
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def GetConfig(self, request, context):
        """One-shot config fetch (startup fast-path before stream establishes)
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def SetConfig(self, request, context):
        """Admin: set or update a config value
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def ListKeys(self, request, context):
        """Admin: list all keys for a namespace
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')


def add_ConfigServiceServicer_to_server(servicer, server):
    rpc_method_handlers = {
            'WatchConfig': grpc.unary_stream_rpc_method_handler(
                    servicer.WatchConfig,
                    request_deserializer=config_dot_v1_dot_config__pb2.WatchConfigRequest.FromString,
                    response_serializer=config_dot_v1_dot_config__pb2.ConfigSnapshot.SerializeToString,
            ),
            'GetConfig': grpc.unary_unary_rpc_method_handler(
                    servicer.GetConfig,
                    request_deserializer=config_dot_v1_dot_config__pb2.GetConfigRequest.FromString,
                    response_serializer=config_dot_v1_dot_config__pb2.ConfigSnapshot.SerializeToString,
            ),
            'SetConfig': grpc.unary_unary_rpc_method_handler(
                    servicer.SetConfig,
                    request_deserializer=config_dot_v1_dot_config__pb2.SetConfigRequest.FromString,
                    response_serializer=config_dot_v1_dot_config__pb2.SetConfigResponse.SerializeToString,
            ),
            'ListKeys': grpc.unary_unary_rpc_method_handler(
                    servicer.ListKeys,
                    request_deserializer=config_dot_v1_dot_config__pb2.ListKeysRequest.FromString,
                    response_serializer=config_dot_v1_dot_config__pb2.ListKeysResponse.SerializeToString,
            ),
    }
    generic_handler = grpc.method_handlers_generic_handler(
            'xstockstrat.config.v1.ConfigService', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))
    server.add_registered_method_handlers('xstockstrat.config.v1.ConfigService', rpc_method_handlers)


 # This class is part of an EXPERIMENTAL API.
class ConfigService(object):
    """ConfigService — live configuration via server-streaming WatchConfig.
    All services call WatchConfig at startup and stream config updates.
    Config values are scoped by environment (dev/production) and trading_mode (paper/live/all).
    """

    @staticmethod
    def WatchConfig(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_stream(
            request,
            target,
            '/xstockstrat.config.v1.ConfigService/WatchConfig',
            config_dot_v1_dot_config__pb2.WatchConfigRequest.SerializeToString,
            config_dot_v1_dot_config__pb2.ConfigSnapshot.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def GetConfig(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/xstockstrat.config.v1.ConfigService/GetConfig',
            config_dot_v1_dot_config__pb2.GetConfigRequest.SerializeToString,
            config_dot_v1_dot_config__pb2.ConfigSnapshot.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def SetConfig(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/xstockstrat.config.v1.ConfigService/SetConfig',
            config_dot_v1_dot_config__pb2.SetConfigRequest.SerializeToString,
            config_dot_v1_dot_config__pb2.SetConfigResponse.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def ListKeys(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/xstockstrat.config.v1.ConfigService/ListKeys',
            config_dot_v1_dot_config__pb2.ListKeysRequest.SerializeToString,
            config_dot_v1_dot_config__pb2.ListKeysResponse.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)
