...
| **INDICATORS-5** | **Header authz is read per-method off `context.invocation_metadata()` (admin = `x-access-scope & 0x04`; author = `x-user-id`) — there is no interceptor.** | Python services thread propagation per method (not via a Go-style interceptor); this service only *reads* inbound metadata and makes no outbound per-request calls. | `app/handlers/servicer.py:37` (`_has_admin_scope`), author read `:214-219` | `app/handlers/servicer.py:37` |
