...
|---|---|---|---|---|
| **CONFIG-1** | **A "DELTA" broadcast carries the ENTIRE namespace, not just changed keys** — see `CLAUDE.md` § WatchConfig Flow for the wire mechanism. | Emitting a true partial delta (changed keys only) would make the consumer silently drop every unchanged key on the wholesale replace. The full-namespace-per-DELTA is load-bearing. | consumer `src/services/configWatcher.ts:50-58` | `src/grpc/configServiceImpl.ts:178-200` |
