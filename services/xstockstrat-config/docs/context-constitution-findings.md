...
| Service CLAUDE.md pg_notify payload documented as `{namespace, key}` | Code emits `{namespace, key, environment, trading_mode}` and depends on env+mode (CONFIG-4) | `CLAUDE.md:54` vs `configServiceImpl.ts:348` | ✓ **RESOLVED** (2026-08-02 refresh) — CLAUDE.md now documents `pg_notify('config_changed', {namespace, key, environment, trading_mode})` |
