...
| **Non-admin users still see a functional config-ui Edit/Save affordance.** Since feature 074, `SetConfig` is admin-gated at both the BFF and the RPC, but `src/app/config-ui/[namespace]/page.tsx` gates the Edit/Save buttons only on `isSecret` — a viewer/trader can still open the editor and only learns on submit, via a raw `Save error: Admin scope required`. `useIsAdmin()` exists and is unused in this segment. Same class as the audit-route row above. | Misleading affordance; poor error UX, not an authorization hole | `src/app/config-ui/[namespace]/page.tsx`; `src/hooks/useLiveStrategies.ts` (`useIsAdmin`) |
