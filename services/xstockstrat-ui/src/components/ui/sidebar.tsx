...
function SidebarMenuButton({
  ...
  const button = (
    <Comp
      ...
      data-size={size}
      // app-specific: `isActive || undefined` (not the bare boolean) so the attribute is omitted
      // entirely when inactive. The bare `data-active:*` Tailwind variant below matches on
      // attribute PRESENCE, not its string value — `data-active="false"` still matches
      // `[data-active]`, defeating the whole active/inactive distinction. See
      // xstockstrat-ui/CLAUDE.md § Styling for the reconciliation note.
...
function SidebarMenuSubButton({
  ...
  return (
    <Comp
      ...
      data-size={size}
      // app-specific: same fix as SidebarMenuButton — see the comment there.
      data-active={isActive || undefined}
      className={cn(
