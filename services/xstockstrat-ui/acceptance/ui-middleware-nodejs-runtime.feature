# Durable per-service business rules for xstockstrat-ui, promoted from feature 128's
# acceptance.feature at launch (Constitution C-16). Source-feature provenance is carried on every
# scenario's `@feature-128` tag. These are the guarantees a future feature must not silently break.
