# Durable business-rule suite for xstockstrat-ui (Constitution C-16). Promoted from feature 155's
# acceptance.feature on code-completion; @feature-155 marks provenance. These are the guarantees a
# future feature must not regress (recon reads this suite; the design-adversary enforces it).

Feature: Watchlist & Opportunities signal cues (color + icon coding, nav/mobile/filter)
