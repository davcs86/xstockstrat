...
| `marketdata.stream.bar_ingest_interval_ms` | int | `300000` | Interval for the always-on bar ingester that upserts recent bars for every queried symbol into `marketdata.ohlcv`. Read live each cycle; `0`/negative pauses it. Default 5m (feature 140): with continuous **daily** ingestion, the forming daily bar needn't refresh every 60s — the UI's 5-min chart poll and `GetBars`' staleness fallback additionally cover freshness. |
| `marketdata.stream.bar_ingest_timeframe` | string (comma-separated list) | `1d` | Bar timeframe(s) the always-on ingester fetches each cycle. A comma-separated list mechanism (not a single value), parsed by the count-agnostic `resolveIngestTimeframes` — feature 143 narrowed the *default* from `15m,1d` to `1d` (feature 140 flipped the platform to daily) because `GetBars`/`BackfillBars` now reject any non-`1d` request, so continuously ingesting 15m served no consumer. Continuous `1d` ingestion keeps daily bars fresh. Each entry is canonicalized through `internal/timeframe`; an unresolvable entry is skipped with a WARN (not a whole-list fallback), and an empty/wholly-unresolvable value falls back to the default. To pause ingestion, use `bar_ingest_interval_ms <= 0` — never a bogus timeframe value. |
| `marketdata.stream.bar_ingest_lookback_ms` | int | `900000` | Lookback window the always-on ingester re-fetches each cycle. Overlap is harmless because inserts upsert, and a window wider than the interval lets the feed self-heal after a pause/restart. This raw value (15min) is far too short for the `1d` default — but `ingestRecentBars` widens each timeframe's own window to at least 2× that timeframe's interval (`minIngestLookback`), so `1d` always looks back ≥48h regardless of this key's value. |
...
| `marketdata.fundamentals.provider` | string | `finnhub` | Selects the active `source.FundamentalsSource` (`finnhub` \| `fmp`). Read **once at boot** (`cmd/server/main.go`) — unlike every other fundamentals key above, changing it requires a restart; the active client object and the config-key dispatch it drives must never diverge mid-process. **Note (feature 198):** this selector governs only the latest-*snapshot* source. The historical point-in-time source (SEC EDGAR) is a **separate lane** — never a value here — so adding it never perturbs this selector or the provider-keyed FMP cap. |
| `marketdata.fundamentals.history.enabled` | bool | `false` | Master gate for the historical point-in-time fundamentals **backfill** (feature 198). Read on `BackfillFundamentals`; the as-of read (`GetHistoricalFundamentals`) is not gated (a stored filing is already public). |
...
- `GetLatestQuote` serves from the `marketdata.quotes` cache, falling back to a live Alpaca call (and caching the result). A background warm poller (`StartWarmQuotePoller`) keeps every queried symbol's latest quote fresh in the DB so per-position P&L reads avoid repeated live calls. **`GetLatestQuotes` (batch, feature 178)** is the same cache-first path for many symbols in one RPC: a batched `DISTINCT ON (symbol)` repo read (`GetLatestQuotesBatch`, over the existing `idx_quotes_symbol_time` — no migration) for the warm hits, then one `alpaca.GetLatestQuotesMulti` for the cold remainder **coalesced under a `singleflight.Group` keyed on the sorted cold set** (concurrent callers share one upstream fetch), caching each cold quote. The response is a partial `repeated Quote` — a symbol with no quote is omitted (null-not-zero), never zero-filled. `xstockstrat-portfolio` calls it in place of the old per-position `GetLatestQuote` fan-out. It prefers one multi-symbol fetch per cycle (`GetLatestQuotesMulti`) and falls back to per-symbol calls; per-symbol fetch errors are aggregated into a single WARN per cycle (`failed`/`fetched`/`total` + a sample error) instead of being dropped silently, so a whole-feed failure (e.g. bad credentials, where every call 401s) is visible rather than hidden
...
- `StartBarIngestPoller` is an **always-on** bar ingester started at boot. Each cycle it upserts recent bars, for every configured `bar_ingest_timeframe` entry (default `1d`), for every warm symbol — the demand-driven set populated by `GetLatestQuote`/`GetBars` — so ingestion runs continuously without a client holding a `StreamBars` RPC open. Each timeframe's window is at least `bar_ingest_lookback_ms`, widened to 2× that timeframe's own interval when the configured value is too short to ever cover it (`minIngestLookback` — see `bar_ingest_lookback_ms` above). The legacy `StreamBars`/`StartBarStream` path (a poll that only runs for the duration of a client stream) remains for explicit subscribers
