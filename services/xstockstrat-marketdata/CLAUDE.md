...
| `marketdata.stream.bar_ingest_timeframe` | string (comma-separated list) | `15m,1d` | Bar timeframe(s) the always-on ingester fetches each cycle — a comma-separated list, not a single value (bug fix: a single `"15m"` left every OTHER continuously-consumed timeframe, chiefly `1d`, frozen at whatever `GetBars`' one-time live-fallback happened to cache, since that fallback only fires on a first-page DB miss). 15m is the smallest supported interval. Each entry is canonicalized through `internal/timeframe`; an unresolvable entry is skipped with a WARN (not a whole-list fallback), and an empty/wholly-unresolvable value falls back to the full default list. To pause ingestion, use `bar_ingest_interval_ms <= 0` — never a bogus timeframe value. |
| `marketdata.stream.bar_ingest_lookback_ms` | int | `900000` | Lookback window the always-on ingester re-fetches each cycle, sized for the `15m` entry (default 15m). Overlap is harmless because inserts upsert, and a window wider than the interval lets the feed self-heal after a pause/restart. **Not** used unmodified for a coarser configured timeframe — `ingestRecentBars` widens each timeframe's own window to at least 2× that timeframe's interval (`minIngestLookback`), so e.g. `1d` always looks back ≥48h regardless of this key's value. |
...
- `StartBarIngestPoller` is an **always-on** bar ingester started at boot. Each cycle it upserts recent bars, for every configured `bar_ingest_timeframe` entry (default `15m,1d`), for every warm symbol — the demand-driven set populated by `GetLatestQuote`/`GetBars` — so ingestion runs continuously without a client holding a `StreamBars` RPC open. Each timeframe's window is at least `bar_ingest_lookback_ms`, widened to 2× that timeframe's own interval when the configured value is too short to ever cover it (bug fix — see `bar_ingest_lookback_ms` above). The legacy `StreamBars`/`StartBarStream` path (a 60s poll that only runs for the duration of a client stream) remains for explicit subscribers
