...
| `marketdata.stream.bar_ingest_timeframe` | string (comma-separated list) | `1d` | Bar timeframe(s) the always-on ingester fetches each cycle. Still a comma-separated list mechanism (not a single value), parsed by the count-agnostic `resolveIngestTimeframes` — feature 143 narrowed the *default* from `15m,1d` to `1d` because `GetBars`/`BackfillBars` now reject any non-`1d` request, so continuously ingesting 15m served no consumer. Continuous `1d` ingestion keeps daily bars fresh (a single one-time-cached daily bar would otherwise go stale — `GetBars`' live-fallback only fires on a first-page DB miss). Each entry is canonicalized through `internal/timeframe`; an unresolvable entry is skipped with a WARN (not a whole-list fallback), and an empty/wholly-unresolvable value falls back to the default. To pause ingestion, use `bar_ingest_interval_ms <= 0` — never a bogus timeframe value. |
