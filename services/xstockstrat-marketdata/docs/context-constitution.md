...
|---|---|---|---|---|
| **MARKETDATA-1** | **`Bar.Timeframe` is stored/queried as the canonical DB string (`15m`/`1h`/`1d`), never the Alpaca spelling.** The Alpaca wire spelling (`15Min`/`1Hour`/`1Day`) is used only on the outbound request, then the canonical string is written back onto the Bar. Since feature 080, `Bar.TimeframeEnum` is set alongside the string at every producer site via `internal/timeframe.FromString`. | Reads filter `WHERE timeframe=$2` on the canonical string; a writer that stores the Alpaca spelling makes bars invisible to reads — the "1Day vs 1d" bug `internal/timeframe` was created to fix. | canonical `internal/timeframe/timeframe.go:78`; wire map `internal/alpaca/client.go:112`; shared builder `internal/alpaca/client.go:143-154` (write-back `:152`); read `internal/repository/marketdata_repo.go:89,149` | `internal/timeframe/timeframe.go:78` |
| **MARKETDATA-2** | **Streamed bars carry `1m` and must NOT be persisted** — the stream path deliberately does not `InsertBars`; storage of the continuously-ingested timeframe (`marketdata.stream.bar_ingest_timeframe`, default `1d` since feature 143 narrowed it from `15m,1d` — still a comma-separated list mechanism, just one element by default) is owned by `StartBarIngestPoller`/`ingestRecentBars`. Since feature 143, `15m`/`1h` are neither ingested nor fetchable — `GetBars`/`BackfillBars` reject any non-`1d` request; historical `15m`/`1h` rows stay stored and remain inspectable via `GetDataCoverage`/deletable via `DeleteBackfilledData` (both deliberately permissive), just never refreshed. Since feature 080, streamed bars also carry the explicit `TimeframeEnum: TIMEFRAME_1MIN` label — this is a label only, not a storability signal; the persistence rule is unchanged. | "Fixing" the stream path to persist pollutes the ohlcv table with 1m bars. Assuming `1h` is continuously fresh (as an earlier version of this rule implied) will silently feed a stale bar to any `1h` consumer — same failure mode the bug fix closed for `1d`. | producer `internal/alpaca/stream.go:29,260` (enum label `:268`); consumer `internal/service/marketdata_service.go:816-850`; ingest list `internal/service/marketdata_service.go:520-610` (`resolveIngestTimeframes`/`ingestRecentBars`) | `internal/service/marketdata_service.go:816-850` |
...
| **MARKETDATA-5** | **Pollers re-read their interval from config every tick and treat `<=0` as "pause, keep last".** | Reading the interval once at startup means config changes never take effect live; `<=0` is the documented pause sentinel. | `marketdata_service.go:404,485` | `internal/service/marketdata_service.go:404` |
| **MARKETDATA-6** | **`DeleteBars` always constrains on the symbol predicate (`$1`) — never an unbounded delete — and is gated on `scope & 0x04`.** `buildDeleteBarsQuery` is extracted as a pure function precisely to unit-test this. | An empty symbol or a changed bitmask turns a targeted delete into a table wipe (DBA gate, FR-5). | `internal/repository/marketdata_repo.go:168`; gate `marketdata_service.go:296` | `internal/repository/marketdata_repo.go:168` |
| **MARKETDATA-7** | **`resolveEnvironment`/`resolveTradingMode` fail *open* to dev/paper on any unrecognized value** — only the literal `"production"`/`"live"` strings match; everything else (including a typo like `"staging"`) silently resolves to `ENVIRONMENT_DEV`/`TRADING_MODE_PAPER`. | A misconfigured `APPLICATION_ENV`/`TRADING_MODE` doesn't error — it silently subscribes this service to dev-scoped config (root PLAT-8). Don't "fix" this into a strict match; that would break first-boot ergonomics — but when debugging "prod config isn't applying," check this first. | `internal/config/config.go:94-108` (`resolveEnvironment`/`resolveTradingMode`) | `internal/config/config.go:94-108` |
| **MARKETDATA-8** | **`GetBars` is dual-mode by range presence.** No `Range.Start` = an implicit-window "latest bars" read: it sizes a default lookback (`defaultBarLookback`), serves the *newest* page via `QueryRecentBars` (not oldest-forward `QueryBars`), and is the **only** path that arms the feature-140 staleness self-refresh (newest stored bar older than one interval → live refetch, rate-limited by `staleCheckDue`) and warms the symbol for the daily ingester. An explicit `Range.Start` = an oldest-page-forward paginated (backtest) read. | Never "fix" a missing window to error, and never run the staleness refetch on explicit-start (backtest) reads — querying is the sole freshness trigger (no chart view required); the DB-miss fallback fires on every window but the staleness refetch fires only on the implicit path (feature 140). Distinct from MARKETDATA-3's cache-miss idiom. | implicit window `marketdata_service.go:175,184,196`; staleness `:214-220`, `staleCheckDue:241`; autonomous-freshness contract `:486-490`; `QueryRecentBars` `internal/repository/marketdata_repo.go:164` | `internal/service/marketdata_service.go:175-196` |
