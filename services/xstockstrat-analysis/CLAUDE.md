...
| `analysis.readiness_materializer.enabled` | bool | `false` | Master kill-switch for the readiness materializer background loop (feature 180). Read via `get_bool` (HasField — an explicit operator value is honored). Default OFF. No seed migration (the `analysis.*` no-seed pattern). |
| `analysis.readiness_materializer.refresh_hour_utc` | int | `0` | Wall-clock UTC hour for the daily readiness re-warm (feature 180). **Dedicated** anchor, decoupled from `analysis.opportunity.refresh_hour_utc` so the two daily loops tune independently (D-2). Read **presence-aware** (`get_int_present`) — `0` = midnight is legitimate. No seed migration. |
| `analysis.readiness_materializer.valid_window_hours` | int | `24` | Backstop TTL (hours) for a materialized row's `valid_until` (feature 180); the **authoritative** freshness bust is the `bar_epoch`-aware FAST gate (`is_readiness_row_fresh`), so this is only a floor. Read via `get_int_present`; `readiness_valid_until` clamps to a 1h minimum. No seed migration. |
| `analysis.readiness_materializer.max_concurrent_bars_fetches` | int | `2` | The materializer loop's **own** bars-fetch semaphore (feature 180), **separate** from `analysis.opportunity.max_concurrent_bars_fetches` so a background pre-warm never starves interactive readiness (feature-176 priority-inversion guard). Read once in `__init__` via `get_int` with a `max(1, …)` clamp. No seed migration. **feature 181:** this sem **also** throttles the `GetWatchlistReadiness` on-read background refresh kick (R-F coupling) — the on-read kick deliberately shares it rather than the interactive `_bars_fetch_sem`. |
