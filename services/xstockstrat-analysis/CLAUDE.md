...
- **`ListOpportunities`** — a **pure read** of the materialized per-user queue (`analysis.opportunities`, migration `011`) LEFT JOIN the persisted dispositions (`analysis.opportunity_actions`, migration `010`). The Universe = `active signals (QuerySignals) ∪ held positions (ListPositions) ∪ watchlist (symbol, strategy) bindings (ListWatchlists)` — held positions and watchlisted symbols now **add rows**, not merely flip a signal row's action. Each row carries **real `passing/total`** wherever a strategy is attributed: entry-rule trace for entry candidates, **exit-rule trace** (`evaluate_conditions_traced(..., rule="exit")`) for held+attributed candidates → a held position whose `exit_rule` fires is a `REDUCE` row **with no sell signal** (FR-8). Attribution is **watchlist-binding-first, else unattributed** (`strategy_id=""`, no trace, `0/0`) — held positions carry no portfolio strategy, so none is fabricated (P-03). `conviction` is the deterministic readiness ordinal; a signal is a **separate ranking axis** (`signal_axis`), counted exactly once and never folded into readiness (Option 2 / FR-3). The key `user|symbol_norm|strategy_id` (action is a stored annotation, not in the key) is server-authoritative; the client echoes it to `SetOpportunityAction`. Freshness is **lazy compute-on-read + stale-while-revalidate + a configured daily refresh** (`run_opportunity_refresh_forever`, `analysis.opportunity.refresh_hour_utc`): a cold (never-materialized) read is **non-blocking** (feature 185 FR-4) — it kicks a background recompute and returns an empty page with a response-level `computing` pending signal (or `compute_failed` after `_OPPORTUNITY_COMPUTE_MAX_ATTEMPTS` consecutive failures), never a synchronous compute under the per-user lock; a stale read serves the old rows and kicks a background recompute (surface `computed_at` as "as of"). **Feature 177 (FR-3):** a *legitimately-empty* universe is no longer recomputed every poll — an empty compute stamps `analysis.opportunity_compute_state` (migration `023`) with an `analysis.opportunity.empty_recompute_ttl_seconds` window; within it the `count==0` branch serves empty and kicks a **background** self-heal (an empty→non-empty transition heals within ≈ one poll) instead of recomputing synchronously. The **three** empty-yielding `replace_for_user` sites (cold materialize, `_kick` recompute, daily refresh tick) route through `_replace_and_stamp_compute_state`, which stamps only on empty and leaves the daily tick's `schedule.advance` untouched (feature-158 `@AC-8/@AC-9`). **Feature 177 (FR-4):** read-time `_enrich_opportunities_live` memoizes each symbol's live quote+sparkline for `analysis.opportunity.live_enrich_ttl_seconds` — **success-only** (a failed/unavailable fetch is never cached, so a stale price is never served as current), so a warm re-read skips `GetLatestPrice`/`GetBars`. **Feature 185:** a per-symbol bars/indicator fetch failure during the compute is a **terminal data-unavailable sentinel** (`Opportunity.data_unavailable`, derived at read from the `"unavailable"` provenance marker — no column; conviction+signal_axis zeroed so it sinks) — distinct from an evaluated `0/N` quiet row; an indicators `grpc.RpcError` marks only that candidate unavailable instead of aborting the whole compute (a `FormulaExecutionError` still propagates). The compute bars-fetch fan-out runs on the background `_readiness_materializer_bars_sem` (FR-3, no new key). A served data-unavailable row older than `_OPPORTUNITY_UNAVAILABLE_RETRY_SECONDS` (300s) triggers a **surgical per-symbol read-time recovery** (`_kick_opportunity_retry` → `_retry_unavailable_symbols` → `opportunities.replace_symbols` heal-only UPDATE, never resurrecting a dropped row) that restores both ranking axes and upserts the readiness-cache subset for a recovered watchlist×strategy entry pair (success-only); the read `ORDER BY` gains an `opportunity_key ASC` tiebreak for paging stability. No migration. The known-user set the daily pass iterates is `opportunities ∪ opportunity_actions` (analysis can't enumerate all users — strategies are global). `max_universe_size` bounds the traced set with watchlist/held ranked **above the cut** (FR-1). **Feature 132 (deny list + owner universe):** a live strategy's coverage is now `resolve_universe(definition, watchlist, held, signals).union` — a non-empty `signal_params.symbols` allowlist is an explicit **override**, else `watchlist ∪ held ∪ (signals iff signal_eligible)`, all **minus** the strategy's `denied_symbols` (entry-only: a held-denied position keeps its exit trace). A denied `(symbol, strategy)` pair within that coverage surfaces as an explicit **muted** row — `Opportunity.muted`, carried by the `"denied"` provenance marker (no `muted` column; survives the DB round-trip) — held-denied flags its existing exit row, non-held-denied is a `0/0` `UNSPECIFIED` placeholder, **never `conviction=0` as a classifier**. The cut is three disjoint buckets (`curated`/`muted_only`/`speculative`) so a muted row is never truncated for a higher-conviction signal, and the read query exempts muted rows from the conviction floor (`OR provenance ? 'denied'`, and — feature 185 — `OR provenance ? 'unavailable'` for the data-unavailable sentinel). The three new proto fields (`denied_symbols`, `signal_eligible`, `Opportunity.muted`) all ride existing JSONB/rows — **no migration**. `signal_eligible` defaults **false**; setting it true alongside a non-empty allowlist is rejected `INVALID_ARGUMENT` at write time. The live loop (`live_loop.py`) evaluates the same `resolve_universe` universe under a **fair-share rotating scheduler** (all `(strategy, symbol)` pairs globally ordered by `created_at`, ≤ `max_strategies_per_cycle` per cycle) and fetches each owner's watchlist/held from portfolio (read-only) memoized per cycle.
...
- **`EvaluateReadiness`** — per-symbol traced condition leaves (`ConditionState` PASS/SOFT/FAIL) + a deterministic conviction ordinal (passing/total leaves, never a probability) for an explicit `strategy_id` — the Signal-detail readiness panel. **Feature 177 (FR-1):** verdicts are cached per `(user, strategy, rule, symbol)` in `analysis.readiness_cache` (migration `022`), keyed also on the definition fingerprint + `bar_epoch`. A repeat call whose fingerprint matches and whose row is within `analysis.readiness.stale_after_seconds` serves a **FAST** path (no bars fetch, no re-eval) that is byte-identical to a fresh compute; any miss/mismatch/expiry re-evaluates (**SLOW**) and re-stamps `bar_epoch = max(evaluated, benchmark)` — never a slow-path `bar_epoch` reuse. The response's `computed_at` (proto field 2, FR-5) = the **oldest** served row's stamp, so it is never presented fresher than its oldest symbol.
...
- **`GetWatchlistReadiness`** (feature 181) — **cache-first** readiness decoration for a keyset page of a watchlist's bound `(symbol, strategy_id)` pairs, backing the `/insights/watchlists` list (kills the client per-strategy `EvaluateReadiness` N+1 fan-out). Derives the owner's bound pairs from portfolio `GetWatchlist` over the **existing** analysis→portfolio edge (owner from `x-user-id`; **no new edge, no portfolio→analysis cycle**; anti-IDOR — pairs come from the owner's own watchlist), sorts `(symbol ASC, strategy_id ASC)`, keyset-slices (opaque base64 `(symbol, strategy_id)` cursor in `PageRequest.page_token` — the `ListPositions` keyset precedent, **not** `ListOpportunities`' offset), and reads `readiness_cache` **only** (a `GetDataCoverage` bar-epoch probe per page symbol — never a SLOW compute in the RPC body). **Four-way classifier** (order matters): `bar_epoch < 0` sentinel → `UNKNOWN`; else `is_readiness_row_fresh` → `RESOLVED` (inline `SymbolReadiness`); else → `PENDING`. Not-fresh pairs fire a **best-effort, guard-set-deduped background refresh** (`_kick_readiness_refresh`, keyed `(owner, strategy_id, symbol)`) that reuses the shared `compute_readiness_row`; **UNKNOWN** rows re-kick only past the `_READINESS_UNKNOWN_RETRY_SECONDS` (300s) recovery cooldown. Clients render rows immediately and poll this same RPC until every row is `RESOLVED` (design R-E). **Scope note:** the `bar_epoch = -1` data-unavailable sentinel is written by the **shared** `compute_readiness_row` (feature 180), so it is a (more-correct) behavior change to the interactive `EvaluateReadiness` + materializer paths too — a persistent primary-bars-fetch failure now re-computes SLOW rather than FAST-serving a degraded row (operator-approved deviation, feature 181 context.md).
  - **R-F (semaphore coupling):** the on-read refresh kick shares `analysis.readiness_materializer.max_concurrent_bars_fetches` (the materializer's own sem, **not** the interactive `_bars_fetch_sem` — feature-176 priority-inversion guard), so that key now also throttles interactive watchlist-read kicks.
  - **R-B (warm-first-render prerequisite):** a **warm FAST first render** (all rows `RESOLVED` on first paint) requires `analysis.readiness_materializer.enabled` (default **OFF**) with a `valid_window` covering the read cadence; with it off, a cold page renders `PENDING` and self-warms over one poll via the kick's 24h stamp. This feature does **not** flip the shipped default. The client N+1 elimination itself is unconditional (one page-bounded RPC + poll, never a per-strategy fan-out).
...
| `analysis.opportunity.max_universe_size` | int | `100` | Max candidates traced per opportunity compute (feature 097); watchlist/held rank **above the cut** so a curated symbol is never truncated — only the speculative signal tail is dropped (FR-1). Seeded by migration `028` (feature 184); bounds `[1,1000]`. |
| `analysis.opportunity.valid_window_hours` | int | `24` | `valid_until` = the compute's session date + this window (feature 097). Seeded by migration `028` (feature 184); bounds `[1,168]`. |
| `analysis.opportunity.snooze_default_hours` | int | `24` | Default bounded "snooze until" when a SNOOZE carries no explicit timestamp (feature 097). Seeded by migration `028` (feature 184); unbounded. |
| `analysis.opportunity.signal_rank_weight` | float | `0.3` | Weight `w ∈ [0,1]` of the independent signal axis in the queue ORDER BY (feature 097, OR-G); `rank = (1−w)·conviction + w·signal_axis`. This is a distinct scalar from the removed `analysis.signals.source_weights` key (superseded by feature 134, deleted by feature 161) — not a re-purpose. Seeded by migration `028` (feature 184); bounds `[0,1]`. |
| `analysis.opportunity.refresh_hour_utc` | int | `0` | Hour (UTC) of the **configured daily refresh** pass (feature 097) — a wall-clock refresh, **not** market close (holiday/DST/early-close drift is expected; a calendar-aligned refresh is a future feature). Read **presence-aware** (mirror `get_bool`'s `HasField`), never `get_int` — `0` = midnight is legitimate and the `get_int` zero-trap would swallow it. Also the wall-clock anchor for the DurableSchedule-backed opportunity refresh (feature 158). Seeded by migration `028` (feature 184); bounds `[0,23]`. |
| `analysis.opportunity.startup_jitter_seconds` | int | `30` | One-shot random delay `[0, N]` seconds applied once at the opportunity refresh loop entry to stagger concurrent redeploys (feature 158); read presence-aware (`get_int_present`) — `0` disables jitter. Mirrors `analysis.fundsignal.startup_jitter_seconds`. Seeded by migration `028` (feature 184); unbounded. |
| `analysis.opportunity.retry_seconds` | int | `300` | On a caught enumeration error the wall-clock `blocked_until_ms` advances by this many seconds (retry soon), not to the next wall-clock hour (feature 158); read presence-aware, clamped `max(1, …)` at the read site. Mirrors `analysis.fundsignal.retry_seconds`. Seeded by migration `028` (feature 184); unbounded. |
| `analysis.opportunity.max_live_strategies_per_symbol` | int | `5` | Per-symbol cap (feature 131): how many live-enabled strategies may **newly** attribute to one symbol via live-coverage. Enforced only at the two candidate-**creation** sites (`_capped_live`); tagging an already-existing curated row (a watchlist-bound or held strategy that is also live) is uncapped. Tiebreak is `created_at` ascending. AC-7. Seeded by migration `028` (feature 184); bounds `[1,50]`. |
| `analysis.opportunity.max_live_only_symbols_per_compute` | int | `20` | Cap (feature 131) on distinct **non-held** signal+live-covered symbols that get a new candidate row per compute pass (design step 6). Ranked by max active-signal conviction descending. Composes **multiplicatively** with the per-symbol cap. AC-8. Seeded by migration `028` (feature 184); bounds `[1,500]`. |
| `analysis.opportunity.max_live_held_symbols_per_compute` | int | `20` | Cap (feature 131) on distinct **held** symbols that may receive a new live-only strategy attribution per compute pass (ranked by held market value descending); does **not** bound the held-row count itself — every held symbol still yields ≥1 row. AC-9. Seeded by migration `028` (feature 184); bounds `[1,500]`. |
| `analysis.opportunity.max_concurrent_bars_fetches` | int | `2` | Process-lifetime singleton semaphore (`_bars_fetch_sem`) bounding cross-request concurrency of the **interactive** read-path bars-fetch calls — `_enrich_opportunities_live` + the `EvaluateReadiness` SLOW path (feature 141, SEV-2 fix for TimescaleDB "out of shared memory" under multi-user load). **Feature 185 (FR-3):** the `_compute_opportunities` bars-fetch fan-out was moved OFF this sem onto the background `_readiness_materializer_bars_sem` so a heavy cold/daily recompute can no longer starve an interactive read; this key now governs only the interactive paths. Mirrors `analysis.series.max_concurrent_components`'s shape, but default `2` (not `4`) to match `xstockstrat-marketdata`'s own `DB_POOL_MAX` default so analysis never queues more concurrent bars-fetch attempts than marketdata can actually execute. Read once in `AnalysisServicer.__init__` via `get_int` with a `max(1, …)` clamp. Seeded by migration `028` (feature 184); bounds `[1,5]`. |
| `analysis.opportunity.max_concurrent_candidates` | int | `4` | Bounds the per-candidate `evaluate_conditions_traced` fan-out in `_compute_opportunities` Phase 2 (feature 176) — **separate** from `analysis.opportunity.max_concurrent_bars_fetches` so a large opportunity compute cannot starve interactive readiness (priority-inversion guard). Read once in `AnalysisServicer.__init__` via `get_int` with a `max(1, …)` clamp. Seeded by migration `028` (feature 184); bounds `[1,50]`. |
| `analysis.compute.max_worker_threads` | int | `4` | Size of the dedicated `ThreadPoolExecutor` that runs the pure-CPU backtest simulator cores + the screener sync tail off the event loop (feature 176, FR-4). Executor threads open **no** DB connection (F-06: the asyncpg pool stays 2). Read once in `AnalysisServicer.__init__` via `get_int` with a `max(1, …)` clamp. No seed migration. |
| `analysis.opportunity.sparkline_bars` | int | `20` | Number of most-recent daily bar closes fetched per opportunity for the Decide-surface sparkline (feature 095, AC-3). Read live per read-time enrichment via `get_int` with a `max(1, …)` clamp;. Seeded by migration `028` (feature 184); bounds `[1,500]`. |
...
| `analysis.opportunity.empty_recompute_ttl_seconds` | int | `30` | FR-3 empty-universe recompute suppression window (feature 177). Read via `get_int_present`. Seeded by migration `028` (feature 184); unbounded. |
| `analysis.opportunity.live_enrich_ttl_seconds` | int | `10` | FR-4 short TTL for the success-only per-symbol live-quote/sparkline memo (feature 177). Read via `get_int_present` (`0` = memo disabled → always fetch). Seeded by migration `028` (feature 184); unbounded. |
| `analysis.readiness_materializer.enabled` | bool | `false` | Master kill-switch for the readiness materializer background loop (feature 180). Read via `get_bool` (HasField — an explicit operator value is honored). Default OFF. Seeded at the code default (`false`) by migration `027_analysis_readiness_materializer_keys` (feature 182) so config-ui shows it and an admin can toggle it. |
| `analysis.readiness_materializer.refresh_hour_utc` | int | `0` | Wall-clock UTC hour for the daily readiness re-warm (feature 180). **Dedicated** anchor, decoupled from `analysis.opportunity.refresh_hour_utc` so the two daily loops tune independently (D-2). Read **presence-aware** (`get_int_present`) — `0` = midnight is legitimate. Seeded (`0`) by migration `027` (feature 182); SetConfig-bounded `[0,23]` via `configServiceImpl.ts` `SCALAR_BOUNDS_REGISTRY`. |
| `analysis.readiness_materializer.valid_window_hours` | int | `24` | Backstop TTL (hours) for a materialized row's `valid_until` (feature 180); the **authoritative** freshness bust is the `bar_epoch`-aware FAST gate (`is_readiness_row_fresh`), so this is only a floor. Read via `get_int_present`; `readiness_valid_until` clamps to a 1h minimum. Seeded (`24`) by migration `027` (feature 182); SetConfig-bounded `[1,168]`. |
| `analysis.readiness_materializer.max_concurrent_bars_fetches` | int | `2` | The materializer loop's **own** bars-fetch semaphore (feature 180), **separate** from `analysis.opportunity.max_concurrent_bars_fetches` so a background pre-warm never starves interactive readiness (feature-176 priority-inversion guard). Read once in `__init__` via `get_int` with a `max(1, …)` clamp. Seeded (`2`) by migration `027` (feature 182); SetConfig-bounded `[1,5]` (ceiling = marketdata PgBouncer pool size, feature-141 SEV-2 guard) — never set `0` (the `get_int` zero-trap collapses it to `2`). **feature 181:** this sem **also** throttles the `GetWatchlistReadiness` on-read background refresh kick (R-F coupling) — the on-read kick deliberately shares it rather than the interactive `_bars_fetch_sem`. **feature 185 (FR-3):** the `_compute_opportunities` bars-fetch fan-out (primary + benchmark + the `_row_for` benchmark loader) *also* runs on this background sem, so a heavy cold/daily opportunity recompute cannot starve an interactive read; it reuses this key rather than minting a third `[1,5]` bars-fetch sem (3×`[1,5]`=15 would re-open the feature-141 SEV-2). |
