...
| `analysis.readiness_materializer.max_concurrent_bars_fetches` | int | `2` | The materializer loop's **own** bars-fetch semaphore (feature 180), **separate** from `analysis.opportunity.max_concurrent_bars_fetches` so a background pre-warm never starves interactive readiness (feature-176 priority-inversion guard). Read once in `__init__` via `get_int` with a `max(1, …)` clamp. No seed migration. |
