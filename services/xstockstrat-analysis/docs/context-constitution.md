...
- **Python config zero-trap applies to *every* int/float/str key here except the two opt-outs documented per-key in `CLAUDE.md`'s Config Keys table** (`get_int_present`, mirroring `get_bool`'s `HasField`) — every other int/float/str key here is still trapped (root gotcha; also a defect in findings). Evidence: `app/config/watcher.py:93,101,130` (trap) vs `:102-113` (`get_int_present`).
