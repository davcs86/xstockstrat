...
# Durable business rules xstockstrat-analysis guarantees for bounded-concurrency fan-out and
# CPU-offload of backtests/screener — deterministic output, user isolation, and latency fairness.

Feature: analysis-concurrency-offload
  As a platform operator launching xstockstrat for multiple concurrent users, I want the analysis
  service to parallelize bounded cross-service fan-out and run CPU-bound work off the event loop,
  so that list loads stay fast and per-user latency stays flat under concurrency.

  @AC-1 @FR-1 @feature-176
