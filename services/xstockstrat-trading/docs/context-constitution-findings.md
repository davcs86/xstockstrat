...
|---|---|---|---|
| CLAUDE.md "Ledger Events Emitted" table omits `account.halt.resumed` | It IS emitted (feature 169) | `internal/service/trading.go:3060` | Add the `account.halt.resumed` row |
| CLAUDE.md "Ledger Events Emitted" table omits `order_intent.reclaimed_unknown` | It IS emitted (feature 101 sweeper) | `internal/service/order_intent.go:166` | Add the `order_intent.reclaimed_unknown` row |
| `trading.order.max_retries` / `retry_delay_ms` documented as if `PlaceOrder`'s broker submission retries | **Updated 2026-08-07**: the keys *are* read (`internal/service/trading.go:2578-2579`, via `GetFloat`, not `GetInt` — the original "zero `GetInt` sites" claim was checking the wrong getter) but only inside `flattenAndHalt`'s retry loop (feature 030's protection-window flatten-and-halt), not `submitOrder`/`PlaceOrder`. `SubmitOrder` itself is still called exactly once with no retry loop. | `internal/service/trading.go:2578-2579` (`flattenAndHalt`) vs `trading.go:523` (`submitOrder`, no retry) | Not dead — but CLAUDE.md's "a retried submission" framing overstates scope; reword to scope the keys to the flatten-and-halt retry path, or extend them to `PlaceOrder` if that was the original intent |
