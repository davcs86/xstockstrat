...
|---|---|---|---|
| `trading.order.max_retries` / `retry_delay_ms` documented (CLAUDE.md:17) as if `PlaceOrder`'s broker submission retries | **Updated 2026-08-07**: the keys *are* read (`internal/service/trading.go:2155-2156`, via `GetFloat`, not `GetInt` — the original "zero `GetInt` sites" claim was checking the wrong getter) but only inside `flattenAndHalt`'s retry loop (feature 030's protection-window flatten-and-halt), not `submitOrder`/`PlaceOrder`. `SubmitOrder` itself is still called exactly once with no retry loop. | `internal/service/trading.go:2155-2156` (`flattenAndHalt`) vs `trading.go:342` (`submitOrder`, no retry) | Not dead — but CLAUDE.md:17's "a retried submission" framing overstates scope; reword to scope the keys to the flatten-and-halt retry path, or extend them to `PlaceOrder` if that was the original intent |
