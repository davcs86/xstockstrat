...
|---|---|---|---|---|
| **TRADING-1** | **Every RPC is implemented twice** — a Connect method *and* a `grpcTradingAdapter` twin — and only the adapter is registered (`grpcHdl.GRPCHandler()`). A new RPC must add both, or it is unreachable over the wire. | The served surface is the gRPC adapter; a Connect-only method compiles but is never registered. | `internal/handler/trading.go:31-176,178-209,280-318`; registration `cmd/server/main.go:129` | `internal/handler/trading.go:178-209` |
