...
| What | Where |
|---|---|
| `AccountRepo` reuses `TradingRepo.Pool()` (no second pool); cap=2; PgBouncer exec-mode (root PLAT-7) | `cmd/server/main.go:81`, `internal/repository/pool.go:15-41` (`defaultMaxConns:16`, `QueryExecModeExec:37`); root pool budget |
...
| Blocks on config `WatchConfig` snapshot (90s) before serving | `internal/config/config.go:164` (`WaitForSnapshot`); `docs/patterns/config-startup.md` |
