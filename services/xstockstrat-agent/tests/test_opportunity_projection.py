...
def test_composite_score_projected_with_value_and_omitted_when_unset():
    """@AC-9 (feature 199) — composite_score reaches the projection with its value, and an
    opportunity with no composite_score (NULL) omits the key entirely (never a fabricated 0.0)."""
    from gen.analysis.v1 import analysis_pb2

    present = _opportunity_to_dict(_full_opportunity(analysis_pb2), analysis_pb2)
    assert present["composite_score"] == 0.512
