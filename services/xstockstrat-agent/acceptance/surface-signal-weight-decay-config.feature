# Business-rule suite: xstockstrat-agent — surface-signal-weight-decay-config
...
Feature: agent signal-source reliability weight and decay config
  Agent tools expose per-source reliability_weight through list_signal_sources and
  manage_signal_source, and the descriptor-parity contract test guards against future proto/tool drift.

  @AC-1 @feature-161 @FR-1
