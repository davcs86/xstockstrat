# Promoted from docs/roadmap/features/149-manage-strategy-accept-object-rules/acceptance.feature at launch
# (Constitution C-16). Source-feature provenance is carried on every scenario's `@feature-149` tag.
...
Feature: manage-strategy-accept-object-rules
  Durable manage_strategy rule-serialization guarantees: entry_rule/exit_rule accept a JSON string or
  a JSON object (dict), an object is coerced to a byte-preserving JSON string, and omitted rules stay
  out of the update mask.

  @AC-1 @FR-1 @feature-149
