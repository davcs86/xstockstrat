...
|---|---|---|---|---|
| **NOTIFY-1** | **`AlertSeverity` has two wire forms and is converted at every DB boundary**: DB column is `INTEGER`, ts-proto `stringEnums` is a string (`"ALERT_SEVERITY_WARNING"`). Insert via `alertSeverityToNumber`, read via `alertSeverityFromJSON`; fan-out keeps the **string**. | Binding `req.severity` into the INTEGER column throws; reading `row.severity` into the proto maps to `UNRECOGNIZED`. Both shipped (PR #698). (Instance of root PLAT-F1/PLAT-2.) | `src/grpc/notifyServiceImpl.ts:47,183,63` | `src/grpc/notifyServiceImpl.ts:47` |
| **NOTIFY-2** | **All RPC errors collapse to a bare numeric gRPC code `13` (INTERNAL); `grpc.status` is never imported.** Match the house style. | Consistency across the file; returning `grpc.status.NOT_FOUND` etc. is harmless but out of style. | `notifyServiceImpl.ts:95,138,152` | `src/grpc/notifyServiceImpl.ts:95` |
...
- **Fan-out ignores `call.write()` backpressure** — `sub.call.write(alert)` drops a subscriber only on a thrown exception; the `false` return (buffer full / slow consumer) is discarded and there is no `drain` handling, so a slow subscriber buffers unbounded server-side. Flagged as an open question in findings. Evidence: `notifyServiceImpl.ts:81`.
- **`StreamSubscriber.severities` is typed `number[]` but the runtime fan-out value is a *string* enum** (`stringEnums`) — the only test exercises the numeric path, so CI won't catch a regression if someone numeric-converts one side. Recorded as a latent bug in findings. Evidence: `notifyServiceImpl.ts:12,167,63`.
