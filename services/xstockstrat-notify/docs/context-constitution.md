...
| **NOTIFY-2** | **All RPC errors collapse to a bare numeric gRPC code `13` (INTERNAL); `grpc.status` is never imported.** Match the house style. | Consistency across the file; returning `grpc.status.NOT_FOUND` etc. is harmless but out of style. | `notifyServiceImpl.ts:101,144,159` | `src/grpc/notifyServiceImpl.ts:101` |
...
| **NOTIFY-4** | **`EmitAlert` rejects an empty/whitespace-only `title` or `body` with gRPC code `3` (INVALID_ARGUMENT) *before* persisting** — proto3 strings default to `""`, so the `NOT NULL` columns would otherwise never fire on a blank alert. | A blank-alert guard must live in the handler, not the schema; the DB default masks it (feature 092, F-10). | `src/grpc/notifyServiceImpl.ts:32-37` (empty/whitespace `title` or `body` → `callback({ code: 3 })`) | `src/grpc/notifyServiceImpl.ts:32-37` |
