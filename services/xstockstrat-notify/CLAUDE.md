...
| `notify.push.min_severity` | int | `2` | Web Push gate: minimum `AlertSeverity` ordinal to send a Web Push (0=UNSPECIFIED,1=INFO,2=WARNING,3=ERROR,4=CRITICAL), clamped to [0,4]. Mirrors `notify.fanout.min_severity`; default 2 (WARNING) excludes INFO fill confirmations. Read live per dispatch. (feature 162) |

**Web Push (feature 162).** `src/fanout/webPush.ts` (`WebPushDispatcher`) is a **third, disjoint,
...
VAPID_PRIVATE_KEY=                  # type: SECRET (feature 162) — Web Push VAPID private key; empty ⇒ push disabled
VAPID_PUBLIC_KEY=                   # feature 162 — VAPID public key (also consumed by xstockstrat-ui); non-secret
VAPID_SUBJECT=                      # feature 162 — VAPID contact URL (mailto: or https:); non-secret; invalid/absent ⇒ push disabled
```
...
> that channel. Web Push needs **all three** VAPID values (feature 162).
