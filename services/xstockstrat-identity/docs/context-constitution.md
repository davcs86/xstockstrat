...
- **bcrypt cost=10 parity**: the seed hash (`$2b$10$…`) and `manage-users.py` (`BCRYPT_ROUNDS=10`) must agree with any hash-producing tool (compare reads cost from the hash, so it's non-load-bearing but keep it consistent). Two new hash sites both use cost 10: `createUser:682`, `updatePassword:740`. Evidence: `migrations/002_seed_admin.up.sql:12`, `identityServiceImpl.ts:682,740`.
...
| What | Where |
|---|---|
| `Date`-for-Timestamp on all responses (`secondsToDate`) | `identityServiceImpl.ts:44-46` (root PLAT-1/F2; PR #442) |
| pg pool cap 2 / `DB_POOL_MAX` | `src/index.ts:32-35`; root pool budget |
