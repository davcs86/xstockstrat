...
| CLAUDE.md Dependencies table lists only config + PostgreSQL — no ledger | identity now has a **REAL** outbound gRPC dependency on `xstockstrat-ledger` for admin-mutation audit (feature 043) with a `LEDGER_ENDPOINT` env var. (This is the same dep whose *fictional* prior form was "resolved" by deletion; it is now genuine and must be re-added.) | `src/grpc/ledgerAudit.ts:31-34`, wired `src/index.ts`, `LEDGER_ENDPOINT` | Add the ledger audit dep + `LEDGER_ENDPOINT` back to CLAUDE.md Dependencies + Env Vars (root §Header Propagation also corrected — see root findings) |
