...
| Test mocks / dummy data — canonical fixture homes, `INVENTORY.md` catalog, `/sdd-qa` skill, Constitution C-12 (frontend) and C-13 (all languages) | `docs/patterns/test-data-inventory.md` |
...
| Using or troubleshooting the agent MCP tools (a wire-connected agent consumes these as the generated `mcp-tools-docs` plugin skill — `plugins/mcp-tools-docs/`) | `docs/runbooks/mcp-tools.md` |
...
full and a single mandated adversarial round still happens (see `plugins/sdd-suite/skills/sdd-design/SKILL.md`).
...
These skills (and the subagents they orchestrate) ship in the **`sdd-suite` plugin** (`plugins/sdd-suite/`, enabled via `.claude/settings.json`); invoke them as `/sdd-suite:<skill>` or the bare `/sdd-<skill>` when unambiguous. See `docs/patterns/context-engineering.md` § 4.
...
| Generated Go stubs (never Read/Grep — checked-in codegen output; read the `.proto` source instead) | `packages/proto/gen/go/` |
| Generated Python stubs (never Read/Grep — checked-in codegen output; read the `.proto` source instead) | `packages/proto/gen/python/` |
| Generated TS stubs (never Read/Grep — checked-in codegen output; read the `.proto` source instead) | `packages/proto/gen/ts/` (compiled JS in `gen/ts/dist/`) |
...
| QA advisory subagent | `plugins/sdd-suite/agents/qa-tester.md` |
| QA skill (test design/run, coverage gaps, flake detection, fixture inventory, defect capture) | `plugins/sdd-suite/skills/sdd-qa/SKILL.md` |
| Plugin marketplace catalogs (Claude + Cursor) | `.claude-plugin/marketplace.json`, `.cursor-plugin/marketplace.json` |
| SDD suite plugin (the `/sdd-*` skills + advisory subagents) | `plugins/sdd-suite/` |
| MCP tools docs plugin + its runbook→skill generator | `plugins/mcp-tools-docs/`, `plugins/mcp-tools-docs/scripts/generate.py` |
