...
| `XSTOCKSTRAT_<SERVICE>_PRIVATE_URL` | `PRIVATE_URL` on DO (e.g. `http://svc.internal`), bare container name in Compose | **nginx container only** — `envsubst` upstream resolution; entrypoint strips `http://` prefix | `XSTOCKSTRAT_AGENT_PRIVATE_URL=xstockstrat-agent` |
