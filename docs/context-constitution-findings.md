...
|---|---|---|
| **Python config zero-trap is a consumer defect** (CF-N10): `get_int/get_str/get_float` use `v.int_val or default`, collapsing a legitimate stored `0`/`""` to the default, even though `config.ConfigValue` is a `oneof` that distinguishes 0 from unset and the Node getter (`??`) handles it correctly. | Setting any Python-consumed int/float/str key to `0`/`""` silently reverts to the coded default (e.g. `analysis.scoring.shrinkage_days=0`, chunk sizes, timeouts). Recorded as a constitution *gotcha* too (current behavior), and here as the fix. **Update (2026-08-09 refresh):** `xstockstrat-analysis` has since added a per-key opt-in fix, `get_int_present` (mirrors `get_bool`'s `HasField` pattern), used for exactly two keys (`analysis.strategy.default_exit_cooldown_days`, `analysis.opportunity.refresh_hour_utc`). `xstockstrat-indicators` and `xstockstrat-ingest` have no equivalent accessor — every other numeric/string key across all three services remains trapped. | `services/xstockstrat-indicators/app/config/watcher.py:93,101,117`; `ingest/app/config/watcher.py:93,101,117` (still trapped, no escape hatch); `analysis/app/config/watcher.py:95,101,130` (trap) vs `:103-114` (`get_int_present` fix) — fix: use `HasField` like `get_bool` |
