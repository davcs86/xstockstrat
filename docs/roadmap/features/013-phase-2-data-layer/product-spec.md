...
FR-2. `GetPnL` must process fills in two passes. **Pass 1**: query `order.filled` events (one per completed order); feed each into a per-symbol signed average-cost-basis accumulator; collect the set of `order_id`s that reached full completion. **Pass 2**: query `order.partially_filled` events (cumulative Alpaca polling updates); for each `order_id`, retain only the last event seen (highest `recorded_at`, since the payload's `filled_qty` is cumulative); for orders whose `order_id` is **not** in the Pass 1 completed set (i.e., the order was partially filled then canceled), feed the last partial fill — using `filled_qty` as quantity and `fill_price` as price — into the same accumulator. On a closing fill (opposite direction to current net position) in either pass, realized gain/loss is computed and accumulated into `realized_pnl`.
...
4. Multiple completed orders on the same symbol (each producing its own `order.filled` event) are processed independently and their realized P&L accumulates correctly via the average-cost-basis loop.
5. A buy order that is partially filled (e.g., 50 of 100 shares filled) and then canceled before full completion contributes the actually-filled quantity and average fill price to the cost-basis accumulator. The partially-filled shares are not silently dropped from P&L computation.
6. `unrealized_pnl` is unchanged for all portfolios — open-position computation is not regressed.
7. The gRPC call from `xstockstrat-portfolio` to `xstockstrat-ledger` propagates `x-user-id`, `x-access-scope`, and `x-trace-id` headers per the platform header-propagation convention.
8. Unit tests cover: closed long, closed short, no fills, multiple independent orders, mixed open+closed, partially-filled-then-canceled order.
9. After the trading service fix, `order.filled` events in the ledger contain a non-zero `fill_price` in their payload when an order is filled at a non-zero price. Both Alpaca and IBKR broker `GetOrder` implementations return `FilledAvgPrice` correctly parsed from their respective API response formats.
