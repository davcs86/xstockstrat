...
- Note: Pass 1 complete fills are applied before Pass 2 partial fills, regardless of chronological order. On Alpaca this produces correct results in all cases: Alpaca enforces buy-to-cover semantics (a buy order is routed to close any open short before opening a long), which structurally prevents the only scenario where ordering would matter — a partial fill closing a short that a later complete fill then re-closes. IBKR behavior unverified; likely correct for standard accounts (same buy-to-cover convention), unknown for portfolio-margin or hedge accounts that may hold simultaneous longs and shorts.
