...
- `services/xstockstrat-ui/pnpm-lock.yaml` — modify
- `services/xstockstrat-ui/src/app/globals.css` — modify
- `services/xstockstrat-ui/src/components/ui/{badge,button,card,combobox,input,select,separator,sheet,skeleton,table}.tsx` — modify (regenerated by the CLI)
...
2. Run the full Vitest unit suite with coverage; confirm the existing `src/lib/**` 40% threshold
   still passes — this feature does not touch `src/lib/**` production code, only adds the two new
   `src/components/ui/*.test.ts` files from Step 8, which are outside the coverage-scoped path.
3. Run the full Playwright e2e suite across all 4 segments — every spec under `e2e/trader/`,
...
4. Run the DRY duplication check.
5. Manually spot-check (per design.md § "7. Verification") the `buy`/`sell` order-side coloring on the
...
- Root `CLAUDE.md:109-116` — Language Versions & Tooling table (currently Go/Python/Node.js/pnpm
  rows, no Tailwind row); `:139-148` — Version Bump Workflow table/process to mirror.
...
2. In root `CLAUDE.md`'s Language Versions & Tooling table (`:109-116`), add a `Tailwind` row:
...
3. Add a `Tailwind` row to the Version Bump Workflow table (`:139-148`) pointing at
   `services/xstockstrat-ui/package.json` and `postcss.config.js` — do not invent a Dockerfile pin
   reference, since Tailwind is a devDependency, not a Docker base image.
