Feature: mcp-client-signal-source
  ...
  (endpoint + bearer token + which tool to query), so that xstockstrat-ingest pulls and ingests its
  signals automatically without the fetch being mediated by the Claude agent.
