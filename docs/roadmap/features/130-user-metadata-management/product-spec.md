...
FR-6. Add a `/config-ui/profile` page in `xstockstrat-ui` that displays the current user's metadata (including user_id as read-only) and allows editing email, phone, and display_name.
...
FR-9. `get_user_metadata` returns the caller's metadata (including read-only user_id). `set_user_metadata` accepts partial updates for email, phone, display_name, and the JSONB metadata field.
...
- `xstockstrat-ui` — new `/config-ui/profile` page, BFF routes, PLATFORM_SUBNAV registration
...
- [x] **UI** — `xstockstrat-ui` segment(s): `/config-ui` (new `/config-ui/profile` page showing user metadata with edit form; user_id displayed as read-only)
...
4. `/config-ui/profile` page renders the current user's metadata, displays user_id as non-editable, and allows saving changes to email, phone, and display_name.
5. Profile page is reachable from the config-ui navigation (PLATFORM_SUBNAV).
