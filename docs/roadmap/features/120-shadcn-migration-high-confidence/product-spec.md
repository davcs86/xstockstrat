...
FR-5. Add `src/components/ui/checkbox.tsx` (`npx shadcn@latest add checkbox`) and replace the two high-confidence raw `<input type="checkbox">` occurrences: `src/components/insights/FormulaWorkspace.tsx:278-285` (Public flag — corrected from the audit's `278-284`; recon.md confirmed the `<label>` block closes at line 285); `src/components/insights/ParameterEditor.tsx:236-244` (Required flag).

FR-6. **Adopt** the existing `src/components/ui/textarea.tsx` (already added by `119-shadcn-ui-migration` — no new primitive needed) and replace three related hand-written `<textarea>` class strings — the same duplicated shape (raw `<textarea>` + hand-rolled Tailwind classes) but not byte-identical across sites (each carries its own sizing/tone modifiers) — which `119` did not touch and are still hand-rolled: `src/components/insights/FormulaWorkspace.tsx:254-259` and `:351-356`; `src/components/insights/RuleEditor.tsx:327-335` (re-verified post-119; was `:274-283` at audit time — recon.md confirms `RuleEditor.tsx:327-335` is current).
...
FR-9. Add `src/components/ui/progress.tsx` (`npx shadcn@latest add progress`) and replace three occurrences of the same hand-rolled percentage-bar pattern — identical shape (a filled-width div over a track div), not identical markup, since sizing/coloring differs per site: `src/components/insights/SignalReadiness.tsx:71-82`; `src/components/insights/WatchlistReadiness.tsx:200-220`; `src/components/mobile/SectionRenderer.tsx:64-71`.
...
3. No two of the touched files still contain independent copies of the same widget's markup after migration (the duplicate timeframe tab-bar, the duplicate breadcrumb, the triplicated progress-bar shape, and the three related textarea class strings are each reduced to one shared implementation).
...
6. `docs/roadmap/features/120-shadcn-migration-high-confidence/context.md` records, per primitive, which call sites were migrated and confirms no visual regression. Pages/components with existing e2e coverage load-bearing on the replaced markup (`RuleEditor.tsx` Visual/JSON toggle, `screener/page.tsx` hard/rank toggle, `OrdersTable.tsx` cancel confirm, `OrderForm.tsx` Buy/Sell, `CopilotRail.tsx`, `WatchlistReadiness.tsx`, `PlatformHeader.tsx` breadcrumb — see recon.md § Risks / Not-found for the exact spec files and selectors) satisfy this via their passing e2e run; `config-ui/audit/page.tsx` has no e2e spec at all (recon.md confirmed) and needs a manual screenshot compare instead.

## Open Questions

- [x] **Resolved by `/sdd-design` recon** (recon.md § Risks / Not-found): grepped every touched file's corresponding `e2e/**/*.spec.ts`. Six call sites carry e2e-load-bearing selectors that must survive migration (`RuleEditor.tsx` role/aria-label, `screener/page.tsx` role/aria-label, `OrdersTable.tsx` data-testid, `OrderForm.tsx` exact-case role names, `CopilotRail.tsx` data-testid, `WatchlistReadiness.tsx` data-testid, `PlatformHeader.tsx` breadcrumb aria-label — 7 listed, see recon.md for the full path:line citations); the remaining call sites had no selector hits in a targeted (non-exhaustive) grep. `config-ui/audit/page.tsx` has no e2e spec at all.
- [x] **Resolved by `/sdd-design` recon** (recon.md § Risks / Not-found "PlatformHeader.tsx sequencing"): confirmed no in-flight feature besides sibling `121-shadcn-migration-medium-confidence` has a planned edit to `PlatformHeader.tsx` (`096-position-and-order-detail-pages` cites it in `recon.md` only, no edit in its `implementation-spec.md`; `112-watchlist-screen-improvements` is already `launched`). In-feature sequencing (FR-7 then FR-8, both inside this feature) is design.md's concern; cross-feature ordering with `121` is a merge-order note, not an open question.
