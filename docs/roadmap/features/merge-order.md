...
| `fundamentals-formula-inputs` (200) | `historical-fundamentals-backtest` (198) | **Consumed-seam build-order dependency** (`/sdd-review` product-spec overlap scan, 2026-09-21): 200 builds directly on 198's landed PIT machinery — the `_load_fundamentals` preload + `_fundamental_as_of_series` as-of alignment (`services/xstockstrat-analysis/app/services/evaluator.py`, `app/handlers/servicer.py`), the `analysis.backtest.fundamentals.enabled` config gate, and the `COMPONENT_KIND_FUNDAMENTAL`/`GetHistoricalFundamentals` proto — all of which must exist first. 198 is `code-completed` (already merged to `main-dev`); 200 was branched off the post-198 `main-dev`. **No resource collision**: 200's indicators migration `006`, its additive `fundamental_inputs` proto field + `FundamentalMetric` enum, and its reuse of 198's config key are all overlap-CLEAN (no other in-flight feature claims them). Soft same-file rebase only with features 196/198 in the analysis dir (disjoint regions). | No (198 code-completed, not yet launched) |
