...
| `broker-state-reconciliation` | `exactly-once-order-intent` | Reconciliation resolves the `UNKNOWN` order-intent state introduced by feature 101; the intent-record contract must exist first | No |
| `trading-state-machine-invariants` | `broker-failure-simulator` | Property-based tests are driven by the scripted broker event streams feature 103 produces | No |
| `trading-crash-consistency` | `exactly-once-order-intent` | Crash-consistency tests prove the durable order-intent model (feature 101) survives a restart; the model must exist first | No |
| `trading-crash-consistency` | `broker-failure-simulator` | Crash-consistency tests run against the feature-103 simulator and ephemeral PostgreSQL, not the real broker | No |
| `live-capital-canary-rollout` | `account-trading-halt-and-kill-switch` | Promotion evidence (FR-3) requires a successful emergency-halt exercise using feature 100's kill switch | No |
| `live-capital-canary-rollout` | `exactly-once-order-intent` | Promotion evidence requires "zero duplicate intents", measured by feature 101 | No |
| `live-capital-canary-rollout` | `broker-state-reconciliation` | Promotion evidence requires "zero unexplained reconciliation mismatches", measured by feature 102 | No |
| `live-capital-canary-rollout` | `position-sizing-engine` | Canary stage limits (notional/symbol/order-count caps) enforce alongside, not instead of, feature 023's sizing gate at the same order-path checkpoint | No |
| `live-capital-canary-rollout` | `stop-loss-bracket-orders` | Promotion evidence requires protection established within its SLO for every fill, delivered by feature 030 | No |
| `trading-safety-dashboard-slos` | `account-trading-halt-and-kill-switch` | Dashboard visualizes halt state/trigger telemetry feature 100 emits | No |
| `trading-safety-dashboard-slos` | `exactly-once-order-intent` | Dashboard visualizes order-intent state distribution (especially `UNKNOWN`) feature 101 emits | No |
| `trading-safety-dashboard-slos` | `broker-state-reconciliation` | Dashboard visualizes reconciliation mismatch/age telemetry feature 102 emits | No |
| `trading-safety-dashboard-slos` | `market-data-freshness-and-quality-gate` | Dashboard visualizes market-data freshness telemetry feature 106 emits | No |
| `live-trading-game-day` | `broker-failure-simulator` | Game-day exercises are run using the feature-103 fault-injection harness | No |
...
**Live-Capital Safety program build order (features 100–109, plus accelerated 023/030 — added
2026-08-04 from an external risk review):** `100 account-trading-halt-and-kill-switch` ∥
`101 exactly-once-order-intent` (no dependency on each other or on anything else — start first) →
`103 broker-failure-simulator` (no dependency; start alongside 100/101) → `102 broker-state-
reconciliation` (needs 101) and `104 trading-state-machine-invariants` (needs 103) in parallel →
`030 stop-loss-bracket-orders` (needs 023, already a pre-existing hard dependency) and
`105 trading-crash-consistency` (needs 101 + 103) in parallel → `106 market-data-freshness-and-
quality-gate` (no new hard dependency beyond marketdata's own observability) → `107 live-capital-
canary-rollout` (needs 100, 101, 102, 023, 030) → `108 trading-safety-dashboard-slos` (needs 100,
101, 102, 106) → `109 live-trading-game-day` (needs 103, most useful once 100/102/030 exist). See
each feature's `context.md` for the full rationale; `023-position-sizing-engine` and
`030-stop-loss-bracket-orders` were promoted to `P0` priority by the same review rather than
duplicated as new features.
