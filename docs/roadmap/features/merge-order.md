...
| `broker-state-reconciliation` | `stop-loss-bracket-orders` | **Proto field pre-assignment** (not a migration collision): 102's design.md claims `BrokerAccount` fields 9-12 (`halted`, `halted_at`, `halt_reason`, `halt_source`) on `packages/proto/trading/v1/trading.proto`. 030's own design.md is DB-only for its halt state and has not claimed a proto surface for the same columns — but nothing prevents 030 from independently deciding at its own future `/sdd-spec` to expose the same fields, racing 102. Whichever feature's `/sdd-spec` runs first should claim fields 9-12 explicitly and this row updated to reflect the actual claim | No |
