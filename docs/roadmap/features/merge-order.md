...
| `symbol-opportunity-ranking` (199) | `opportunity-composite-score` (198) | **Hard dependency**: 199 consumes `Opportunity.composite_score` (the per-opportunity quality input for the symbol roll-up, FR-1), and must take the next-free `Opportunity` proto field after 198's `composite_score = 21` **and** the next-free analysis migration NNN after 198's `024` (if 199 persists `symbol_score`). `buf breaking` / golang-migrate cannot see 198's uncommitted claims, so **198 must land first**; 199 re-derives both numbers from the merged tree at `/sdd-spec`. Also same-file rebase overlap with 198/187/193/188 on `servicer.py` `_compute_opportunities`, `opportunities.py` ORDER BY, `insights/opportunities/page.tsx`, and the agent `list_opportunities` projection | No |
