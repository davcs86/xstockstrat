...
| `symbol-opportunity-ranking` (200) | `opportunity-composite-score` (199) | **Hard dependency**: 200 consumes `Opportunity.composite_score` (the per-opportunity quality input for the symbol roll-up, FR-1), and must take the next-free `Opportunity` proto field after 199's `composite_score = 21` **and** the next-free analysis migration NNN after 199's `024` (if 200 persists `symbol_score`). `buf breaking` / golang-migrate cannot see 199's uncommitted claims, so **199 must land first**; 200 re-derives both numbers from the merged tree at `/sdd-spec`. Also same-file rebase overlap with 199/187/193/188 on `servicer.py` `_compute_opportunities`, `opportunities.py` ORDER BY, `insights/opportunities/page.tsx`, and the agent `list_opportunities` projection | No |
