...
| `signal-time-decay` (022) | `signal-source-reliability-weight` (134) | **Same-expression overlap** (not a field/config/migration collision): both features multiply an additional factor into the identical `c["signal_axis"] = max(c["signal_axis"], sig.conviction)` expression in `_compute_opportunities` (`services/xstockstrat-analysis/app/handlers/servicer.py:2163`) — 134 adds a `× source_weight` term, 022 adds a `× exp(-λ × age_hours)` term (022's own `product-spec.md` FR-1 names this coordination explicitly). 134 reached `spec-ready` first (2026-08-13); 022 executes second and rebases the expression onto 134's landed change to carry both terms. **Resolved 2026-08-15**: 134 merged (#953) and is an ancestor of `feature/signal-time-decay`; 022's Step 5 rebased onto the real landed `source_weights` factor (`effective = raw × source_weight × decay_multiplier`, D-1) | Yes |
| `backtest-next-bar-fill` (151) | `backtest-portfolio-sizing` (150) | **Proto field + migration pre-assignment (recorded 2026-08-23, at design-approved — not yet at spec).** Both features add additive fields to the SAME persisted-verbatim messages in `packages/proto/analysis/v1/analysis.proto` (`BacktestResult` bytes are stored verbatim in `analysis.backtest_details`, feature 068 — `analysis.proto:80-83`; `buf breaking` runs per-branch and cannot see the other branch's uncommitted field claim). Reserved split: **150** takes `RunBacktestRequest.sizing_mode = 8`, `BacktestResult` `17`/`18`/`19` (`sizing_mode`/`capital_skips`/`portfolio_equity_curve`), `BacktestRunSummary.sizing_mode = 17`, migration `017`; **151** takes `RunBacktestRequest.fill_model = 9`, `BacktestResult.fill_model = 20`, `BacktestRunSummary.fill_model = 18`, migration `018`. Both are `design-approved` (story+design only); each `/sdd-spec` run MUST re-derive the next-free numbers from the actually-merged tree and honor this split — if 150's implementation grows an extra field, 151 falls back to "next-free after 150" per its recon. Whichever lands second also renumbers its migration to the next free NNN. No hard build-order dependency between them (the two engine changes are orthogonal — sizing vs fill timing); this row governs field/migration non-collision only. **Same-function source overlap (added 2026-08-23, both `implementation-ready`):** the numbering is deconflicted, but both specs edit ~17 of the same files and three are **same-function** (manual-merge, not mechanical-rebase): `services/xstockstrat-analysis/app/handlers/servicer.py` (both touch the `RunBacktest` resolve-once/routing block, **both** simulators `_backtest_symbol`/`_backtest_symbol_evaluated`, and `_row_to_backtest_summary`), `services/xstockstrat-analysis/app/repositories/backtest_runs.py` (both add kwargs to `BacktestRunsRepository.insert`), and `services/xstockstrat-agent/app/backtest_view.py` + `tests/test_backtest_view.py` (both extend `_HEAD_KEYS` and must jointly satisfy the one descriptor-parity guard `kept | _INTENTIONALLY_DROPPED == BacktestResult.DESCRIPTOR.fields_by_name` — the second merger must account for the other's new `BacktestResult` fields or the test fails closed). Whichever lands second does a manual reconciliation of those files, re-derives its migration NNN to next-free, and re-runs `./scripts/buf-gen.sh` after merging the `.proto` edits | No |
| `signal-time-decay` (022) | `live-strategy-opportunity-attribution` (131) | **Same-function overlap, three-way with 134** (`/sdd-review` overlap scan, 2026-08-13): 131's FR-2 restructures the candidate-creation/signals-merge block in `_compute_opportunities` (`servicer.py:2144-2168`) to fold in a new `live_by_symbol` attribution origin — the identical block containing the `signal_axis` line (`:2163`) that 022 and 134 already coordinate on (row above). Recommended landing order: **134 → 131 → 022** — 134's factor addition is the simplest and already `spec-ready` first; 131 restructures which candidates enter the loop (structural, should land before further math edits); 022's decay factor composes last on top of whatever loop shape 131 leaves, landing on the combined `sig.conviction × source_weight × exp(-λ × age_hours)` expression. 131 executes independent of this row (its own dependency surface is clean per its own overlap scan) — this row governs 022's ordering relative to 131, not the reverse. **Resolved 2026-08-15**: 131 merged (#954) and is an ancestor of `feature/signal-time-decay`; the landing order 134 → 131 → 022 held, and Step 5's `sig_contribs` hoist was re-confirmed against 131's landed two-level signals-merge loop | Yes |
| `strategy-symbol-denylist` (132) | `strategy-user-ownership` (133) | **Hard dependency (owner-scoping + proto field coordination)**: 132's FR-3 union universe (`watchlist ∪ held ∪ signals − denied`) is owner-scoped via 133's `ListPositions(user_id=owner)` + synthetic-header `ListWatchlists` mechanism (133 design decision 6) and reads `definition.user_id` (133's `StrategyDefinition.user_id = 13`); 132's SetStrategyLive precondition edit targets 133's *rewritten* `get_by_owner_and_id` block. 132 also claims `StrategyDefinition` fields `12` (`denied_symbols`) and `14` (`signal_eligible`) around 133's `13` (`user_id`) — the second-merging feature re-verifies against the other's landed state. 133 merges first | No |
...
| `fundamentals-formula-inputs` (201) | `historical-fundamentals-backtest` (198) | **Consumed-seam build-order dependency** (`/sdd-review` product-spec overlap scan, 2026-09-21; renumbered 200→201 on 2026-09-23 to resolve a same-number collision with `symbol-opportunity-ranking`): 201 builds directly on 198's landed PIT machinery — the `_load_fundamentals` preload + `_fundamental_as_of_series` as-of alignment (`services/xstockstrat-analysis/app/services/evaluator.py`, `app/handlers/servicer.py`), the `analysis.backtest.fundamentals.enabled` config gate, and the `COMPONENT_KIND_FUNDAMENTAL`/`GetHistoricalFundamentals` proto — all of which must exist first. 198 is `code-completed` (already merged to `main-dev`); 201 was branched off the post-198 `main-dev`. **No resource collision**: 201's indicators migration `006`, its additive `fundamental_inputs` proto field + `FundamentalMetric` enum, and its reuse of 198's config key are all overlap-CLEAN (no other in-flight feature claims them). Soft same-file rebase only with features 196/198 in the analysis dir (disjoint regions). | No (198 code-completed, not yet launched) |
...
| `symbol-opportunity-ranking` (200) | `opportunity-composite-score` (199) | **Hard dependency**: 200 consumes `Opportunity.composite_score` (the per-opportunity quality input for the symbol roll-up, FR-1). **199 is now BUILT (`code-completed`, PR #1157)** — synced into 200's recon/context 2026-09-21: `composite_score` LANDED as proto `= 21` + column (migration `024`), so 200's reserved surfaces are `symbol_score = 22`, `OPPORTUNITY_SORT_SYMBOL_SCORE = 3`, migration `025`, `ANALYSIS-12` (re-confirmed free against the merged tree). **199 must still merge before 200.** Also same-file rebase overlap with 199/187/193/188 on `servicer.py` `_compute_opportunities`, `opportunities.py` ORDER BY, `insights/opportunities/page.tsx`, and the agent `list_opportunities` projection. **Update 2026-09-23:** 199 MERGED to `main-dev` (#1157); 200 was executed on a fresh branch off the post-199 `main-dev`, so `composite_score`/migration `024` are present and the reserved surfaces landed as `symbol_score = 22` / sort `3` / migration `025` / **`ANALYSIS-13`** (`ANALYSIS-12` was taken by the since-merged 201). | Yes (199 merged via #1157) |
| `formula-fundamental-inputs-authoring` (205) | `fundamentals-formula-inputs` (201) | **Consumed-seam build-order dependency** (`/sdd-review` product-spec overlap scan, 2026-09-24): 205 is the formula-authoring exposure layer built directly on 201's landed plumbing — it consumes 201's `fundamental_inputs` field (`FormulaDefinition = 14`, `RegisterFormulaRequest = 10`, `UpdateFormulaRequest = 11`), the `FundamentalMetric` enum (`packages/proto/indicators/v1/indicators.proto:136-149`), and indicators migration `006_add_formula_fundamental_inputs` — all delivered by 201, which must exist first. 201 is `code-completed` (merged to `main-dev`), so 205 was/should be branched off the post-201 tree. **No resource collision** (overlap-CLEAN): 205 adds no migration, no config key, and at most an additive `ListFundamentalMetrics` RPC on an uncontested indicators RPC/field-number surface. Soft rebase-only overlap with 187/197/198/199/200 in the shared agent `client.py`/`tools.py` (disjoint functions) and `docs/runbooks/mcp-tools.md` (section-disjoint). | No (201 code-completed, not yet launched) |
