...
FR-3. A shared system prompt file must be maintained at `services/xstockstrat-agent/app/prompts/signal_extraction.md`. It must encode:
  - The standard email ingestion flow:
    1. Call `list_signal_sources` filtered to `["mediated_email", "mediated_email_with_attachment", "mediated_email_with_linked_url"]`. Each source in the response includes `config_json` patterns and an `extractor_tool` field.
