...
- **Tool-count surfaces (52 → 43 on the current post-merge baseline; was 49→40 pre-merge):** `services/xstockstrat-ui/src/lib/copilot.ts:21`
...
- **CHANGE** `@AC-9` (agent tool count 42 + `db_` prefix contract) — the count drops (52→43 on the
...
- **PRESERVE** `@AC-6`/`@AC-7` (non-admin / unauthenticated denied `db_` tools) — the *principle* holds
  ...
  agent's `& 0x04`.
- **PRESERVE** the agent's own OAuth/JWT gate for its remaining 43 tools (unchanged).
...
- **[Deployment — RESOLVED to a HARD dependency on 084]** `084-droplet-compose-deploy` (`spec-ready`)
  restructures deployment around a droplet + Caddy. Per the amended design (`design.md`), the psql MCP
  is an **isolated on-demand `supervisord` container on the 084 droplet** (internal-only, `docker
  start/stop`), **not** a DO App Platform component/ingress route. 193 therefore **hard-depends on 084**
  (merge-order `084 → 193`); `/sdd-spec` grounds the psql-MCP compose block against 084's landed topology.
...
- **[Tool-count invariant]** 52→43 must update all six surfaces in lockstep or CI fails
...
3. **Remove db_* from the agent** — delete the 9 tool defs + `postgres_mcp_client.py` usage + the
   supervisord postgres-mcp entry + `POSTGRES_MCP_*` agent env; update all six tool-count surfaces to 43 (52-baseline − 9).
