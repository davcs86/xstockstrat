...
- **✓ RESOLVED (2026-09-21) — feature 198 (PIT) is now MERGED to `main-dev`.** The initial recon ran
  on a pre-198 checkout (hence its "unmerged" findings); re-verified against merged `main-dev`, the PIT
  seams exist at real `path:line`: `_load_fundamentals` `app/handlers/servicer.py:1452`, gate read
  `analysis.backtest.fundamentals.enabled` `servicer.py:1472`, `_definition_has_fundamental`
  `servicer.py:5048`, `_fundamental_as_of_series` `app/services/evaluator.py:69`, live-loop
  `_load_fundamentals` `app/engine/live_loop.py:585` (gate `:597`). `GetHistoricalFundamentals` +
  `COMPONENT_KIND_FUNDAMENTAL` are in the merged proto. **No build-order dependency remains**; feature
  200 builds both paths off the post-198 `main-dev` on its own branch (no stacking, no merge-order row).
