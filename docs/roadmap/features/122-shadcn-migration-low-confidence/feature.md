...
`/sdd-review shadcn-migration-low-confidence impl-spec` — validate implementation spec, then `/sdd-execute shadcn-migration-low-confidence` for Steps 1–8 (FR-1/FR-2/FR-3/FR-4). **FR-1's own migration steps are not yet specced** (blocked on sibling `120-shadcn-migration-high-confidence` shipping `ui/alert.tsx` — design.md § Round 4) — re-run `/sdd-spec shadcn-migration-low-confidence` once `120` merges to `main-dev` to plan them.
