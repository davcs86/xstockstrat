...
### 2026-09-17 — sysadmin-db-write-role — design
- **Pattern**: When a risky capability lives inside a prompt-injectable LLM surface (the MCP agent), an in-surface authorization gate (a scope bit, a confirm flag) cannot fully close it — a read-SHAPED statement performs writes via side-effect functions (`SELECT dblink_exec('INSERT …')`, `nextval`, `pg_terminate_backend`) that any syntactic classifier structurally misses. The sound fix is **privilege separation**: extract the capability into a standalone service authenticated by an INDEPENDENT credential (its own per-operator token, never the platform JWT/ACL), so the injectable surface has no tool, credential, or use for it. This supersedes the earlier "an agent-side MCP tool owns its own gate" insight (2026-09-16) for the prompt-injection threat model: own the gate for non-injectable callers, but separate the surface for injectable ones.
- **Corollaries**: (a) an alternative library with the RIGHT auth model but the WRONG tool model is still a reject — pgEdge shipped native Bearer auth but a read-only NL/RAG `query_database` + embedding deps, failing tool-parity for a raw-SQL write path; borrow its auth idea, not the library. (b) A compensating control decided at a design gate (rate-limit, per-operator token, durable audit) MUST become an FR + `@AC-*` scenario in the SAME pass, or it ships untested with CI green (the control-that-isn't-a-control failure). (c) A public endpoint's audit trail must be durable + non-optional (not `OTEL_ENABLED`-gated stdout, whose init errors are swallowed) and stored where the acting DML role cannot mutate it.
- **Evidence**: `docs/roadmap/features/193-sysadmin-db-write-role/design.md` (Chosen Approach + Rejected Alternatives); recon.md Candidate Architectures; security-audit H-5/DT-2 (`docs/reports/2026-09-16-trading-system-security-audit.md:133-151`).
- **Rule it implies**: for a risky capability reachable from an LLM that ingests untrusted content, prefer separating the capability behind an independent-auth boundary over gating it in-surface; and gate-time compensating decisions become acceptance scenarios in the same design pass.
