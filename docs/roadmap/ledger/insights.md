...
### 2026-09-26 — edgar-fundamentals-enrichment — design
- **Pattern**: When unifying two data sources to make two surfaces agree (here: backtest PIT/EDGAR vs live snapshot/vendor), aligning *some* metric conventions is not parity — every metric must be computed the same way on both surfaces. A single hidden per-metric convention can silently defeat the goal: ROE was `net_income/ending-equity` on EDGAR (~52% for BABA) but TTM/avg-equity on the vendor (~7%), so a "just fix the cheap PIT side" scope still left a large quality-sub-score divergence. The parity-forcing move is to make BOTH surfaces read ONE source, not to reconcile conventions pairwise. Corollary: a currency-consistency fix for one price-derived ratio (P/B) must be applied to every price-derived ratio (P/E), and dimensionless ratios need no per-row currency label while absolute facts do.
- **Evidence**: `docs/roadmap/features/207-edgar-fundamentals-enrichment/design.md` § Rejected Alternatives (PIT-only "(Y)"), § Chosen Approach steps 1/3; `services/xstockstrat-marketdata/internal/edgar/edgar_client.go:335-339` (ROE ending-equity), `:341-346` (D/E), `:317` (hardcoded USD).
- **Rule it implies**: A "make X and Y agree by sharing a source" design must enumerate EVERY field's computation convention on both sides before declaring parity; a per-metric convention mismatch (ROE base, currency basis) is a silent parity blocker. Reinforces C-18 (analyze the real staff-engineer scope, not the cheapest partial one).
