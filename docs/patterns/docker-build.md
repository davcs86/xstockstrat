...
1. **`python:3.13-slim`**: Lean base image (no dev tools, no pip)
...
**Characteristics:**
- Multi-stage: `builder` (golang:1.27-alpine) → `final` (gcr.io/distroless/static-debian12)
...
1. **`golang:1.27-alpine`**: Lean build base (no unnecessary tools)
...
```
golang:1.27-alpine builder:  300 MB
...
- **Node.js base**: `node:24-alpine` (pinned in root CLAUDE.md)
- **Python base**: `python:3.13-slim` (pinned in root CLAUDE.md)
- **Go base**: `golang:1.27-alpine` (pinned in root CLAUDE.md)
