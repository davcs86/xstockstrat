...
The bug is treated as a lightweight feature. It uses the existing SDD skill chain (an optional
`/sdd-design`, then `/sdd-spec`, `/sdd-execute`) with a pre-populated artifact set created by
`/sdd-triage`.
...
2. **Run `/sdd-triage <issue-number>`** — the skill creates
   ...
   the issue body, and `context.md`. Status: `draft`. Triage also prints a **design-depth
   recommendation** (skip / quick / full) based on severity and scope.
3. **(Optional) Run the recommended design depth** — `/sdd-design <slug>` (full debate) or
   `/sdd-design <slug> quick` (one adversarial round) for fixes worth a grounded design; or **skip**
   straight to step 4 for small, single-service, clear-root-cause bugs. A run writes `recon.md` +
   `design.md` and advances status to `design-approved`; `/sdd-spec` then consumes them.
4. **Run `/sdd-spec <slug>`** — generates the numbered implementation steps with grep-backed
   evidence, exactly as for a feature (consumes `design.md` if step 3 ran).
5. **Run `/sdd-execute <slug> next`** (repeat) — same step loop as features.
6. Integration PR from `feature/<slug>` to `main-dev`. Normal CI runs.
7. Status: `code-completed`. The fix rides the next `/promote` cycle to production.
8. The promote skill includes the bug in the CHANGELOG under "Bug Fixes".
...
- `docs/roadmap/features/<slug>/recon.md` + `design.md` (only when a design depth other than skip is run)
