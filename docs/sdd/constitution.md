...
| **C-05** | **Config key naming** is `<service>.<category>.<key>`; services subscribe at startup; defaults are declared in each service's `CLAUDE.md`; sensitive keys use the `secret.*` prefix. *(Feature 147 retired the `secret.*` prefix — secret-ness is now the `is_secret` flag alone — and re-permits encrypted secrets in config under a 3-guard pattern: encrypt at rest (`value_encrypted`, AES-256-GCM) + redact at every read/broadcast edge + decrypt only via the `GetSecret` resolver for allow-listed internal callers, with `is_secret` row-authoritative on write. The override of feature 076's "never a config key" ban was signed off by the operator, recorded in feature 147's `context.md`.)* | root `CLAUDE.md` § Config Governance Rules |
...
| **C-15** | **Acceptance scenarios trace to tests.** Every feature carries an `acceptance.feature` (Gherkin) whose `Scenario:` blocks each have a stable `@AC-N` tag and at least one `@FR-N` tag; every `FR-N` in `product-spec.md` is covered by ≥1 scenario. Every scenario must be covered by at least one `test` step / RED assertion (the step body names it: `**Covers**: AC-2, AC-5`); an **uncovered scenario blocks `impl-spec` review**. Scenario IDs (`@AC-*`) are **append-only per feature** — never renumbered, because test steps and RED assertions cite them. `acceptance.feature` replaces the prose `## Acceptance Criteria` list as the single source of acceptance truth. | `.claude/skills/sdd-story/SKILL.md` (acceptance.feature); `.claude/skills/sdd-spec/reference/spec-template.md` § Scenario coverage; `docs/sdd/business-rules/CLAUDE.md` |
| **C-16** | **Business-rule regression guard.** Recon (`/sdd-design` Phase 0) reads the durable business-rule suites of every affected service (`services/xstockstrat-<svc>/acceptance/*.feature` + `docs/sdd/business-rules/platform.feature`) and lists them in `recon.md`. A design that would **break an existing `@AC-*` guarantee is a regression** and is blocked (the design-adversary cites the rule ID, as it does a `C-*`/`F-*` breach). **Changing** (not merely adding to) an existing business rule requires **explicit user sign-off recorded in `context.md`**; `design.md` records which existing rules the approach preserves, extends, or changes. On launch/integration a feature's scenarios are **promoted** into the affected services' durable suites (`/sdd-execute` integration PR, `/promote` backstop); `/sdd-archiver` never deletes them, it curates them. | `.claude/skills/sdd-design/reference/recon-checklist.md`; `.claude/skills/sdd-design/reference/grilling-protocol.md`; `docs/sdd/business-rules/CLAUDE.md` |
| **C-17** | **UI/UX consistency in `xstockstrat-ui`.** Any new or changed UI (page, route, component, or state) **consumes design-role tokens** (never a hardcoded hex/`oklch`/`hsl` literal or arbitrary color utility — use `bg-background`/`text-destructive`/`text-buy`/`--chart-*`, etc.), **reuses an existing `ui/*` primitive + variant before adding a near-duplicate** (DRY guard rail), gives **every interactive/icon-only control a unique accessible name** with valid ARIA, and **routes loading/empty/error/form/table states through the canonical primitives** (`Skeleton`/`QueryStateMessages`/`EmptyState`/`CardNotice`/`FormDialog`/`DataTable`) rather than hand-rolled markup. New pages also satisfy **C-10(a)** nav registration. Full rules, the shared-shell contract, the accessibility baseline, and the known-deviation backlog live in `docs/patterns/ui-ux-governance.md`. | `docs/patterns/ui-ux-governance.md`; `services/xstockstrat-ui/CLAUDE.md` § Styling; `docs/patterns/dry-guard-rail.md` |
| **C-18** | **Design quality — SOLID, DRY, YAGNI, tempered by staff-engineer judgment.** Every design (`/sdd-design`) and code-bearing step applies the three heuristics to what it adds or reshapes: **YAGNI** — the least mechanism that satisfies the stated requirement; no speculative abstraction, option, extension point, or "while I'm here" generality the task didn't ask for (the root `CLAUDE.md` §How to Act #2 behavior). **DRY** — reuse an existing pattern/helper/type before adding a near-duplicate; the DRY guard rail (`docs/patterns/dry-guard-rail.md`: jscpd hook + `dry-reviewer` subagent) is the tooling arm. **SOLID** — each new/changed unit carries one clear responsibility and depends on stable seams, applied to the seams the change actually touches, never as a mandate to add interfaces/layers/indirection the change doesn't need. These heuristics are **subordinate to scalability, maintainability, and performance**: when one genuinely conflicts with a higher quality, the quality wins and the trade-off is recorded in `design.md`/`context.md` — **the recorded trade-off is compliance, not an override; only skipping the analysis altogether violates this rule.** The litmus for every design-quality point (raised by the design-adversary or rebutted by the design-proposer) is *what would a staff engineer optimizing the long-term health of THIS system do?* — so a dogmatic objection (premature abstraction dressed as SOLID, a DRY merge coupling genuinely unrelated call-sites, YAGNI invoked to starve a stated scalability need) is itself answerable at the design gate. The design-adversary cites C-18 as an **Objection** (addressed-or-waived), never a Floor breach. | root `CLAUDE.md` § How to Act (#2, minimum footprint); `docs/patterns/dry-guard-rail.md`; `.claude/agents/design-adversary.md` / `design-proposer.md` |
