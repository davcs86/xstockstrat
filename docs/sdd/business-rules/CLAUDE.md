...
2. **Canonical** — on launch/integration the scenarios are **promoted** (deduped) into the affected
   ...
   (`.claude/plugins/sdd-suite/agents/scenario-promoter.md`) — it maps each `@AC-*` to its owning suite (per-service or
...
- **Recon reads, never guesses.** `/sdd-design` Phase 0 spawns the read-only **`scenario-recon`**
  subagent (`.claude/plugins/sdd-suite/agents/scenario-recon.md`, the read-side mirror of `scenario-promoter`) to load
